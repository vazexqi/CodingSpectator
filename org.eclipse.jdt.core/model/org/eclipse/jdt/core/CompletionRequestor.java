...
/**
 * Abstract base class for a completion requestor which is passed completion proposals as they are
 * generated in response to a code assist request.
 * <p>
...
public abstract class CompletionRequestor {
	...
	/**
	 * Creates a new completion requestor. The requestor is interested in all kinds of completion
	...
	/**
	 * Creates a new completion requestor. If <code>ignoreAll</code> is <code>true</code> the
	 * requestor is not interested in all kinds of completion proposals; all will be ignored. For
	 * each kind of completion proposals that is of interest, <code>setIgnored(kind, false)</code>
	 * must be called. If <code>ignoreAll</code> is <code>false</code> the requestor is interested
	 * in all kinds of completion proposals; none will be ignored.
	 * 
	...
	/**
	 * Returns whether a proposal of a given kind with a required proposal of the given kind is
	 * allowed.
	 * 
	...
	/**
	 * Sets whether a proposal of a given kind with a required proposal of the given kind is
	 * allowed.
	 * 
	 ...
	 * e.g. <code>{"java.util.Arrays.*"}</code> It can be a reference to a static method or field
	 * (as in a static import)<br>
	...
	/**
	 * Set the favorite references which will be used to compute some completion proposals. A
	 * favorite reference is a qualified reference as it can be seen in an import statement.<br>
	 * 
	...
	/**
	 * Notification of failure to produce any completions. The problem object explains what
	 * prevented completing.
	 * <p>
	 ...
	 * <p>
	 * Similarly, implementers should check {@link #isIgnored(int) isIgnored(proposal.getKind())}
	 * and ignore proposals that have been declared as uninteresting. The proposal object passed is
	 * only valid for the duration of completion operation.
	 * 
