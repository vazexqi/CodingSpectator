...
/**
 * Represent the root Java element corresponding to the workspace. Since there is only one such root
 * element, it is commonly referred to as <em>the</em> Java model element. The Java model element
 * needs to be opened before it can be navigated or manipulated. The Java model element has no
 * parent (it is the root of the Java element hierarchy). Its children are <code>IJavaProject</code>
 * s.
 * <p>
 * This interface provides methods for performing copy, move, rename, and delete operations on
 * multiple Java elements.
 * </p>
 * <p>
...
public interface IJavaModel extends IJavaElement, IOpenable, IParent {
	/**
	 * Returns whether this Java model contains an <code>IJavaElement</code> whose resource is the
	 * given resource or a non-Java resource which is the given resource.
	 * <p>
	 * Note: no existency check is performed on the argument resource. If it is not accessible (see
	 * <code>IResource.isAccessible()</code>) yet but would be located in Java model range, then it
	 ...
	 * <p>
	 * Optionally, each copy can positioned before a sibling element. If <code>null</code> is
	 ...
	 * </p>
	 * <p>
	 * Optionally, any existing child in the destination container with the same name can be
	 * replaced by specifying <code>true</code> for force. Otherwise an exception is thrown in the
	 * event that a name collision occurs.
	 * </p>
	 * 
	 ...
	 *            name should be replaced, and <code>false</code> to throw an exception in the event
	 *            of a name collision
	 * @param monitor a progress monitor
	 * @exception JavaModelException if an element could not be copied. Reasons include:
	 *                <ul>
	 *                <li>There is no element to process (NO_ELEMENTS_TO_PROCESS). The given
	 *                elements is null or empty</li>
	...
	/**
	 * Returns the Java project with the given name. The given name must be a valid path segment as
	 * defined by {@link IPath#isValidSegment(String)}. This is a handle-only method. The project
	 ...
	 * 
	 * @return the Java projects in this Java model, or an empty array if there are none
	 ...
	 * </p>
	 * 
	 * @return an array of non-Java projects (<code>IProject</code>s) contained in the workspace.
	 ...
	 * <p>
	 * Optionally, each element can positioned before a sibling element. If <code>null</code> is
	 * specified for sibling, the element is inserted as the last child of its associated container.
	 * </p>
	 * <p>
	...
	/**
	 * Triggers an update of the JavaModel with respect to the referenced external archives. This
	 * operation will issue a JavaModel delta describing the discovered changes, in term of Java
	 * element package fragment roots added, removed or changed. Note that a collection of elements
	 * can be passed so as to narrow the set of archives to refresh (passing <code>null</code> along
	 * is equivalent to refreshing the entire mode). The elements can be:
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * In case an archive is used by multiple projects, the delta issued will account for all of
	 * them. This means that even if a project was not part of the elements scope, it may still be
	 * notified of changes if it is referencing a library comprised in the scope.
	 * <p>
	 * 
	 ...
	 * renamed within that container. If more than one container is specified, the number of
	 * elements and containers must match, and each element is renamed within its associated
	 * container.
	 * 
	 ...
	 *                <li>A child already exists with the same name and <code>replace</code> has
	 *                been specified as <code>false</code> (<code>NAME_COLLISION</code>)
