 ...
 * included. For example, if a type hierarchy is created for a <code>java.io.File</code>, and the
 * region the hierarchy was created in is the package fragment <code>java.io</code>, the supertype
 * <code>java.lang.Object</code> will still be included. As a historical quirk,
 * <code>java.lang.Object</code> has always been included in type hierarchies created on interface
 * types.
 * <p>
 ...
 * hierarchy, change notifications are sent to registered <code>ITypeHierarchyChangedListener</code>
...
public interface ITypeHierarchy {
	...
	/**
	 * Returns all classes in this type hierarchy's graph, in no particular order. Any classes in
	 * the creation region which were not resolved to have any subtypes or supertypes are not
	...
	/**
	 * Returns all resolved subtypes (direct and indirect) of the given type, in no particular
	 * order, limited to the types in this type hierarchy's graph. An empty array is returned if
	 * there are no resolved subtypes for the given type.
	 * 
	 * @param type the given type
	...
	/**
	 * Returns all resolved superinterfaces (direct and indirect) of the given type. If the given
	 * type is a class, this includes all superinterfaces of all superclasses. An empty array is
	 * returned if there are no resolved superinterfaces for the given type.
	 * 
	 * <p>
	 * NOTE: once a type hierarchy has been created, it is more efficient to query the hierarchy for
	 ...
	 * 
	 * @param type the given type
	 * @return all resolved superinterfaces (direct and indirect) of the given type, an empty array
	 *         if none
	 */
	 ...
	 * 
	 * @param type the given type
	 * @return all resolved supertypes of the given class, in bottom-up order, an empty array if
	 *         none
	 */
	 ...
	 * creation region which were not resolved to have any subtypes or supertypes are not included
	 * in the result.
	 * 
	 ...
	 * <code>IMember.getFlags()</code>), or <code>-1</code> if this information wasn't cached on the
	 * hierarchy during its computation.
	 * 
	 * @param type the given type
	...
	/**
	 * Returns all interfaces resolved to extend the given interface, in no particular order,
	 * limited to the interfaces in this hierarchy's graph. Returns an empty collection if the given
	 * type is a class, or if no interfaces were resolved to extend the given interface.
	 * 
	 * @param type the given type
	 * @return all interfaces resolved to extend the given interface limited to the interfaces in
	 *         this hierarchy's graph, an empty array if none.
	 */
	IType[] getExtendingInterfaces(IType type);

	/**
	 * Returns all classes resolved to implement the given interface, in no particular order,
	 * limited to the classes in this type hierarchy's graph. Returns an empty collection if the
	 * given type is a class, or if no classes were resolved to implement the given interface.
	 * 
	 * @param type the given type
	 ...
	 * the classes in this type hierarchy's graph. Returns an empty collection if the given type is
	 * an interface, or if no classes were resolved to be subclasses of the given class.
	 * 
	 * @param type the given type
	...
	/**
	 * Returns the direct resolved subtypes of the given type, in no particular order, limited to
	 * the types in this type hierarchy's graph. If the type is a class, this returns the resolved
	...
	/**
	 * Returns the resolved superclass of the given class, or <code>null</code> if the given class
	 * has no superclass, the superclass could not be resolved, or if the given type is an
	 * interface.
	 * 
	 * @param type the given type
	 * @return the resolved superclass of the given class, or <code>null</code> if the given class
	 *         has no superclass, the superclass could not be resolved, or if the given type is an
	 *         interface
	 */
	 ...
	 * gives the interfaces that the class implements. For interfaces, this gives the interfaces
	 * that the interface extends.
	 * 
	 * @param type the given type
	 * @return the direct resolved interfaces that the given type implements or extends limited to
	 *         the interfaces in this type hierarchy's graph
	 */
	IType[] getSuperInterfaces(IType type);

	/**
	 * Returns the resolved supertypes of the given type, in no particular order, limited to the
	 * types in this type hierarchy's graph. For classes, this returns its superclass and the
	 * interfaces that the class implements. For interfaces, this returns the interfaces that the
	 * interface extends. As a consequence <code>java.lang.Object</code> is NOT considered to be a
	 * supertype of any interface type.
	 * 
	 * @param type the given type
	 * @return the resolved supertypes of the given type limited to the types in this type
	 *         hierarchy's graph
	 */
	...
	/**
	 * Removes the given listener from this type hierarchy. Has no affect if an identical listener
	 * is not registered.
	 * 
	 * @param listener the listener
	 */
	 ...
	 * IType#loadTypeHierachy(IJavaProject, InputStream, IProgressMonitor). Listeners of this
	 * hierarchy are not stored.
	 * 
