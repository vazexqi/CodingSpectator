 ...
 * </p>
 * <p>
 * Note that type parameters are not children of their declaring type or method. To get a list of
 * the type parameters use {@link IType#getTypeParameters()} for a type and use
...
public interface ITypeParameter extends IJavaElement, ISourceReference {
	...
	/**
	 * Returns the signatures for this type parameter's bounds. The type parameter may have been
	 * declared as part of a type or a method. The signatures represent only the individual bounds
	 * and do not include the type variable name or the <code>extends</code> keyword. The signatures
	 * may be either unresolved (for source types) or resolved (for binary types). See
	...
	/**
	 * Returns the source range of this type parameter's name, or <code>null</code> if this type
	 * parameter does not have associated source code (for example, in a binary type).
	 * 
	 * @exception JavaModelException if this element does not exist or if an exception occurs while
	 *                accessing its corresponding resource.
	 * @return the source range of this type parameter's name, or <code>null</code> if this type
	 *         parameter does not have associated source code (for example, in a binary type)
	 */
