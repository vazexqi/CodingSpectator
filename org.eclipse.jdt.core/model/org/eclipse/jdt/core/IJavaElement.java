...
/**
 * Common protocol for all elements provided by the Java model. Java model elements are exposed to
 * clients as handles to the actual underlying element. The Java model may hand out any number of
 * handles for each element. Handles that refer to the same element are guaranteed to be equal, but
 * not necessarily identical.
 * <p>
 * Methods annotated as "handle-only" do not require underlying elements to exist. Methods that
 * require underlying elements to exist throw a <code>JavaModelException</code> when an underlying
 * element is missing. <code>JavaModelException.isDoesNotExist</code> can be used to recognize this
 * common special case.
 * </p>
 * 
...
public interface IJavaElement extends IAdaptable {
	...
	/**
	 * Constant representing a Java model (workspace level object). A Java element with this type
	 * can be safely cast to {@link IJavaModel}.
	 */
	int JAVA_MODEL= 1;

	/**
	 * Constant representing a Java project. A Java element with this type can be safely cast to
	 * {@link IJavaProject}.
	 */
	int JAVA_PROJECT= 2;

	/**
	 * Constant representing a package fragment root. A Java element with this type can be safely
	 * cast to {@link IPackageFragmentRoot}.
	 */
	int PACKAGE_FRAGMENT_ROOT= 3;

	/**
	 * Constant representing a package fragment. A Java element with this type can be safely cast to
	 * {@link IPackageFragment}.
	 */
	int PACKAGE_FRAGMENT= 4;

	/**
	 * Constant representing a Java compilation unit. A Java element with this type can be safely
	 * cast to {@link ICompilationUnit}.
	 */
	int COMPILATION_UNIT= 5;

	/**
	 * Constant representing a class file. A Java element with this type can be safely cast to
	 * {@link IClassFile}.
	 */
	int CLASS_FILE= 6;

	/**
	 * Constant representing a type (a class or interface). A Java element with this type can be
	 * safely cast to {@link IType}.
	 */
	int TYPE= 7;

	/**
	 * Constant representing a field. A Java element with this type can be safely cast to
	 * {@link IField}.
	 */
	int FIELD= 8;

	/**
	 * Constant representing a method or constructor. A Java element with this type can be safely
	 * cast to {@link IMethod}.
	 */
	int METHOD= 9;

	/**
	 * Constant representing a stand-alone instance or class initializer. A Java element with this
	 * type can be safely cast to {@link IInitializer}.
	 */
	int INITIALIZER= 10;

	/**
	 * Constant representing a package declaration within a compilation unit. A Java element with
	 * this type can be safely cast to {@link IPackageDeclaration}.
	 */
	int PACKAGE_DECLARATION= 11;

	/**
	 * Constant representing all import declarations within a compilation unit. A Java element with
	 * this type can be safely cast to {@link IImportContainer}.
	 */
	int IMPORT_CONTAINER= 12;

	/**
	 * Constant representing an import declaration within a compilation unit. A Java element with
	 * this type can be safely cast to {@link IImportDeclaration}.
	 */
	int IMPORT_DECLARATION= 13;

	/**
	 * Constant representing a local variable declaration. A Java element with this type can be
	 * safely cast to {@link ILocalVariable}.
	 * 
	 * @since 3.0
	 */
	...
	/**
	 * Constant representing a type parameter declaration. A Java element with this type can be
	 * safely cast to {@link ITypeParameter}.
	 * 
	 * @since 3.1
	 */
	...
	/**
	 * Constant representing an annotation. A Java element with this type can be safely cast to
	 * {@link IAnnotation}.
	 * 
	 * @since 3.4
	 */
	...
	/**
	 * <p>
	 * Returns the Javadoc as an html source if this element has an attached javadoc, null
	 * otherwise.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * The encoding used to read the javadoc is the one defined by the content type of the file. If
	 * none is defined, then the project's encoding of this java element is used. If the project's
	 * encoding cannot be retrieved, then the platform encoding is used.
	 * </p>
	 * <p>
	 ...
	 * 
	 * <p>
	 * The html is extracted from the attached javadoc and provided as is. No transformation or
	 * validation is done.
	 * </p>
	 * 
	...
	/**
	 * Returns the resource that corresponds directly to this element, or <code>null</code> if there
	 * is no resource that corresponds to this element.
	 * <p>
	 * For example, the corresponding resource for an <code>ICompilationUnit</code> is its
	 * underlying <code>IFile</code>. The corresponding resource for an
	 * <code>IPackageFragment</code> that is not contained in an archive is its underlying
	 * <code>IFolder</code>. An <code>IPackageFragment</code> contained in an archive has no
	 * corresponding resource. Similarly, there are no corresponding resources for
	 * <code>IMethods</code>, <code>IFields</code>, etc.
	 * <p>
	 * 
	 ...
	 * 
	 * @return the kind of element; one of the constants declared in <code>IJavaElement</code>
	 * @see IJavaElement
	 */
	int getElementType();
	...
	/**
	 * Returns the Java project this element is contained in, or <code>null</code> if this element
	 * is not contained in any Java project (for instance, the <code>IJavaModel</code> is not
	 * contained in any Java project). This is a handle-only method.
	 * 
	 * @return the containing Java project, or <code>null</code> if this element is not contained in
	 *         a Java project
	 */
	IJavaProject getJavaProject();

	/**
	 * Returns the first openable parent. If this element is openable, the element itself is
	 * returned. Returns <code>null</code> if this element doesn't have an openable parent. This is
	 * a handle-only method.
	 * 
	 * @return the first openable parent or <code>null</code> if this element doesn't have an
	 *         openable parent.
	 * @since 2.0
	 */
	...
	/**
	 * Returns the smallest underlying resource that contains this element, or <code>null</code> if
