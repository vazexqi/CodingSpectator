...
/**
 * A checked exception representing a failure in the Java model. Java model exceptions contain a
 * Java-specific status object describing the cause of the exception.
 * <p>
 * Instances of this class are automatically created by the Java model when problems arise, so there
...
public class JavaModelException extends CoreException {
	...
	/**
	 * Creates a Java model exception that wrappers the given <code>Throwable</code>. The exception
	 * contains a Java-specific status object with severity <code>IStatus.ERROR</code> and the given
	 * status code.
	 * 
	...
	/**
	 * Creates a Java model exception for the given <code>CoreException</code>. Equivalent to
	 ...
	 * 
	 * @return the wrappered <code>Throwable</code>, or <code>null</code> if the direct case of the
	 *         failure was at the Java model layer
	 */
	...
	/**
	 * Returns whether this exception indicates that a Java model element does not exist. Such
	 * exceptions have a status with a code of
	 * <code>IJavaModelStatusConstants.ELEMENT_DOES_NOT_EXIST</code> or
	 * <code>IJavaModelStatusConstants.ELEMENT_NOT_ON_CLASSPATH</code>. This is a convenience
	 * method.
	 * 
	 * @return <code>true</code> if this exception indicates that a Java model element does not
	 *         exist
