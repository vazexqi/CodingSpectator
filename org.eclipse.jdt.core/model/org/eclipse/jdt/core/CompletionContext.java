...
public class CompletionContext {
	...
	/**
	 * The completion token is a string literal. The string literal ends quote can be not present
	 * the source. <code>"foo"</code> or <code>"foo</code>.
	 * 
	 * @since 3.2
	 */
	 ...
	 * 
	 * @return signatures expected types of a potential completion proposal at the completion
	 *         position or <code>null</code> if there is no expected types.
	 * 
	 * @see Signature
	 */
	 ...
	 * request offset is not within or immediately after an identifier or keyword or a string
	 * literal then the returned value is <code>null</code>.
	 * 
	 ...
	 * <p>
	 * The set of different kinds of completion token is expected to change over time. It is
	 ...
	 * recommended that clients do <b>not</b> assume that the location contains only known value,
	 ...
	 * elements in the same compilation unit which can be navigated to from the returned Java
	 * element are special Java elements:
	 * <ul>
	 ...
	 * <li>they do not give information about categories. {@link IMember#getCategories()} will
	 ...
	 * <li>the completion occurred in a binary type. However this restriction might be relaxed in
	 * the future.</li>
	 * </ul>
	 * 
	...
	/**
	 * Return the elements which are visible from the completion location and which can be assigned
	 * to the given type. An element is assignable if its type can be assigned to a variable of the
	 * given type, as specified in section 5.2 of <em>The Java Language
