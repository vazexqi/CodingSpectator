...
/**
 * This operation copies/moves a collection of elements from their current container to a new
 * container, optionally renaming the elements.
 * <p>
 * Notes:
 * <ul>
...
public class CopyElementsOperation extends MultiOperation implements SuffixConstants {
	 ...
	 * elements and destination containers must be in the correct order. If there is > 1
	 * destination, the number of destinations must be the same as the number of elements being
			...
			switch (element.getElementType()) {
				...
				case IJavaElement.TYPE:
					if (isRenamingMainType(element, dest)) {
						...
						return new RenameResourceElementsOperation(new IJavaElement[] { dest }, new IJavaElement[] { dest.getParent() }, new String[] { getNewNameFor(element) + '.' + extension },
								this.force);
					} else {
	 ...
	 * 
	 * @exception JavaModelException if the operation is unable to be completed
	 */
	...
	/**
	 * Returns the anchor used for positioning in the destination for the element being renamed. For
	 ...
	 * <li>INDEX_OUT_OF_BOUNDS - the number of renamings supplied to the operation does not match
	 ...
	 *      element is provided in the status, otherwise, the non-existant element is supplied in
	 *      the status.
	 ...
	 *      <li>INVALID_DESTINATION - The destination parent specified for <code>element</code> is
	 *      of an incompatible type. The destination for a package declaration or import declaration
	 *      must be a compilation unit; the destination for a type must be a type or compilation
	 *      unit; the destination for any type member (other than a type) must be a type. When this
	 *      error occurs, the element provided in the operation status is the <code>element</code>.
