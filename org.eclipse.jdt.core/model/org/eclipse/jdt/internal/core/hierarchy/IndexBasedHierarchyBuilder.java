...
public class IndexBasedHierarchyBuilder extends HierarchyBuilder implements SuffixConstants {
	...
	/**
	 * A temporary cache of compilation units to handles to speed info to handle translation - it
	 * only contains the entries for the types in the region (in other words, it contains no
	 * supertypes outside the region).
	 */
	...
	private void buildForProject(JavaProject project, ArrayList potentialSubtypes, org.eclipse.jdt.core.ICompilationUnit[] workingCopies, HashSet localTypes, IProgressMonitor monitor)
	...
	/**
	 * Returns all of the possible subtypes of this type hierarchy. Returns null if they could not
	 * be determine.
	 */
	 ...
	 * 
	 * The requestor is notified of super type references (with actual path of its occurrence) for
	 * all types which are potentially involved inside a particular hierarchy. The match locator is
	 * not used here to narrow down the results, the type hierarchy resolver is rather used to
	 * compute the whole hierarchy at once.
	 * 
	 * @param type
	 * @param scope
