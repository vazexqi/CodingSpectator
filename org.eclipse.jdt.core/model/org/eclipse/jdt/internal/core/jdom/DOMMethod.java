...
class DOMMethod extends DOMMember implements IDOMMethod {
	...
	/**
	 * Contains the return type of the method when the return type has been altered from the
	 * contents in the document, otherwise <code>null</code>.
	 */
	protected String fReturnType;

	/**
	 * The original inclusive source range of the method's return type in the document, or -1's if
	 * no return type is present in the document. If the return type of this method is qualified
	 * with '[]' following the parameter list, this array has four entries. In this case, the last
	 * two entries of the array are the inclusive source range of the array qualifiers.
	 */
	protected int[] fReturnTypeRange;

	/**
	 * Contains the textual representation of the method's parameter list, including open and
	 * closing parentheses when the parameters had been altered from the contents in the document,
	...
	/**
	 * Contains the textual representation of the method's exception list when the exceptions had
	 * been altered from the contents in the document, otherwise <code>null</code>. The exception
	 * list is a comment delimited list of exceptions, not including the "throws" keyword.
	 */
	...
	/**
	 * Contains the method's body when the body has been altered from the contents in the document,
	 * otherwise <code>null</code>. The body includes everything between and including the enclosing
	 * braces, and trailing whitespace.
	 */
	...
	/**
	 * Names of parameters in the method parameter list, or <code>null</code> if the method has no
	 * parameters.
	 */
	protected String[] fParameterNames;

	/**
	 * Types of parameters in the method parameter list, or <code>null</code> if the method has no
	 * parameters.
	 */
	protected String[] fParameterTypes;

	/**
	 * The exceptions the method throws, or <code>null</code> if the method throws no exceptions.
	 */
	...
	/**
	 * Default value for this attotation type member (only), or <code>null</code> if none.
	 * 
	 * @since 3.0
	 */
	 ...
	 * @param commentRange - a two element array describing the comments that precede the member
	 *            declaration. The first matches the start of this node's sourceRange, and the
	 *            second is the new-line or first non-whitespace character following the last
	 *            comment. If no comments are present, this array contains two -1's.
	 * @param flags - an integer representing the modifiers for this member. The integer can be
	 *            analyzed with org.eclipse.jdt.core.Flags
	 * @param modifierRange - a two element array describing the location of modifiers for this
	 *            member within its source range. The first integer is the first character of the
	 *            first modifier for this member, and the second integer is the last whitespace
	 *            character preceeding the next part of this member declaration. If there are no
	 *            modifiers present in this node's source code (that is, package default
	 ...
	 * @param returnTypeRange - a two element array describing the location of the return type
	 *            within the method's source range. The first integer is is the position of the
	 *            first character in the return type, and the second integer is the position of the
	 *            last character in the return type. For constructors, the contents of this array
	 *            are -1's. If the return type of this method is qualified with '[]' following the
	 *            parameter list, this array has four entries. In this case, the last two entries of
	 *            the array are the inclusive source range of the array qualifiers.
	 * @param parameterTypes - an array of parameter types in the method declaration or
	 *            <code>null</code> if the method has no parameters
	 * @param parameterNames - an array of parameter names in the method declaration or
	 *            <code>null</code> if the method has no parameters
	 * @param parameterRange - a two element array describing the location of the parameter list in
	 *            the method. The first integer is the location of the open parenthesis and the
	 *            second integer is the location of the closing parenthesis.
	 * @param exceptions - an array of the names of exceptions thrown by this method or
	 *            <code>null</code> if the method throws no exceptions
	 * @param exceptionRange - a two element array describing the location of the exception list in
	 *            the method declaration. The first integer is the position of the first character
	 *            in the first exception the method throws, and the second integer is the position
	 *            of the last character of the last exception this method throws.
	 * @param bodyRange - a two element array describing the location of the method's body. The
	 *            first integer is the first character following the method's parameter list, or
	 *            exception list (if present). The second integer is the location of the last
	 *            character in the method's source range.
	 */
	DOMMethod(char[] document, int[] sourceRange, String name, int[] nameRange, int[] commentRange, int flags, int[] modifierRange, boolean isConstructor, String returnType, int[] returnTypeRange,
			String[] parameterTypes, String[] parameterNames, int[] parameterRange, String[] exceptions, int[] exceptionRange, int[] bodyRange) {
	 ...
	 *            <code>null</code> if the method throws no exceptions
	 */
	DOMMethod(char[] document, int[] sourceRange, String name, int[] nameRange, int flags, boolean isConstructor, String returnType, String[] parameterTypes, String[] parameterNames,
			String[] exceptions) {
		this(document, sourceRange, name, nameRange, new int[] { -1, -1 }, flags, new int[] { -1, -1 }, isConstructor, returnType, new int[] { -1, -1 }, parameterTypes, parameterNames, new int[] {
				-1, -1 }, exceptions, new int[] { -1, -1 }, new int[] { -1, -1 });
	...
	/**
	 * Returns the simple name of the enclsoing type for this constructor. If the constuctor is not
	 * currently enclosed in a type, the original name of the constructor as found in the documnent
	 * is returned.
	 */
	...
	/**
	 * Returns true if this method's return type has array qualifiers ('[]') following the parameter
	 * list.
	 */
	...
	/**
	 * Returns true if this method's return type has been altered from the original document
	 * contents.
	 */
	 ...
	 * 
	 *      <p>
	 *      Two methods have equal signatures if there names are the same and their parameter types
	 *      are the same.
	 */
	...
	/**
	 * Sets the state of this method declaration as having the return type altered from the original
	 * document.
	 */
