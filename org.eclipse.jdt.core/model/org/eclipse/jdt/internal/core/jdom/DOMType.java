...
/* package */class DOMType extends DOMMember implements IDOMType {
	/**
	 * The 'class' or 'interface' keyword if altered from the documents contents, otherwise
	 * <code>null</code>.
	 */
	protected String fTypeKeyword;

	/**
	 * The original inclusive source range of the 'class' or 'interface' keyword in the document.
	 */
	protected int[] fTypeRange;

	/**
	 * The superclass name for the class declaration if altered from the document's contents,
	 * otherwise <code>null</code>. Also <code>null</code> when this type represents an interface.
	 */
	protected String fSuperclass;

	/**
	 * The original inclusive source range of the superclass name in the document, or -1's of no
	 * superclass was specified in the document.
	 */
	protected int[] fSuperclassRange;


	/**
	 * The original inclusive souce range of the 'extends' keyword in the document, including
	 * surrounding whitespace, or -1's if the keyword was not present in the document.
	 */
	protected int[] fExtendsRange;

	/**
	 * The original inclusive souce range of the 'implements' keyword in the document, including
	 * surrounding whitespace, or -1's if the keyword was not present in the document.
	 */
	protected int[] fImplementsRange;

	/**
	 * The comma delimited list of interfaces this type implements or extends, if altered from the
	 * document's contents, otherwise <code>null</code>. Also <code>null</code> if this type does
	 * not implement or extend any interfaces.
	 */
	protected char[] fInterfaces;

	/**
	 * The original inclusive source range of the list of interfaces this type implements or
	 * extends, not including any surrouding whitespace. If the document did not specify interfaces,
	 * this array contains -1's.
	 */
	...
	/**
	 * The original source range of the first character following the type name superclass name, or
	 * interface list, up to and including the first character before the first type member.
	 */
	protected int[] fOpenBodyRange;

	/**
	 * The original source range of the first new line or non whitespace character preceding the
	 * close brace of the type's body, up to the and including the first character before the next
	 * node (if there are no following nodes, the range ends at the position of the last character
	 * in the document).
	 */
	protected int[] fCloseBodyRange;

	/**
	 * A list of interfaces this type extends or implements. <code>null</code> when this type does
	 * not extend or implement any interfaces.
	 */
...
//	protected int fInsertionPosition;
	 ...
	 * @param typeRange - a two element array describing the location of the 'class' or 'interface'
	 *            keyword in the type declaration - first and last character positions.
	 * @param superclassRange - a two element array describing the location of the superclass name
	 *            in the type declaration - first and last character positions or two -1's if a
	 *            superclass is not present in the document.
	 * @param extendsRange - a two element array describing the location of the 'extends' keyword in
	 *            the type declaration, including any surrounding whitespace, or -1's if the
	 *            'extends' keyword is not present in the document.
	 * @param implementsList - an array of names of the interfaces this type implements or extends,
	 *            or <code>null</code> if this type does not implement or extend any interfaces.
	 * @param implementsRange - a two element array describing the location of the comment delimited
	 *            list of interfaces this type implements or extends, not including any surrounding
	 *            whitespace, or -1's if no interface list is present in the document.
	 ...
	 * @param openBodyRange - a two element array describing the location of the open brace of the
	 *            type's body and whitespace following the type declaration and preceeding the first
	 *            member in the type.
	 * @param closeBodyRange - a two element array describing the source range of the first new line
	 *            or non whitespace character preceeding the close brace of the type's body, up to
	 *            the close brace
	 * @param isClass - true is the type is a class, false if it is an interface
	 */
	DOMType(char[] document, int[] sourceRange, String name, int[] nameRange, int[] commentRange, int flags, int[] modifierRange, int[] typeRange, int[] superclassRange, int[] extendsRange,
			String[] implementsList, int[] implementsRange, int[] implementsKeywordRange, int[] openBodyRange, int[] closeBodyRange, boolean isClass) {
	...
	/**
	 * Returns the position of the closing brace for the body of this type. This value this method
	 * returns is only valid before the type has been normalized and is present only for
	 * normalization.
	 */
	 ...
	 * 
	 * <p>
	 * When the name of a type is set, all of its constructors must be marked as fragmented, since
	 * the names of the constructors must reflect the name of this type.
	 * 
