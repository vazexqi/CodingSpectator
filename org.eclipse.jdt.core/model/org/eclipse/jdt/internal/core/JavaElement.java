...
//	private static final QualifiedName PROJECT_JAVADOC= new QualifiedName(JavaCore.PLUGIN_ID, "project_javadoc_location"); //$NON-NLS-1$
	 ...
	 * 
	 * @exception IllegalArgumentException if the type is not one of the valid Java element type
	 *                constants
	 * 
	 */
	...
	/**
	 * Returns true if this handle represents the same Java element as the given handle. By default,
	 * two handles represent the same element if they are identical or if they represent the same
	 * type of element, have equal names, parents, and occurrence counts.
	 * 
	 * <p>
	...
	/**
	 * Returns the <code>ASTNode</code> that corresponds to this <code>JavaElement</code> or
	 * <code>null</code> if there is no corresponding node.
	 */
	...
	/**
	 * Generates the element infos for this element, its ancestors (if they are not opened) and its
	 * children (if it is an Openable). Puts the newly created element info in the given map.
	 */
	...
	/**
	 * Returns the info for this handle. If this element is not already open, it and all of its
	 * parents are opened. Does not return null. NOTE: BinaryType infos are NOT rooted under
	 * JavaElementInfo.
	 * 
	...
	/**
	 * Returns the element that is located at the given source position in this element. This is a
	 * helper method for <code>ICompilationUnit#getElementAt</code>, and only works on compilation
	 * units and types. The position given is known to be within this element's source range
	 * already, and if no finer grained element is found at the position, this element is returned.
	 */
	...
	/**
	 * Returns the SourceMapper facility for this element, or <code>null</code> if this element does
	 * not have a SourceMapper.
	 */
	...
	/**
	 * Returns the hash code for this Java element. By default, the hash code for an element is a
	 * combination of its name and parent's hash code. Elements with other requirements must
