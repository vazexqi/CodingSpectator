...
/**
 * This operation creates an <code>ITypeHierarchy</code> for a specific type within a specified
 * region, or for all types within a region. The specified region limits the number of resolved
 * subtypes (to the subset of types in the specified region). The resolved supertypes may go outside
 * of the specified region in order to reach the root(s) of the type hierarchy. A Java Project is
 * required to provide a context (classpath) to use while resolving supertypes and subtypes.
 * 
 * @see ITypeHierarchy
 */
...
public class CreateTypeHierarchyOperation extends JavaModelOperation {
	...
	/**
	 * Constructs an operation to create a type hierarchy for the given type within the specified
	 * region, in the context of the given project.
	 */
	 ...
	 * Possible failures:
	 * <ul>
	 * <li>NO_ELEMENTS_TO_PROCESS - at least one of a type or region must be provided to generate a
	 * type hierarchy.
