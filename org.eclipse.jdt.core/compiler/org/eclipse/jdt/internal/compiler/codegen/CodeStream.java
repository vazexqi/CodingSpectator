...
public class CodeStream {
	...
	/**
	 * This methods searches for an existing entry inside the pcToSourceMap table with a pc equals
	 * to @pc. If there is an existing entry it returns -1 (no insertion required). Otherwise it
	 * returns the index where the entry for the pc has to be inserted. This is based on the fact
	 * that the pcToSourceMap table is sorted according to the pc.
	 * 
	...
	/**
	 * Generates the sequence of instructions which will perform the conversion of the expression on
	 * the stack into a different type (e.g. long l = someInt; --> i2l must be inserted).
	 * 
...
//			break;
	 ...
	 * variable arguments of a constructor invocation of a nested type. (bug 26122) - synthetic
	 * values for outer locals must be passed after user arguments, e.g. new X(i = 1){}
	 */
 ...
 */
	...
	/**
	 * Returns the type that should be substituted to original binding declaring class as the proper
	 * receiver type
	 * 
	 * @param currentScope
...
// We didn't call it new, because there is a conflit with the new keyword
	...
	/**
	 * Note: it will walk the locals table and extend the end range for all matching ones, no matter
	 * if visible or not. { int i = 0; { int j = 1; } } <== would process both 'i' and 'j'
	 * Processing non-visible ones is mandated in some cases (include goto instruction after if-then
	 * block)
	 */
	public void updateLastRecordedEndPC(Scope scope, int pos) {
		...
		/**
		 * OLD CODE if (!generateLineNumberAttributes) return; pcToSourceMap[pcToSourceMapSize -
		 * 1][1] = position; // need to update the initialization endPC in case of generation of
		 * local variable attributes. updateLocalVariablesAttribute(pos);
		 */
