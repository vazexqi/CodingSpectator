		...
		switch (repair.code) {
			...
			case BEFORE_CODE: {
				if (repair.symbol > NT_OFFSET)
					...
					name_index= getTermIndex(this.stack,
												this.stateStackTop,
												repair.symbol,
												repair.bufferPosition);
	...
	private RepairCandidate secondaryPhase(int error_token) {
		...
		if (this.nextStackTop >= 0) {
			...
			for (next_last_index= MAX_DISTANCE - 1; next_last_index >= 1 &&
					this.lexStream.kind(this.buffer[next_last_index]) == EOFT_SYMBOL; next_last_index--) {/*empty*/
			}
		...
		for (last_index= MAX_DISTANCE - 1; last_index >= 1 && this.lexStream.kind(this.buffer[last_index]) == EOFT_SYMBOL; last_index--) {/*empty*/
		}
		last_index++;
...
//
	private SecondaryRepairInfo secondaryRecovery(int stck[], int stack_top, int last_index, SecondaryRepairInfo repair, boolean stack_flag) {
		...
		for (int top= stack_top; top >= 0 && repair.numDeletions >= stack_deletions; top--) {
			...
			for (int i= 2; i <= (last_index - MIN_DISTANCE + 1) &&
