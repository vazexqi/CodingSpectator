...
abstract public class ReferenceBinding extends TypeBinding {
	...
	/**
	 * Returns a combined range value representing: (start + (end<<32)), where start is the index of
	 * the first matching method (remember methods are sorted alphabetically on selectors), and end
	 * is the index of last contiguous methods with same selector. -1 means no method got found
	 * 
	 * @param selector
	 * @param sortedMethods
...
// The suggested parameter length is optional and may not be guaranteed by every type.
	...
	/**
	 * Returns true if the two types have an incompatible common supertype, e.g. List<String> and
	 * List<Integer>
	 */
	...
	/**
	 * Answer true if the receiver implements anInterface or is identical to anInterface. If
	 * searchHierarchy is true, then also search the receiver's superclasses.
	 * 
...
// Internal method... assume its only sent to classes NOT interfaces
	...
	/**
	 * Answer true if the receiver type can be assigned to the argument type (right) In addition to
	 * improving performance, caching also ensures there is no infinite regression since per nature,
	 * the compatibility check is recursive through parameterized type arguments (122775)
	 */
	...
	/**
	 * Answer true if the receiver is in the superclass hierarchy of aType NOTE:
	 * Object.isSuperclassOf(Object) -> false
	 */
	 ...
	 * connected. (Throwable<- Exception <- RumtimeException, Throwable <- Error). Thus no need to
	 * check #isCompatibleWith but rather check in type IDs so as to avoid some eager class loading
	 * for JCL writers. When 'includeSupertype' is true, answers true if the given type can be a
	 * supertype of some unchecked exception type (i.e. Throwable or Exception).
	 * 
