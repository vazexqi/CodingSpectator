 ...
 */
				...
				if (lastFieldBinding != null) {
					...
					if (fieldConstant != Constant.NotAConstant) {
						...
						if (needValue || (i > 0 && complyTo14) || lastGenericCast != null) {
							...
							if (accessor == null) {
								TypeBinding constantPoolDeclaringClass= CodeStream.getConstantPoolDeclaringClass(currentScope, lastFieldBinding, lastReceiverType, i == 0
										&& this.indexOfFirstFieldBinding == 1);
								if (lastFieldBinding.isStatic()) {
									codeStream.fieldAccess(Opcodes.OPC_getstatic, lastFieldBinding, constantPoolDeclaringClass);
								} else {
	...
	/**
	 * Returns the receiver type for the final field in sequence (i.e. the return type of the
	 * previous binding)
	 * 
				...
				case Binding.TYPE | Binding.VARIABLE:
					if (this.binding instanceof LocalVariableBinding) {
						...
						if (this.resolvedType != null && (this.resolvedType.tagBits & TagBits.HasMissingType) != 0) {
							...
							scope.problemReporter().invalidField(this, new ProblemFieldBinding(lastField.declaringClass, lastField.name, ProblemReasons.NotFound), this.tokens.length,
