 ...
 * to the <code>search(...)</code> methods. When a search starts, the <code>aboutToStart()</code>
 ...
 * <code>EXACT_MATCH</code> accuracy - or they might be potential matches only - they have a
 * <code>POTENTIAL_MATCH</code> accuracy. This last case can occur when a problem prevented the
 * <code>SearchEngine</code> from resolving the match.
 * </p>
 * <p>
 ...
 * 
 * @see SearchEngine
 * @deprecated Since 3.0, the class {@link org.eclipse.jdt.core.search.SearchRequestor} replaces
 *             this interface.
 */
public interface IJavaSearchResultCollector {
	...
	/**
	 * The search result is potentially a match for the search pattern, but a problem prevented the
	 * search engine from being more accurate (typically because of the classpath was not correctly
	 * set).
	 * 
	 ...
	 * @param end the end position of the match, -1 if it is unknown; the ending offset is
	 *            exclusive, meaning that the actual range of characters covered is
	 *            <code>[start, end]</code>
	 * @param enclosingElement the Java element that contains the character range
	 *            <code>[start, end]</code>; the value can be <code>null</code> indicating that no
	 *            enclosing Java element has been found
	 * @param accuracy the level of accuracy the search result has; either <code>EXACT_MATCH</code>
	 *            or <code>POTENTIAL_MATCH</code>
