...
/**
 * A {@link SearchEngine} searches for Java elements following a search pattern. The search can be
 * limited to a search scope.
 * <p>
 * Various search patterns can be created using the factory methods
 * {@link SearchPattern#createPattern(String, int, int, int)},
 * {@link SearchPattern#createPattern(IJavaElement, int)},
 * {@link SearchPattern#createOrPattern(SearchPattern, SearchPattern)}.
 * </p>
 * <p>
 * For example, one can search for references to a method in the hierarchy of a type, or one can
 * search for the declarations of types starting with "Abstract" in a project.
 * </p>
 * <p>
...
public class SearchEngine {
	...
	/**
	 * Creates a new search engine with the given working copy owner. The working copies owned by
	 * this owner will take precedence over the primary compilation units in the subsequent search
	 * operations.
	 * 
	 * @param workingCopyOwner the owner of the working copies that take precedence over their
	...
	/**
	 * Returns a Java search scope limited to the hierarchy of the given type. The Java elements
	 * resulting from a search with this scope will be types in this hierarchy, or members of the
	...
	/**
	 * Returns a Java search scope limited to the hierarchy of the given type. When the hierarchy is
	 * computed, the types defined in the working copies owned by the given owner take precedence
	 * over the original compilation units. The Java elements resulting from a search with this
	 * scope will be types in this hierarchy, or members of the types in this hierarchy.
	 * 
	 * @param type the focus of the hierarchy scope
	...
	/**
	 * Returns a Java search scope limited to the hierarchy of the given type and to a given
	 * project. The Java elements resulting from a search with this scope will be types in this
	 * hierarchy.
	 * <p>
	 * Unlike the <code>createHierarchyScope</code> methods, this method creates <em>strict</em>
	 * scopes that only contain types that actually span the hierarchy of the focus type, but do not
	 * include additional enclosing or member types.
	 * </p>
	 * <p>
	 * By default, hierarchy scopes include all direct and indirect supertypes and subtypes of the
	 * focus type. This method, however, allows to restrict the hierarchy to true subtypes, not
	 * including supertypes. Also inclusion of the focus type itself is controled by a parameter.
	 * </p>
	 * 
	 * @param project the project to which to constrain the search, or <code>null</code> if search
	 *            should consider all types in the workspace
	 * @param type the focus of the hierarchy scope
	 * @param onlySubtypes if true only subtypes of <code>type</code> are considered
	 * @param includeFocusType if true the focus type <code>type</code> is included in the resulting
	 *            scope, otherwise it is excluded
	 * @param owner the owner of working copies that take precedence over original compilation
	...
	/**
	 * Returns a Java search scope limited to the given resources. The Java elements resulting from
	 * a search with this scope will have their underlying resource included in or equals to one of
	 * the given resources.
	 * <p>
	...
	/**
	 * Returns a Java search scope limited to the given Java elements. The Java elements resulting
	 * from a search with this scope will be children of the given elements.
	 * <p>
	 * If an element is an IJavaProject, then the project's source folders, its jars (external and
	 * internal) and its referenced projects (with their source folders and jars, recursively) will
	 * be included. If an element is an IPackageFragmentRoot, then only the package fragments of
	 * this package fragment root will be included. If an element is an IPackageFragment, then only
	 * the compilation unit and class files of this package fragment will be included. Subpackages
	 * will NOT be included.
	 * </p>
	 * <p>
	 * In other words, this is equivalent to using SearchEngine.createJavaSearchScope(elements,
	 * true).
	 * </p>
	 * 
	 ...
	 * 
	 * If an element is an IJavaProject, then the project's source folders, its jars (external and
	 * internal) and - if specified - its referenced projects (with their source folders and jars,
	 * recursively) will be included. If an element is an IPackageFragmentRoot, then only the
	 * package fragments of this package fragment root will be included. If an element is an
	 * IPackageFragment, then only the compilation unit and class files of this package fragment
	 * will be included. Subpackages will NOT be included.
	 * 
	 * @param elements the Java elements the scope is limited to
	 * @param includeReferencedProjects a flag indicating if referenced projects must be recursively
	 *            included
	 * @return a new Java search scope
	 * @since 2.0
	 */
	 ...
	 * from a search with this scope will be children of the given elements.
	 * 
	 * If an element is an IJavaProject, then it includes: - its source folders if
	 * IJavaSearchScope.SOURCES is specified, - its application libraries (internal and external
	 * jars, class folders that are on the raw classpath, or the ones that are coming from a
	 * classpath path variable, or the ones that are coming from a classpath container with the
	 * K_APPLICATION kind) if IJavaSearchScope.APPLICATION_LIBRARIES is specified - its system
	 * libraries (internal and external jars, class folders that are coming from an
	 * IClasspathContainer with the K_SYSTEM kind) if IJavaSearchScope.APPLICATION_LIBRARIES is
	 * specified - its referenced projects (with their source folders and jars, recursively) if
	 * IJavaSearchScope.REFERENCED_PROJECTS is specified. If an element is an IPackageFragmentRoot,
	 * then only the package fragments of this package fragment root will be included. If an element
	 * is an IPackageFragment, then only the compilation unit and class files of this package
	 * fragment will be included. Subpackages will NOT be included.
	 * 
	 * @param elements the Java elements the scope is limited to
	...
	/**
	 * Returns a search pattern that combines the given two patterns into a "or" pattern. The search
	 * result will match either the left pattern or the right pattern.
	 * 
	 ...
	 *            with the corresponding element. In case the element is a method, declarations of
	 *            matching methods in subtypes will also be found, allowing to find declarations of
	 *            abstract methods, etc.</li>
	 * 
	 *            <li>{@link IJavaSearchConstants#REFERENCES}: will search references to the given
	 *            element.</li>
	 * 
	 *            <li>{@link IJavaSearchConstants#ALL_OCCURRENCES}: will search for either
	 *            declarations or references as specified above.</li>
	 * 
	 *            <li>{@link IJavaSearchConstants#IMPLEMENTORS}: for types, will find all types
	 *            which directly implement/extend a given interface. Note that types may be only
	 *            classes or only interfaces if {@link IJavaSearchConstants#CLASS } or
	 *            {@link IJavaSearchConstants#INTERFACE} is respectively used instead of
	 *            {@link IJavaSearchConstants#TYPE}.</li>
	 *            </ul>
	 * 
	...
	/**
	 * Searches for the Java element determined by the given signature. The signature can be
	 * incomplete. For example, a call like <code>search(ws, "run()", METHOD,REFERENCES, col)</code>
	 * searches for all references to the method <code>run</code>.
	 * 
	 * Note that by default the pattern will be case insensitive. For specifying case s sensitive
	 * search, use
	 * <code>search(workspace, createSearchPattern(patternString, searchFor, limitTo, true), scope, resultCollector);</code>
	 * 
	 * @param workspace the workspace
	 * @param patternString the pattern to be searched for
	 * @param searchFor a hint what kind of Java element the string pattern represents. Look into
	 *            {@link IJavaSearchConstants} for valid values
	 * @param limitTo one of the following values:
	 *            <ul>
	 *            <li>{@link IJavaSearchConstants#DECLARATIONS}: search for declarations only</li>
	 *            <li>{@link IJavaSearchConstants#REFERENCES}: search for all references</li>
	 *            <li>{@link IJavaSearchConstants#ALL_OCCURRENCES}: search for both declarations and
	 *            all references</li>
	 *            <li>{@link IJavaSearchConstants#IMPLEMENTORS}: for types, will find all types
	 *            which directly implement/extend a given interface.<br>
	 *            Note that types may be only classes or only interfaces if respectively
	 *            {@link IJavaSearchConstants#CLASS} or {@link IJavaSearchConstants#INTERFACE} is
	 *            used for searchFor parameter instead of {@link IJavaSearchConstants#TYPE}.</li>
	 *            </ul>
	 ...
	 *                </ul>
	 * @deprecated Use
	 *             {@link #search(SearchPattern, SearchParticipant[], IJavaSearchScope, SearchRequestor, IProgressMonitor)}
	 *             instead.
	 */
	 ...
	 * selecting specific types (given a package exact full name or a type name with specific match
	 * mode).
	 * 
	 ...
	 * @param typeName the dot-separated qualified name of the searched type (the qualification
	 *            include the enclosing types if the searched type is a member type), or a prefix
	 *            for this type, or a wild-carded string for this type. May be <code>null</code>,
	 *            then any type name is accepted.
	 * @param matchRule type name match rule one of
	 *            <ul>
	 *            <li>{@link SearchPattern#R_EXACT_MATCH} if the package name and type name are the
	 *            full names of the searched types.</li>
	 *            <li>{@link SearchPattern#R_PREFIX_MATCH} if the package name and type name are
	 *            prefixes of the names of the searched types.</li>
	 *            <li>{@link SearchPattern#R_PATTERN_MATCH} if the package name and type name
	 *            contain wild-cards.</li>
	 *            <li>{@link SearchPattern#R_CAMELCASE_MATCH} if the type name is a camel case of
	 *            the searched types name.</li>
	 *            <li>{@link SearchPattern#R_CAMELCASE_SAME_PART_COUNT_MATCH} if the type name is a
	 *            camel case with same part count of the searched types name.</li>
	 *            </ul>
	 *            combined with {@link SearchPattern#R_CASE_SENSITIVE}, e.g.
	 *            {@link SearchPattern#R_EXACT_MATCH} | {@link SearchPattern#R_CASE_SENSITIVE} if an
	 *            exact and case sensitive match is requested, or
	 *            {@link SearchPattern#R_PREFIX_MATCH} if a prefix non case sensitive match is
	 *            requested.
	 * @param searchFor determines the nature of the searched elements
	 *            <ul>
	 *            <li>{@link IJavaSearchConstants#CLASS}: only look for classes</li>
	 ...
	 *            interfaces</li>
	 *            <li>{@link IJavaSearchConstants#TYPE}: look for all types (ie. classes,
	 ...
	 *            <ul>
	 *            <li>{@link IJavaSearchConstants#FORCE_IMMEDIATE_SEARCH} if the search should start
	 *            immediately</li>
	 *            <li>{@link IJavaSearchConstants#CANCEL_IF_NOT_READY_TO_SEARCH} if the search
	 *            should be cancelled if the underlying indexer has not finished indexing the
	 *            workspace</li>
	 *            <li>{@link IJavaSearchConstants#WAIT_UNTIL_READY_TO_SEARCH} if the search should
	 *            wait for the underlying indexer to finish indexing the workspace</li>
	 *            </ul>
	...
	/**
	 * Searches for all top-level types and member types in the given scope. The search can be
	 * selecting specific types (given a package name using specific match mode and/or a type name
	 * using another specific match mode).
	 * 
	 * @param packageName the full name of the package of the searched types, or a prefix for this
	 *            package, or a wild-carded string for this package. May be <code>null</code>, then
	 ...
	 *            <li>{@link SearchPattern#R_PATTERN_MATCH} if the package name and type name
	 *            contain wild-cards.</li>
	 *            <li>{@link SearchPattern#R_CAMELCASE_MATCH} if the package name is a camel case of
	 *            the searched types package name.</li>
	 *            <li>{@link SearchPattern#R_CAMELCASE_SAME_PART_COUNT_MATCH} if the package name is
	 *            a camel case with same part count of the searched types package name.</li>
	 *            </ul>
	 *            combined with {@link SearchPattern#R_CASE_SENSITIVE}, e.g.
	 *            {@link SearchPattern#R_EXACT_MATCH} | {@link SearchPattern#R_CASE_SENSITIVE} if an
	 *            exact and case sensitive match is requested, or
	 ...
	 * using another specific match mode).
	 * <p>
	 * Provided {@link TypeNameMatchRequestor} requestor will collect {@link TypeNameMatch} matches
	...
	/**
	 * Searches for all top-level types and member types in the given scope matching any of the
	 * given qualifications and type names in a case sensitive way.
	 * 
	 ...
	 * @param typeNames the simple names of the searched types. If this parameter is
	 ...
	 *            <li>{@link IJavaSearchConstants#TYPE} if searching for both classes and interfaces
	 *            </li>
	 *            </ul>
	 * @param scope the scope to search in
	 * @param nameRequestor the requestor that collects the results of the search
	 ...
	 * @since 3.0
	 * @deprecated Use
	 *             {@link #searchAllTypeNames(char[], char[], int, int, IJavaSearchScope, TypeNameRequestor, int, IProgressMonitor)}
	 *             instead
	 */
	public void searchAllTypeNames(
			final char[] packageName,
	 ...
	 *            <ul>
	 *            <li>{@link IJavaSearchConstants#EXACT_MATCH} if the package name and type name are
	 *            the full names of the searched types.</li>
	 *            <li>{@link IJavaSearchConstants#PREFIX_MATCH} if the package name and type name
	 *            are prefixes of the names of the searched types.</li>
	 *            <li>{@link IJavaSearchConstants#PATTERN_MATCH} if the package name and type name
	 *            contain wild-cards.</li>
	 *            </ul>
	 ...
	 *                </ul>
	 * @deprecated Use
	 *             {@link #searchAllTypeNames(char[], char[], int, int, IJavaSearchScope, ITypeNameRequestor, int, IProgressMonitor)}
	 *             instead
	 */
	public void searchAllTypeNames(
	 ...
	 * 		}
	 * </pre>
	 * </code> then searching for declarations of accessed fields in method <code>X.test()</code>
	 * would collect the fields <code>B.value</code> and <code>A.field1</code>.
	 * </p>
	 * 
	...
	/**
	 * Searches for all declarations of the fields accessed in the given element. The element can be
	 * a compilation unit, a source type, or a source method. Reports the field declarations using
	 * the given collector.
	 * <p>
	 * Consider the following code: <code>
	 * <pre>
	 ...
	 *                </ul>
	 * @deprecated Use
	 *             {@link #searchDeclarationsOfAccessedFields(IJavaElement, SearchRequestor, IProgressMonitor)}
	 *             instead.
	 */
	 ...
	 * 		}
	 * </pre>
	 * </code> then searching for declarations of referenced types in method <code>X.test()</code>
	...
	/**
	 * Searches for all declarations of the types referenced in the given element. The element can
	 * be a compilation unit, a source type, or a source method. Reports the type declarations using
	 * the given collector.
	 * <p>
	 * Consider the following code: <code>
	 * <pre>
	 ...
	 *                </ul>
	 * @deprecated Use
	 *             {@link #searchDeclarationsOfReferencedTypes(IJavaElement, SearchRequestor, IProgressMonitor)}
	 *             instead.
	 */
	 ...
	 * 		}
	 * </pre>
	 * </code> then searching for declarations of sent messages in method <code>X.test()</code>
	 * would collect the methods <code>A.foo()</code>, <code>B.foo()</code>, and
	 * <code>A.bar()</code>.
	 * </p>
	 * 
	 * @param enclosingElement the field, method, type or compilation unit to be searched in
	...
	/**
	 * Searches for all declarations of the methods invoked in the given element. The element can be
	 * a compilation unit, a source type, or a source method. Reports the method declarations using
	 * the given collector.
	 * <p>
	 * Consider the following code: <code>
	 * <pre>
	 ...
	 *                </ul>
	 * @deprecated Use
	 *             {@link #searchDeclarationsOfSentMessages(IJavaElement, SearchRequestor, IProgressMonitor)}
	 *             instead.
	 */
