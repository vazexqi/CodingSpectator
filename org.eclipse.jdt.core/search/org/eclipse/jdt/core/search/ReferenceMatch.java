...
public abstract class ReferenceMatch extends SearchMatch {
	...
	/**
	 * Returns the local element of this search match, or <code>null</code> if none. A local element
	 * is the inner-most element that contains the reference and that is not reachable by navigating
	 * from the root of the {@link IJavaModel} using {@link IParent#getChildren()}.
	 * <p>
	 ...
	 * However clients should not assume that this set of element types is closed as other types of
	 * elements may be returned in the future, e.g. if new types of elements are added in the Java
	 * model. Clients can only assume that the {@link IJavaElement#getParent() parent} chain of this
	 * local element eventually leads to the element from {@link #getElement()}.
	 * </p>
	 * <p>
	 ...
	 * </pre>
	 * 
	 * will return one {@link MethodReferenceMatch} match whose local element is the
	 * {@link IAnnotation} '<code>Annot</code>'.<br>
	 * <br>
	 * </li>
	 ...
	 * </pre>
	 * 
	 * will return one {@link TypeReferenceMatch} match whose local element is the
	 * {@link IAnnotation} '<code>Deprecated</code>'.<br>
	 * <br>
	 * </li>
	 ...
	 * </pre>
	 * 
	 * will return one {@link FieldReferenceMatch} match whose local element is the
	 * {@link IAnnotation} '<code>Num</code>'.<br>
	 * <br>
	 * </li>
	 * </ul>
	 * </p>
	 * <p>
	 * A local element may also be a {@link ILocalVariable} whose type is the referenced type. For
	 * example,
	 * <ul>
	 ...
	 * 
	 * will return one {@link TypeReferenceMatch} match whose local element is the
	 * {@link ILocalVariable} '<code>local</code>'.<br>
	 * <br>
	 * </li>
	 * </ul>
	 * Or a local element may be an {@link ITypeParameter} that extends the referenced type. For
	 * example,
	 * <ul>
	 * <li>searching for the references to the type <code>Test</code> in
	 * 
	 ...
	 * 
	 * will return one {@link TypeReferenceMatch} match whose local element is the
	 * {@link ITypeParameter} '<code>T</code>'.<br>
	 * <br>
	 * </ul>
	 * </p>
	 * 
