...
public final class StringOperation {
	 ...
	 * E.g. if the regions are <code>{ start1, length1, start2, length2 }</code>, then
	 * <code>start1+length1</code> will always be smaller than <code>start2</code>.
	 * </p>
	 * <p>
	 * 
	 * <pre>
	 ...
	 * </p>
	 * 
	 * @see CharOperation#camelCaseMatch(char[], int, int, char[], int, int, boolean) for more
	 *      details on the camel case behavior
	 ...
	 *            </ul>
	 * @return an array of <code>int</code> having two slots per returned regions (first one is the
	 *         starting index of the region and the second one the length of the region).<br>
	...
	/**
	 * Answers all the regions in a given name matching a given <i>pattern</i> pattern (e.g.
	 * "H*M??"). </p>
	 * <p>
	 * Each of these regions is made of its starting index and its length in the given name. They
	 * are all concatenated in a single array of <code>int</code> which therefore always has an even
