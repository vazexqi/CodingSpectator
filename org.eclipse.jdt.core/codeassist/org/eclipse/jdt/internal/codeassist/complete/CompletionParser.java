...
public class CompletionParser extends AssistParser {
	...
	/**
	 * Checks if the completion is on the exception type of a catch clause. Returns whether we found
	 * a completion node.
	 */
	...
	/**
	 * Checks if the completion is on the type following a 'new'. Returns whether we found a
	 * completion node.
	 */
	...
	/**
	 * Checks if the completion is on the dot following an array type, a primitive type or an
	 * primitive array type. Returns whether we found a completion node.
	 */
	...
	/**
	 * Checks if the completion is inside a method invocation or a constructor invocation. Returns
	 * whether we found a completion node.
	 */
	...
	/**
	 * Checks if the completion is in the context of a method and on the type of one of its
	 * arguments Returns whether we found a completion node.
	 */
	 ...
	 * Persists the identifier into a fake field return type Returns whether we found a completion
	 * node.
	 */
 ...
 */
		...
		if (this.indexOfAssistIdentifier() < 0) {
			...
			if (this.indexOfAssistIdentifier() != 0 ||
				...
				if (this.currentElement != null) {
					...
					if (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) != Util.getLineNumber((int)(selectorSource >>> 32), this.scanner.lineEnds, 0,
	...
	public TypeReference createParameterizedQualifiedAssistTypeReference(char[][] previousIdentifiers, TypeReference[][] typeArguments, char[] assistName, TypeReference[] assistTypeArguments,
			long[] positions) {
