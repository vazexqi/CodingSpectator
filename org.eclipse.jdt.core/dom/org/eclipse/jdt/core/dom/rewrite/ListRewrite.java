...
public final class ListRewrite {
	 ...
	 * contained in the list. The AST itself is not actually modified in any way; rather, the
	 * rewriter just records a note that this node has been removed from this list.
	 * 
	 ...
	 * contained in the list. The replacement node must either be brand new (not part of the
	 * original AST) or a placeholder node (for example, one created by
	 * {@link ASTRewrite#createCopyTarget(ASTNode)}, {@link ASTRewrite#createMoveTarget(ASTNode)},
	 * or {@link ASTRewrite#createStringPlaceholder(String, int)}). The AST itself is not actually
	 * modified in any way; rather, the rewriter just records a note that this node has been
	 * replaced in this list.
	 * 
	 * @param node the node being removed. The node can either be an original node in this list or
	 *            (since 3.4) a new node already inserted or used as replacement in this AST
	 *            rewriter.
	 ...
	 *             if the described modification is otherwise invalid (not a member of this node's
	 *             original list)
	 */
	...
	/**
	 * Inserts the given node into the list after the given element. The existing node must be in
	 * the list, either as an original or as a new node that has been inserted. The inserted node
	 * must either be brand new (not part of the original AST) or a placeholder node (for example,
	 ...
	 * @param editGroup the edit group in which to collect the corresponding text edits, or
	 *            <code>null</code> if ungrouped
	 * @throws IllegalArgumentException if the node or element is null, or if the node is not part
	 *             of this rewriter's AST, or if the inserted node is not a new node (or
	 *             placeholder), or if <code>element</code> is not a member of the list (original or
	 *             new), or if the described modification is otherwise invalid
	 */
	...
	/**
	 * Inserts the given node into the list before the given element. The existing node must be in
	 * the list, either as an original or as a new node that has been inserted. The inserted node
	 * must either be brand new (not part of the original AST) or a placeholder node (for example,
	 * one created by {@link ASTRewrite#createCopyTarget(ASTNode)},
	 * {@link ASTRewrite#createMoveTarget(ASTNode)}, or
	 ...
	 *            <code>null</code> if ungrouped
	 * @throws IllegalArgumentException if the node is null, or if the node is not part of this
	 *             rewriter's AST, or if the inserted node is not a new node (or placeholder), or if
	 *             the described modification is otherwise invalid (not a member of this node's
	 *             original list)
	 ...
	 *             the described modification is otherwise invalid (not a member of this node's
	 *             original list)
	 * @throws IndexOutOfBoundsException if the index is negative and not -1, or if it is larger
	...
	/**
	 * Creates and returns a placeholder node for a true copy of a range of nodes of the current
	 * list. The placeholder node can either be inserted as new or used to replace an existing node.
	 * When the document is rewritten, a copy of the source code for the given node range is
	 * inserted into the output document at the position corresponding to the placeholder
	 ...
	 *             <code>null</code>, if a node is not a child of the current list or if the first
	 *             node is not before the last node. An <code>IllegalArgumentException</code> is
	 *             also thrown if the copied range is overlapping with an other moved or copied
	 *             range.
	 */
	 ...
	 * the document is rewritten, a copy of the source code for the given node range is inserted
	 ...
	 *             <code>null</code>, if a node is not a child of the current list or if the first
	 *             node is not before the last node. An <code>IllegalArgumentException</code> is
	 *             also thrown if the moved range is overlapping with an other moved or copied
	 *             range.
	 * 
	 * @since 3.1
	 */
	...
	/**
	 * Creates and returns a placeholder node for a move of a range of nodes of the current list.
	 * The moved nodes can optionally be replaced by a specified node.
	 * 
	 * The placeholder node can either be inserted as new or used to replace an existing node. When
	 * the document is rewritten, a copy of the source code for the given node range is inserted
	 * into the output document at the position corresponding to the placeholder (indentation is
	 ...
	 * @param editGroup the edit group in which to collect the corresponding text edits fro a
	 *            replace, or <code>null</code> if ungrouped
	 * @return the new placeholder node
	 * @throws IllegalArgumentException An exception is thrown if the first or last node are
	 *             <code>null</code>, if a node is not a child of the current list or if the first
	 *             node is not before the last node. An <code>IllegalArgumentException
	...
	/**
	 * Returns the original nodes in the list property managed by this rewriter. The returned list
	 * is unmodifiable.
	 * 
	...
	/**
	 * Returns the nodes in the revised list property managed by this rewriter. The returned list is
	 * unmodifiable.
	 * 
