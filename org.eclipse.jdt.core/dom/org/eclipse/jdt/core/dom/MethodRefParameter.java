...
/**
 * AST node for a parameter within a method reference ({@link MethodRef}). These nodes only occur
 * within doc comments ({@link Javadoc}). For JLS2:
 * 
 * <pre>
 * MethodRefParameter:
 * 		Type [ Identifier ]
 ...
 * </pre>
 * <p>
 * Note: The 1.5 spec for the Javadoc tool does not mention the possibility of a variable arity
 * indicator in method references. However, the 1.5 Javadoc tool itself does indeed support it.
 * Since it makes sense to have a way to explicitly refer to variable arity methods, it seems more
 * likely that the Javadoc spec is wrong in this case.
 * </p>
 * 
 * @see Javadoc
 * @since 3.0
...
public class MethodRefParameter extends ASTNode {
	...
	/**
	 * Creates a new AST node for a method referenece parameter owned by the given AST. By default,
	 * the node has an unspecified (but legal) type, not variable arity, and no parameter name.
	 * <p>
	...
	/**
	 * Returns whether this method reference parameter is for the last parameter of a variable arity
	 * method (added in JLS3 API).
	 * <p>
	 * Note that the binding for the type <code>Foo</code>in the vararg method reference
	 * <code>#fun(Foo...)</code> is always for the type as written; i.e., the type binding for
	 * <code>Foo</code>. However, if you navigate from the MethodRef to its method binding to the
	 * type binding for its last parameter, the type binding for the vararg parameter is always an
	 * array type (i.e., <code>Foo[]</code>) reflecting the way vararg methods get compiled.
	 * </p>
	 * 
	 * @return <code>true</code> if this is a variable arity parameter, and <code>false</code>
	 *         otherwise
	...
	/**
	 * Sets whether this method reference parameter is for the last parameter of a variable arity
	 * method (added in JLS3 API).
	 * 
	 * @param variableArity <code>true</code> if this is a variable arity parameter, and
