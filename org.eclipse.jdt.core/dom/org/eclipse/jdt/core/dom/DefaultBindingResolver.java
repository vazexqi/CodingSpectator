 ...
 * <p>
 * IMPORTANT: The methods on this class are synchronized. This is required because there may be
 * multiple clients in separate threads concurrently reading an AST and asking for bindings for its
 * nodes. These requests all end up invoking instance methods on this class. There are various
 * internal tables and caches which are built and maintained in the course of looking up bindings.
 * To ensure that they remain coherent in the presence of multiple threads, the methods are
 * synchronized on the DefaultBindingResolver instance.
 * </p>
 */
class DefaultBindingResolver extends BindingResolver {
	...
	static class BindingTables {
		...
		/**
		 * This map is used to keep the correspondance between new bindings and the compiler
		 * bindings as well as new annotation instances to their internal counterpart. This is an
		 * identity map. We should only create one object for one binding or annotation.
		 */
	...
	/**
	 * This map is used to retrieve an old ast node using the new ast node. This is not an identity
	 * map.
	 */
	Map newAstToOldAst;
	...
	/**
	 * The working copy owner that defines the context in which this resolver is creating the
	 * bindings.
	 */
	WorkingCopyOwner workingCopyOwner;

	/**
	 * Toggle controlling whether DOM bindings should be created when missing internal compiler
	 * bindings..
	 */
	...
	/**
	 * Returns the new type binding corresponding to the given variable declaration. This is used
	 * for recovered binding only.
	 * <p>
		...
		if (node instanceof QualifiedNameReference) {
			...
			if (index >= 0) {
				...
				if (importReferenceLength == index) {
					try {
						binding= this.scope.getImport(CharOperation.subarray(importReference.tokens, 0, index), (importReference.bits & org.eclipse.jdt.internal.compiler.ast.ASTNode.OnDemand) != 0,
