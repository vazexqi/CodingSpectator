...
class DefaultCommentMapper {
	 ...
	 * </p>
	 * <p>
	 * When first comment is found before node, goes up in comment list until one of following
	 ...
	 * </ol>
	 * If some comment have been found, then no token should be on on the same line before, so
	 * remove all comments which do not verify this assumption.
	 * </p>
	 * <p>
	 * If finally there's leading still comments, then stores indexes of the first and last one in
	...
	/**
	 * Search and store node trailing comments. Comments are searched in position range from node
	 * end position to specified next start. If one or several comment are found, returns last
	 * comment end position, otherwise returns node end position.
	 * <p>
	 * Starts to search for first comment after node end position and return if none was found...
	 * </p>
	 * <p>
	 * When first comment is found after node, goes down in comment list until one of following
	 * conditions becomes true:
	 * <ol>
	 ...
	 * </p>
	 * <p>
	 * If finally there's still trailing comments, then stores indexes of the first and last one in
