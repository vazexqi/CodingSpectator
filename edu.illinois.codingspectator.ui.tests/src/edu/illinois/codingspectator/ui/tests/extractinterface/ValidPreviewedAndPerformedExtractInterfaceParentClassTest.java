...
public class ValidPreviewedAndPerformedExtractInterfaceParentClassTest extends RefactoringTest {
	...
	private void codingspectatorAttributesShouldBeCorrect(CapturedRefactoringDescriptor capturedDescriptor) throws EncryptionException {
		...
		assertEquals("44ee7d45a160da76d50b585a3d25a038", Encryptor.toMD5(capturedDescriptor.getCodeSnippet()));
		...
		PatternMatcher expectedNavigationHistoryPatternMatcher= new PatternMatcher(sequence(text("{[Extract Interface,BEGIN_REFACTORING,"), timestampPattern,
				text("],[PreviewPage,Previe&w >,"), timestampPattern, text("],[PreviewPage,OK,"), timestampPattern, text("],}")));
