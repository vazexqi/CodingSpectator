 ...
 * {@link IWorkspace#addSaveParticipant(org.eclipse.core.runtime.Plugin, ISaveParticipant)}
...
public interface ISavedState {
	 ...
	 * successful save in which the plug-in <b>actively</b> participated, or <code>0</code> if the
	...
	/**
	 * Returns the mapped location associated with the given path or <code>null</code> if none.
	 * 
	 ...
	 * active. The listener receives notifications of changes to the workspace resource tree since
	 * the time this state was saved. After this method is run, the delta is forgotten. Subsequent
	 * calls to this method will have no effect.
	 * <p>
	 * No notification is received in the following cases:
	 * <ul>
	 * <li>if a saved state was never recorded ({@link ISaveContext#needDelta()}</code> was not
	 * called)</li>
	 * <li>a saved state has since been forgotten (using {@link IWorkspace#forgetSavedTree(String)})
	 * </li>
	 ...
	 * case should be very similar to the first time a plug-in is activated, and only has the
