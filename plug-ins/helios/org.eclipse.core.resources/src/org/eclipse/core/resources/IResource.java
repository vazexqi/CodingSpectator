 ...
 * is, resource objects do not actually contain data themselves but rather represent resource state
 * and give it behavior. Programmers are free to manipulate handles for resources that do not exist
 * in a workspace but must keep in mind that some methods and operations require that an actual
 * resource be available.</li>
 * <li>Resources have two different kinds of properties as detailed below. All properties are keyed
 * by qualified names.</li>
 * <ul>
 * <li>Session properties: Session properties live for the lifetime of one execution of the
 * workspace. They are not stored on disk. They can carry arbitrary object values. Clients should be
 * aware that these values are kept in memory at all times and, as such, the values should not be
 * large.</li>
 * <li>Persistent properties: Persistent properties have string values which are stored on disk
 * across platform sessions. The value of a persistent property is a string which should be short
 * (i.e., under 2KB).</li>
 * </ul>
 * </li>
 * <li>Resources are identified by type and by their <i>path</i>, which is similar to a file system
 * path. The name of a resource is the last segment of its path. A resource's parent is located by
 * removing the last segment (the resource's name) from the resource's full path.</li>
 * <li>Resources can be local or non-local. A non-local resource is one whose contents and
 * properties have not been fetched from a repository.</li>
 * <li><i>Phantom</i> resources represent incoming additions or outgoing deletions which have yet to
 * be reconciled with a synchronization partner.</li>
 * </ul>
 * </p>
 * <p>
 * Resources implement the <code>IAdaptable</code> interface; extensions are managed by the
...
public interface IResource extends IAdaptable, ISchedulingRule {
	 ...
	 * <code>ALWAYS_DELETE_PROJECT_CONTENT</code> indicates that the contents of a project are to be
	 * deleted regardless of whether the project is open or closed at the time; specifying
	 * <code>NEVER_DELETE_PROJECT_CONTENT</code> indicates that the contents of a project are to be
	 * retained regardless of whether the project is open or closed at the time.
	 * </p>
	 * 
	 ...
	 * should not attempt to configure and de-configure natures.
	 * 
	 ...
	 * time or creating a linked folder should refresh in the background.
	 * 
	...
	/**
	 * Update flag constant (bit mask value 256) indicating that a resource should be replaced with
	 * a resource of the same name at a different file system location.
	 * 
	...
	/**
	 * Update flag constant (bit mask value 0x800) indicating that a resource should be marked as
	 * team private.
	 * 
	 * @see IFile#create(java.io.InputStream, int, IProgressMonitor)
	 * @see IFolder#create(int, boolean, IProgressMonitor)
	 ...
	 * is called, and is provided with a proxy to this resource. The proxy is a transient object
	 * that can be queried very quickly for information about the resource. If the actual resource
	 * handle is needed, it can be obtained from the proxy. Requesting the resource handle, or the
	 ...
	 * {@link IContainer#INCLUDE_PHANTOMS} flag is specified, the visit will also include any
	 * phantom member resource that the workspace is keeping track of.
	 * </p>
	 * <p>
	 * If the {@link IContainer#INCLUDE_TEAM_PRIVATE_MEMBERS} flag is not specified (recommended),
	 * team private members will not be visited. If the
	 * {@link IContainer#INCLUDE_TEAM_PRIVATE_MEMBERS} flag is specified in the member flags, team
	 * private member resources are visited as well.
	 * </p>
	 * <p>
	 ...
	 *                <ul>
	 *                <li>the {@link IContainer#INCLUDE_PHANTOMS} flag is not specified and this
	 ...
	 * 
	 * @param visitor the visitor
	 * @param depth the depth to which members of this resource should be visited. One of
	 *            {@link IResource#DEPTH_ZERO}, {@link IResource#DEPTH_ONE}, or
	 *            {@link IResource#DEPTH_INFINITE}.
	 * @param includePhantoms <code>true</code> if phantom resources are of interest;
	 *            <code>false</code> if phantom resources are not of interest.
	 * @exception CoreException if this request fails. Reasons include:
	 *                <ul>
	 *                <li> <code>includePhantoms</code> is <code>false</code> and this resource does
	 *                not exist.</li>
	 *                <li> <code>includePhantoms</code> is <code>true</code> and this resource does
	 *                not exist and is not a phantom.</li>
	 *                <li>The visitor failed with this exception.</li>
	 *                </ul>
	 * @see IResource#isPhantom()
	 * @see IResourceVisitor#visit(IResource)
	 ...
	 * {@link IContainer#INCLUDE_PHANTOMS} flag is specified, the visit also includes any phantom
	 ...
	 *            {@link IResource#DEPTH_INFINITE}.
	 * @param memberFlags bit-wise or of member flag constants ({@link IContainer#INCLUDE_PHANTOMS},
	 *            {@link IContainer#INCLUDE_TEAM_PRIVATE_MEMBERS}, {@link IContainer#INCLUDE_HIDDEN}
	 *            and {@link IContainer#EXCLUDE_DERIVED}) indicating which members are of interest
	 * @exception CoreException if this request fails. Reasons include:
	 *                <ul>
	 *                <li>the {@link IContainer#INCLUDE_PHANTOMS} flag is not specified and this
	 *                resource does not exist.</li>
	 ...
	 * </p>
	 * <p>
	 * The <code>SHALLOW</code> update flag controls how this method deals with linked resources. If
	 * <code>SHALLOW</code> is not specified, then the underlying contents of the linked resource
	 * will always be copied in the file system. In this case, the destination of the copy will
	 * never be a linked resource or contain any linked resources. If <code>SHALLOW</code> is
	 ...
	 * </p>
	 * <p>
	 * The {@link #DERIVED} update flag indicates that the new resource should immediately be set as
	 * a derived resource. Specifying this flag is equivalent to atomically calling
	 * {@link #setDerived(boolean)} with a value of <code>true</code> immediately after creating the
	 * resource.
	 * </p>
	 * <p>
	 * The {@link #TEAM_PRIVATE} update flag indicates that the new resource should immediately be
	 * set as a team private resource. Specifying this flag is equivalent to atomically calling
	 * {@link #setTeamPrivateMember(boolean)} with a value of <code>true</code> immediately after
	 * creating the resource.
	 * </p>
	 * <p>
	 * The {@link #HIDDEN} update flag indicates that the new resource should immediately be set as
	 * a hidden resource. Specifying this flag is equivalent to atomically calling
	 * {@link #setHidden(boolean)} with a value of <code>true</code> immediately after creating the
	 * resource.
	 * </p>
	 * <p>
	 ...
	 *                <li>The workspace and the local file system are out of sync at the destination
	 *                resource or one of its descendants.</li>
	 *                <li>The source resource is a file and the destination path specifies a
	 *                project.</li>
	 *                <li>The source is a linked resource, and the destination path does not specify
	 *                a project.</li>
	 *                <li>The location of the source resource on disk is the same or a prefix of the
	 *                location of the destination resource on disk.</li>
	 *                <li>Resource changes are disallowed during certain types of resource change
	 *                event notification. See <code>IResourceChangeEvent</code> for more details.</li>
	 *                </ul>
	 * @exception OperationCanceledException if the operation is canceled. Cancellation can occur
	 *                even if no progress monitor is provided.
	 * @see #FORCE
	 * @see #SHALLOW
	 * @see #DERIVED
	 ...
	 * <p>
	 * The <code>SHALLOW</code> update flag controls how this method deals with linked resources. If
	 * <code>SHALLOW</code> is not specified, then the underlying contents of any linked resources
	 ...
	 * 
	 * @param description the destination project description
	 * @param updateFlags bit-wise or of update flag constants (<code>FORCE</code> and
	 *            <code>SHALLOW</code>)
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
	 * @exception CoreException if this resource could not be copied. Reasons include:
	 *                <ul>
	 ...
	 * <p>
	 * Note that once a proxy has been created, it does not stay in sync with the corresponding
	 * resource. Changes to the resource after the proxy is created will not be reflected in the
	 * state of the proxy.
	 * </p>
	 * 
	 ...
	 * correspond to, and are in sync with, resources in the workspace; it will fail if it
	 * encounters a file or directory in the file system that is out of sync with the workspace.
	 * This option ensures there is no unintended data loss; it is the recommended setting. However,
	 * if {@link #FORCE} is specified, the method will ruthlessly attempt to delete corresponding
	 * files and directories in the local file system, including ones that have been recently
	 * updated or created.
	 * </p>
	 * <p>
	 ...
	 * handled. If {@link #ALWAYS_DELETE_PROJECT_CONTENT} is specified, then the files and folders
	 * in a project's local content area are deleted, regardless of whether the project is open or
	 * closed; {@link #FORCE} is assumed regardless of whether it is specified. If
	 * {@link #NEVER_DELETE_PROJECT_CONTENT} is specified, then the files and folders in a project's
	 * local content area are retained, regardless of whether the project is open or closed; the
	 * {@link #FORCE} flag is ignored. If neither of these flags is specified, files and folders in
	 * a project's local content area from open projects (subject to the {@link #FORCE} flag), but
	 ...
	 * <p>
	 * <code>IResource</code> objects are lightweight handle objects used to access resources in the
	 * workspace. However, having a handle object does not necessarily mean the workspace really has
	 * such a resource. When the workspace does have a genuine resource of a matching type, the
	 * resource is said to <em>exist</em>, and this method returns <code>true</code>; in all other
	 * cases, this method returns <code>false</code>. In particular, it returns <code>false</code>
	 * if the workspace has no resource at that path, or if it has a resource at that path with a
	 * type different from the type of this resource handle.
	 * </p>
	 * <p>
	 ...
	 * specified type on this resource, and, optionally, on its children. If
	 * <code>includeSubtypes</code>is <code>false</code>, only markers whose type exactly matches
	 * the given type are considered. Returns <code>-1</code> if there are no matching markers.
	 * Returns {@link IMarker#SEVERITY_ERROR} if any of the markers has a severity greater than or
	 * equal to {@link IMarker#SEVERITY_ERROR}.
	 * 
	 ...
	 * <code>NULL_STAMP</code> if the resource does not exist or is not local or is not accessible.
	 * The return value is represented as the number of milliseconds since the epoch (00:00:00 GMT,
	 * January 1, 1970). The returned value may not be the same as the actual time stamp on disk if
	 * the file has been modified externally since the last local refresh.
	 * <p>
	 ...
	 * such a project, this method returns <code>null</code>. This method also returns
	 * <code>null</code> if called on a resource that is not stored in the local file system. For
	 * such resources {@link #getLocationURI()} should be used instead.
	 * </p>
	 * 
	 * @return the absolute path of this resource in the local file system, or <code>null</code> if
	 *         no path can be determined
	 * @see #getRawLocation()
	 * @see #getLocationURI()
	 ...
	 * <p>
	 * If this resource is the workspace root, this method returns the absolute location of the
	 ...
	 * <p>
	 * If this resource is a linked resource under a project that is open, this method returns the
	 * resolved URI to the linked resource's local contents. This value will be null in the case
	 * where the location is relative to an undefined workspace path variable.
	 * </p>
	 * <p>
	 * If this resource is a file or folder under a project that exists, or a linked resource under
	 * a closed project, this method returns a (non- <code>null</code>) URI computed from the
	 * location of the project's local content area and the project- relative path of the file or
	 * folder. This is true regardless of whether the file or folders exists, or whether the project
	 * is open or closed. In the case of linked resources, the location of a linked resource within
	 * a closed project is computed from the location of the project's local content area and the
	 ...
	 * created. In all other cases, this method returns the same value as {@link #getLocation()}.
	 * </p>
	 * 
	...
	/**
	 * Returns the file system location of this resource, or <code>null</code> if no path can be
	 * determined. The returned path will either be an absolute URI, or a relative URI whose first
	 * path segment is the name of a workspace path variable.
	 * <p>
	 * If this resource is an existing project, the returned path will be equal to the location path
	 * in the project description. If this resource is a linked resource in an open project, the
	 * returned path will be equal to the location path supplied when the linked resource was
	 ...
	 * </p>
	 * 
	 * @return the raw path of this resource in the file system, or <code>null</code> if no path can
	 *         be determined
	 * @see #getLocationURI()
	 * @see IFile#createLink(URI, int, IProgressMonitor)
	 * @see IFolder#createLink(URI, int, IProgressMonitor)
	 ...
	 * </p>
	 * <p>
	 * Attributes that are not supported by the underlying file system will have a value of
	 * <code>false</code>.
	 * </p>
	 * <p>
	 * Sample usage: <br>
	 ...
	 * </p>
	 * 
	 * @return the extended attributes from the file system, or <code>null</code> if they could not
	 *         be obtained
	 ...
	 * 
	 * @return the map containing the session properties where the key is the {@link QualifiedName}
	 *         of the property and the value is the property value (an {@link Object}.
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li>This resource does not exist.</li>
	 *                <li>This resource is not local.</li>
	 ...
	 * 
	 * <p>
	 * This is a convenience method, fully equivalent to <code>isDerived(IResource.NONE)</code>.
	 * </p>
	 * 
	 ...
	 *            applicable)
	 * @return <code>true</code> if this resource subtree is derived, and <code>false</code>
	 *         otherwise
	 * @see IResource#setDerived(boolean)
	 * @since 3.4
	 */
	 ...
	 * <p>
	 * This is a convenience method, fully equivalent to <code>isLinked(IResource.NONE)</code>.
	 * </p>
	 * 
	...
	/**
	 * Returns whether this resource is a virtual resource. Returns <code>true</code> for folders
	 * that have been marked virtual using the {@link #VIRTUAL} update flag. Returns
	 * <code>false</code> in all other cases, including the case where this resource does not exist.
	 * The workspace root, projects and files currently cannot be made virtual.
	 * 
	 ...
	 * content area or another location within the project. Returns <code>false</code> in all other
	 * cases, including the case where this resource does not exist. The workspace root and projects
	 * are never linked.
	 * <p>
	 * This method returns true for a resource that has been linked using the
	 * <code>createLink</code> method.
	 * </p>
	 * <p>
	 * The {@link #CHECK_ANCESTORS} option flag indicates whether this method should consider
	 * ancestor resources in its calculation. If the {@link #CHECK_ANCESTORS} flag is present, this
	 ...
	 * </p>
	 * 
	 * @param depth valid values are <code>DEPTH_ZERO</code>, <code>DEPTH_ONE</code>, or
	 ...
	 * <code>exists</code>, which returns <code>false</code> for phantoms) are therefore invisible
	 * except through a handful of phantom-enabled API methods (notably
	 * <code>IContainer.members(boolean)</code>).
	 * </p>
	 * 
	 ...
	 * <li>The timestamp in the file system has not changed since the last synchronization.</li>
	 * <li>The resource in the workspace is of the same type as the corresponding file in the file
	 * system (they are either both files or both folders).</li>
	 * </ul>
	 ...
	 * be a linked resource or contain any linked resources. If <code>SHALLOW</code> is specified
	 * when a linked resource is moved into another project, a new linked resource is created in the
	 ...
	 * </p>
	 * <p>
	 * Update flags other than <code>FORCE</code>, <code>KEEP_HISTORY</code>and <code>SHALLOW</code>
	 * are ignored.
	 * </p>
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 ...
	 * successful completion, the old project and any direct or indirect members will no longer
	 * exist; but corresponding new resources will now exist in the new project.
	 * <p>
	 * When a resource moves, its session and persistent properties move with it. Likewise for all
	 ...
	 * the location is different, then the project contents will be moved to the new location. In
	 * all other cases the directories and files on disk are left untouched. Parts of the supplied
	 ...
	 * <p>
	 * The <code>SHALLOW</code> update flag controls how this method deals with linked resources. If
	 * <code>SHALLOW</code> is not specified, then the underlying contents of any linked resource
	 ...
	 * linked resources is moved, new linked resources are created in the destination project
	 * pointing to the same file system locations. In this case, no files on disk under any linked
	 * resource are actually moved. The <code>SHALLOW</code> update flag is ignored when moving non-
	 * linked resources.
	 * </p>
	 * <p>
	 * The {@link #REPLACE} update flag controls how this method deals with a change of location. If
	 * the location changes and the {@link #REPLACE} flag is not specified, then the projects
	 * contents on disk are moved to the new location. If the location changes and the
	 * {@link #REPLACE} flag is specified, then the project is reoriented to correspond to the new
	 * location, but no contents are moved on disk. The contents already on disk at the new location
	 * become the project contents. If the new project location does not exist, it will be created.
	 * </p>
	 * <p>
	 * Update flags other than those listed above are ignored.
	 * </p>
	 ...
	 * @param description the destination project description
	 * @param updateFlags bit-wise or of update flag constants ({@link #FORCE},
	 *            {@link #KEEP_HISTORY}, {@link #SHALLOW} and {@link #REPLACE}).
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
	 * @exception CoreException if this resource could not be moved. Reasons include:
	 *                <ul>
	 *                <li>This resource does not exist.</li>
	 ...
	 *                <li>Resource changes are disallowed during certain types of resource change
	 *                event notification. See <code>IResourceChangeEvent</code> for more details.</li>
	 *                <li>The destination file system location is occupied. When moving a project in
	 *                the file system, the destination directory must either not exist or be empty.</li>
	 *                </ul>
	 * @exception OperationCanceledException if the operation is canceled. Cancelation can occur
	 *                even if no progress monitor is provided.
	 * @see IResourceDelta#getFlags()
	 ...
	 * <p>
	 * It is the caller's responsibility to ensure that the value of the reverted modification stamp
	 * matches this resource's modification stamp prior to the change that has been rolled back.
	 ...
	 * resource states should be different if and only if one or more of the attributes listed in
	 * the specification as affecting the modification stamp have changed.
	 * <p>
	 ...
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li> This resource does not exist.</li> <li> This resource is not local.</li>
	 *                <li> This resource is not accessible.</li> <li> Resource changes are
	 *                disallowed during certain types of resource change event notification. See
	 ...
	 * that the resource is not under version and configuration management <i>by default</i>. That
	 * is, the resource should only be stored in a team repository if the user explicitly indicates
	 ...
	 * tree, and are discarded when the resources are deleted. Derived marks are saved to disk when
	 * a project is closed, or when the workspace is saved.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * 
	 * @param isDerived <code>true</code> if this resource is to be marked as derived, and
	 ...
	 * </p>
	 * <p>
	 * These changes will be reported in a subsequent resource change event, including an indication
	 * that this file's derived flag has changed.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	 ...
	 * <p>
	 * Hidden resources are invisible to most clients. Newly-created resources are not hidden
	 ...
	 * </p>
	 * 
	 * @param isHidden <code>true</code> if this resource is to be marked as hidden, and
	 *            <code>false</code> otherwise
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li>This resource does not exist.</li>
	 ...
	 * always local and attempting to set either to non-local (i.e., passing <code>false</code>) has
	 * no affect on the resource.
	 * <p>
	 * When a resource is not local, its content and properties are unavailable for both reading and
	 * writing.
	 * </p>
	 ...
	 * </p>
	 * <p>
	 * Note that a resource cannot be converted into a symbolic link by setting resource attributes
	 ...
	 * <p>
	 * A <b>team private member</b> resource is a special file or folder created by a team provider
	 ...
	 * resources are deleted. Team private member marks are saved to disk when a project is closed,
	 * or when the workspace is saved.
	 * </p>
	 * <p>
	 * Projects and the workspace root are never considered team private members; attempts to mark
