 ...
 * passed to the {@link ResourceChangeValidator#validateChange(IResourceDelta, IProgressMonitor)}
 * method in order to validate the change with any model providers stored in those resources. The
 * deltas created by calls to the methods of this interface will be the same as those generated by
 * the workspace if the proposed operations were performed.
 * <p>
...
public interface IResourceChangeDescriptionFactory {
	...
	/**
	 * Record the set of deltas representing a copy of the given resource to the given workspace
	 * path.
	 * 
	...
	/**
	 * Record the set of deltas representing a move of the given resource to the given workspace
	 * path. Note that this API is used to describe a resource being moved to another path in the
	 * workspace, rather than a move in the file system.
	 * 
