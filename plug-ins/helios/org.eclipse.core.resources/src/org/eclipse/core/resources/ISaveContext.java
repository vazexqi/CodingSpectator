 ...
 * <p>
 * Note that <code>IWorkspace.save</code> uses a different save context for each registered
...
public interface ISaveContext {
	...
	/**
	 * Returns current files mapped with the <code>ISaveContext.map</code> facility or an empty
	 * array if there are no mapped files.
	 * 
	...
	/**
	 * Returns the number for the previous save in which the plug-in actively participated, or
	 * <code>0</code> if the plug-in has never actively participated in a save before.
	 * <p>
	 * In the event of an unsuccessful save, this is the value to <code>rollback</code> to.
	 * </p>
	 * 
	...
	/**
	 * Returns the number for this save. This number is guaranteed to be <code>1</code> more than
	 * the previous save number.
	 * <p>
	 * This is the value to use when, for example, creating files in which a participant will save
	 * its data.
	 * </p>
	 * 
	 ...
	 * locations.
	 * <p>
	 * For example, assume a plug-in has a configuration file named "config.properties". The map
	 ...
	 * </p>
	 * <p>
	 * Each save participant must manage the deletion of its old state files. Old state files can be
	 * discovered using <code>getPreviousSaveNumber</code> or by using <code>getFiles</code> to
	...
	/**
	 * Indicates that the saved workspace tree should be remembered so that a delta will be
	 * available in a subsequent session when the plug-in re-registers to participate in saves. If
	 * this method is not called, no resource delta will be made available. This facility is not
	 * available for marker deltas. Plug-ins must assume that all markers may have changed when they
	 * are activated.
	 * <p>
	 * Note that this is orthogonal to <code>needSaveNumber</code>. That is, one can ask for a delta
	 * regardless of whether or not one is an active participant.
	 * </p>
	 * <p>
	 * Note that deltas are not guaranteed to be saved even if saving is requested. Deltas cannot be
	 * supplied where the previous state is too old or has become invalid.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * Note that this is orthogonal to <code>needDelta</code>. That is, one can be an active
