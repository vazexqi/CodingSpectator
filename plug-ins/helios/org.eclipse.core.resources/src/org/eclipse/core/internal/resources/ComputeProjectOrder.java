...
/**
 * Implementation of a sort algorithm for computing the project order. This algorithm handles cycles
 * in the project reference graph in a reasonable way.
 * 
 * @since 2.1
 */
class ComputeProjectOrder {
	...
	/**
	 * A directed graph. Once the vertexes and edges of the graph have been defined, the graph can
	 * be queried for the depth-first finish time of each vertex.
	 * <p>
	 * Ref: Cormen, Leiserson, and Rivest <it>Introduction to Algorithms</it>, McGraw-Hill, 1990.
	 * The depth-first search algorithm is in section 23.3.
	 * </p>
	 */
	private static class Digraph {
		/**
		 * struct-like object for representing a vertex along with various values computed during
		...
		public static class Vertex {
			...
			/**
			 * Color of the vertex. One of <code>WHITE</code> (unvisited), <code>GREY</code> (visit
			 * in progress), or <code>BLACK</code> (visit finished). <code>WHITE</code> initially.
			 */
			...
			/**
			 * Timestamp indicating when the vertex was finished (became BLACK) in the DFS. Finish
			 * times are between 1 and the number of vertexes.
			 */
			...
			/**
			 * Ordered list of adjacent vertexes. In other words, "this" is the "from" vertex and
			 * the elements of this list are all "to" vertexes.
			 * 
		...
		/**
		 * Indicates whether the graph has been initialized. Initially <code>false</code>.
		 */
		 ...
		 * <p>
		 * After this graph's vertexes and edges are defined with <code>addVertex</code> and
		 * <code>addEdge</code>, call <code>freeze</code> to indicate that the graph is all there,
		 * and then call <code>idsByDFSFinishTime</code> to read off the vertexes ordered by DFS
		 * finish time.
		 * </p>
		 */
		public Digraph() {
		...
		/**
		 * Freezes this graph. No more vertexes or edges can be added to this graph after this
		 * method is called. Has no effect if the graph is already frozen.
		 */
		public void freeze() {
			if (!initialized) {
				initialized= true;
		...
		/**
		 * Defines a new vertex with the given id. The depth-first search is performed in the
		 * relative order in which vertexes were added to the graph.
		 * 
		 * @param id the id of the vertex
		 * @exception IllegalArgumentException if the vertex id is already defined or if the graph
		 *                is frozen
		 */
		...
		/**
		 * Adds a new directed edge between the vertexes with the given ids. Vertexes for the given
		 * ids must be defined beforehand with <code>addVertex</code>. The depth-first search is
		 * performed in the relative order in which adjacent "to" vertexes were added to a given
		 * "from" index.
		 * 
		 * @param fromId the id of the "from" vertex
		 * @param toId the id of the "to" vertex
		 * @exception IllegalArgumentException if either vertex is undefined or if the graph is
		 *                frozen
		 */
		...
		/**
		 * Returns the ids of the vertexes in this graph ordered by depth-first search finish time.
		 * The graph must be frozen.
		 * 
		 * @param increasing <code>true</code> if objects are to be arranged into increasing order
		 *            of depth-first search finish time, and <code>false</code> if objects are to be
		 *            arranged into decreasing order of depth-first search finish time
		 * @return the list of ids ordered by depth-first search finish time (element type:
		 *         <code>Object</code>)
		...
		/**
		 * Returns the non-trivial components of this graph. A non-trivial component is a set of 2
		 * or more vertexes that were traversed together. The graph must be frozen.
		 * 
		 * @return the possibly empty list of non-trivial components, where each component is an
		 *         array of ids (element type: <code>Object[]</code>)
		 * @exception IllegalArgumentException if the graph is not frozen
		 */
		...
		/**
		 * Performs a depth-first search of this graph and records interesting info with each
		 * vertex, including DFS finish time. Does not employ recursion.
		 */
		private void DFS() {
	...
	/**
	 * Sorts the given list of project in a manner that honors the given project reference
	 * relationships. That is, if project A references project B, then the resulting order will list
	 * B before A if possible. For graphs that do not contain cycles, the result is the same as a
	 * conventional topological sort. For graphs containing cycles, the order is based on ordering
	 * the strongly connected components of the graph. This has the effect of keeping each knot of
	 * projects together without otherwise affecting the order of projects not involved in a cycle.
	 * For a graph G, the algorithm performs in O(|G|) space and time.
	 * <p>
	 * When there is an arbitrary choice, vertexes are ordered as supplied. Arranged projects in
	 * descending alphabetical order generally results in an order that builds "A" before "Z" when
	 * there are no other constraints.
	 * </p>
	 * <p>
	 * Ref: Cormen, Leiserson, and Rivest <it>Introduction to Algorithms</it>, McGraw-Hill, 1990.
	 * The strongly-connected-components algorithm is in section 23.5.
	 * </p>
	 * 
	 * @param projects a list of projects (element type: <code>IProject</code>)
	 * @param references a list of project references [A,B] meaning that A references B (element
	 *            type: <code>IProject[]</code>)
