...
public class MarkerDelta implements IMarkerDelta, IMarkerSetElement {
	...
	/**
	 * Merge two Maps of (IPath->MarkerSet) representing changes. Use the old map to store the
	 * result so we don't have to build a new map to return.
	 */
	...
	/**
	 * Merge two sets of marker changes. Both sets must be on the same resource. Use the original
	 * set of changes to store the result so we don't have to build a completely different set to
	 * return.
	 * 
	 * add + add = N/A add + remove = nothing (no delta) add + change = add remove + add = N/A
	 * remove + remove = N/A remove + change = N/A change + add = N/A change + change = change
	 * (note: info held onto by the marker delta should be that of the oldest change, and not
	 * replaced when composed) change + remove = remove (note: info held onto by the marker delta
	 * should be that of the oldest change, and not replaced when changed to a remove)
	 */
