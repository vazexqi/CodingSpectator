...
/**
 * Manages monitors by creating new monitors when projects are added and removing monitors when
 * projects are removed. Also handles the polling mechanism when contributed native monitors cannot
 * handle a project.
 * 
 * @since 3.0
 */
class MonitorManager implements ILifecycleListener, IPathVariableChangeListener, IResourceChangeListener, IResourceDeltaVisitor {
	...
	/**
	 * Queries extensions of the refreshProviders extension point, and creates the provider classes.
	 * Will never return <code>null</code>.
	 * 
	 * @return RefreshProvider[] The array of registered <code>RefreshProvider</code> objects or an
	 *         empty array.
	 */
	...
	/**
	 * Collects the set of root resources that required monitoring. This includes projects and all
	 * linked resources.
	 */
	...
	/**
	 * Installs a monitor on the given resource. Returns true if the polling monitor was installed,
	 * and false if a refresh provider was installed.
	 */
