...
public class PathVariableManager implements IPathVariableManager, IManager {
	...
	/**
	 * Fires a property change event corresponding to a change to the current value of the variable
	 * with the given name.
	 * 
	 * @param name the name of the variable, to be used as the variable in the event object
	 * @param value the current value of the path variable or <code>null</code> if the variable was
	 *            deleted
	...
	/**
	 * Note that if a user changes the key in the preferences file to be invalid and then calls
	 * #getValue using that key, they will get the value back for that. But then if they try and
	 * call #setValue using the same key it will throw an exception. We may want to revisit this
