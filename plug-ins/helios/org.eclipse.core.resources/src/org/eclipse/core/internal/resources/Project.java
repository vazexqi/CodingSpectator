...
public class Project extends Container implements IProject {
	...
	/**
	 * Option constant (value 2) indicating that the snapshot location shall be persisted with the
	 * project for autoloading the snapshot when the project is imported in another workspace.
	 * <p>
	...
	/**
	 * Checks that this resource is accessible. Typically this means that it exists. In the case of
	 * projects, they must also be open. If phantom is true, phantom resources are considered.
	 * 
	...
	protected void internalBuild(final int trigger, final String builderName, final Map args, IProgressMonitor monitor) throws CoreException {
		workspace.run(new IWorkspaceRunnable() {
			 ...
			 * 
			 * @return <code>true</code> if the build should proceed, and <code>false</code>
			 *         otherwise.
			 */
	...
	/**
	 * This is an internal helper method. This implementation is different from the API method
	 * getDescription(). This one does not check the project accessibility. It exists in order to
	 * prevent "chicken and egg" problems in places like the project creation. It may return null.
	 */
	...
	/**
	 * Sets this project's description to the given value. This is the body of the corresponding API
	 * method but is needed separately since it is used during workspace restore (i.e., when you
	 * cannot do an operation)
	 */
	...
	/**
	 * Returns true if this resource represents the project description file, and false otherwise.
	 */
	...
	/**
	 * Loads a snapshot of project meta-data from the given location URI. Like
	 * {@link IProject#loadSnapshot(int, URI, IProgressMonitor)} but can be called when the project
	 * is open.
	 * 
	...
	/**
	 * The project description file has changed on disk, resulting in a changed set of linked
	 * resources. Perform the necessary creations and deletions of links to bring the links in sync
	 * with those described in the project description.
	 * 
	 * @param newDescription the new project description that may have changed link descriptions.
	 * @return status ok if everything went well, otherwise an ERROR multi-status describing the
	...
	/**
	 * Restore the non-persisted state for the project. For example, read and set the description
	 * from the local meta area. Also, open the property store etc. This method is used when an open
	 * project is restored and so emulates the behaviour of open().
	 */
	...
	/**
	 * The project description file on disk is better than the description in memory. Make sure the
	 * project description in memory is synchronized with the description file contents.
	 */
	...
	/**
	 * Writes the project description file to disk. This is the only method that should ever be
	 * writing the description, because it ensures that the description isn't then immediately
	 * discovered as an incoming change and read back from disk.
	 * 
