...
/**
 * An element changed event describes a change to the structure or contents of a tree of Java
 * elements. The changes to the elements are described by the associated delta object carried by
 * this event.
 * <p>
 * This class is not intended to be instantiated or subclassed by clients. Instances of this class
 * are automatically created by the Java model.
 * </p>
 * 
...
public class ElementChangedEvent extends EventObject {
	 ...
	 * and modifications to one or more Java element(s) expressed as a hierarchical java element
	 * delta as returned by <code>getDelta()</code>.
	 * 
	 * Note: this notification occurs during the corresponding POST_CHANGE resource change
	 * notification, and contains a full delta accounting for any JavaModel operation and/or
	 * resource change.
	 * 
	 * @see IJavaElementDelta
	 ...
	 * notification. The delta, which is notified here, only contains information relative to the
	 * previous JavaModel operations (in other words, it ignores the possible resources which have
	 * changed outside Java operations). In particular, it is possible that the JavaModel be
	 * inconsistent with respect to resources, which got modified outside JavaModel operations (it
	 * will only be fully consistent once the POST_CHANGE notification has occurred).
	 * 
	 * @see IJavaElementDelta
	 * @see org.eclipse.core.resources.IResourceChangeEvent
	 * @see #getDelta()
