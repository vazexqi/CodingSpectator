 ...
 * Opening a textual element (such as a compilation unit) involves opening a buffer on its contents.
 * While open, any changes to the buffer can be reflected in the element's structure; see
 * {@link #isConsistent} and {@link #makeConsistent(IProgressMonitor)}.
 * <p>
 * To reduce complexity in clients, elements are automatically opened by the Java model as element
 * properties are accessed. The Java model maintains an LRU cache of open elements, and
 ...
 * </p>
 * <p>
 * To open an element, all openable parent elements must be open. The Java model automatically opens
 ...
 * themselves {@link IOpenable}. For example, opening a compilation unit provides access to all its
 * constituent elements, but opening a package fragment does not open all compilation units in the
 * package fragment.
 * </p>
 * 
...
public interface IOpenable {
	...
	/**
	 * Finds and returns the recommended line separator for this element. The element's buffer is
	 * first searched and the first line separator in this buffer is returned if any. Otherwise the
	 ...
	 * <p>
	 * Note: Using this functionality on a working copy will interfere with any subsequent
	 * reconciling operation. Indeed, the next
	 ...
	 * @exception JavaModelException if the element is unable to access the contents of its
	 ...
	 * 
	 * @param progress the given progress monitor
	 * @exception JavaModelException if an error occurs accessing the contents of its underlying
	 ...
	 *                resource. Reasons include:
	 *                <ul>
	 *                <li>This Java element does not exist (ELEMENT_DOES_NOT_EXIST)</li> <li>This
	 *                Java element is read-only (READ_ONLY)</li>
	 *                </ul>
	 */
