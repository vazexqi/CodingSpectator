...
/**
 * A Java element delta describes changes in Java element between two discrete points in time. Given
 * a delta, clients can access the element that has changed, and any children that have changed.
 * <p>
 ...
 * constants for a more detailled description):
 * <ul>
 ...
 * values:
 * <ul>
 * <li>{@link #F_ADDED_TO_CLASSPATH} - A classpath entry corresponding to the element has been added
 * to the project's classpath. This flag is only valid if the element is an
 * {@link IPackageFragmentRoot}.</li>
 * <li>{@link #F_ARCHIVE_CONTENT_CHANGED} - The contents of an archive has changed in some way. This
 * flag is only valid if the element is an {@link IPackageFragmentRoot} which is an archive.</li>
 * <li>{@link #F_CHILDREN} - A child of the element has changed in some way. This flag is only valid
 * if the element is an {@link IParent}.</li>
 * <li>{@link #F_CLASSPATH_REORDER} - A classpath entry corresponding to the element has changed
 * position in the project's classpath. This flag is only valid if the element is an
 * {@link IPackageFragmentRoot}.</li>
 * <li>{@link #F_CLOSED} - The underlying {@link org.eclipse.core.resources.IProject} has been
 * closed. This flag is only valid if the element is an {@link IJavaProject}.</li>
 ...
 * <li>{@link #F_MODIFIERS} - The modifiers on the element have changed in some way. This flag is
 * only valid if the element is an {@link IMember}.</li>
 * <li>{@link #F_OPENED} - The underlying {@link org.eclipse.core.resources.IProject} has been
 * opened. This flag is only valid if the element is an {@link IJavaProject}.</li>
 * <li>{@link #F_REMOVED_FROM_CLASSPATH} - A classpath entry corresponding to the element has been
 * removed from the project's classpath. This flag is only valid if the element is an
 * {@link IPackageFragmentRoot}.</li>
 * <li>{@link #F_SOURCEATTACHED} - The source attachment path or the source attachment root path of
 * a classpath entry corresponding to the element was added. This flag is only valid if the element
 * is an {@link IPackageFragmentRoot}.</li>
 * <li>{@link #F_SOURCEDETACHED} - The source attachment path or the source attachment root path of
 * a classpath entry corresponding to the element was removed. This flag is only valid if the
 * element is an {@link IPackageFragmentRoot}.</li>
 ...
 * {@link #F_CLASSPATH_REORDER} flags are triggered by changes to a project's classpath. They do not
 * mean that the underlying resource was added, removed or changed. For example, if a project P
 * already contains a folder src, then adding a classpath entry with the 'P/src' path to the
 * project's classpath will result in an {@link IJavaElementDelta} with the
 * {@link #F_ADDED_TO_CLASSPATH} flag for the {@link IPackageFragmentRoot} P/src. On the contrary,
 * if a resource is physically added, removed or changed and this resource corresponds to a
 * classpath entry of the project, then an {@link IJavaElementDelta} with the {@link #ADDED},
 * {@link #REMOVED}, or {@link #CHANGED} kind will be fired.
 * </p>
 * <p>
 * Note that when a source attachment path or a source attachment root path is changed, then the
 * flags of the delta contain both {@link #F_SOURCEATTACHED} and {@link #F_SOURCEDETACHED}.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * {@link IJavaElementDelta} object are not valid outside the dynamic scope of the notification.
 * </p>
 * 
...
public interface IJavaElementDelta {
	...
	/**
	 * Status constant indicating that the element has been removed. Note that a removed java
	...
	/**
	 * Change flag indicating that the modifiers of the element have changed. This flag is only
	 * valid if the element is an {@link IMember}.
	 */
	public int F_MODIFIERS= 0x000002;

	/**
	 * Change flag indicating that there are changes to the children of the element. This flag is
	 * only valid if the element is an {@link IParent}.
	 */
	...
	/**
	 * Change flag indicating that the element was moved to another location. The location of the
	 * new element can be retrieved using {@link #getMovedToElement}.
	 */
	...
	/**
	 * Change flag indicating that a classpath entry corresponding to the element has changed
	 * position in the project's classpath. This flag is only valid if the element is an
	 * {@link IPackageFragmentRoot}.
	 * 
	...
	/**
	 * Change flag indicating that the element has changed position relatively to its siblings. If
	 * the element is an {@link IPackageFragmentRoot}, a classpath entry corresponding to the
	 * element has changed position in the project's classpath.
	 * 
	 * @since 2.1
	 */
	...
	/**
	 * Change flag indicating that the underlying {@link org.eclipse.core.resources.IProject} has
	 * been opened. This flag is only valid if the element is an {@link IJavaProject}.
	 */
	public int F_OPENED= 0x000200;

	/**
	 * Change flag indicating that the underlying {@link org.eclipse.core.resources.IProject} has
	 * been closed. This flag is only valid if the element is an {@link IJavaProject}.
	 */
	public int F_CLOSED= 0x000400;

	/**
	 * Change flag indicating that one of the supertypes of an {@link IType} has changed.
	 */
	public int F_SUPER_TYPES= 0x000800;

	/**
	 * Change flag indicating that the source attachment path or the source attachment root path of
	 * a classpath entry corresponding to the element was added. This flag is only valid if the
	 ...
	 * <p>
	 * Clients can use this flag to find out if a compilation unit that have a {@link #F_CONTENT}
	 ...
	 * primary working copy has reverted to a compilation unit. This flag is only valid if the
	 * element is an {@link ICompilationUnit}.
	 * 
	 * @since 3.0
	 */
	 ...
	 * {@link IJavaProject#getOutputLocation() output folder}) of a project has changed. This flag
	 * is only valid if the element is an {@link IJavaProject}. Also see
	 * {@link #F_RESOLVED_CLASSPATH_CHANGED}, which indicates that there is a change to the
	 * {@link IJavaProject#getResolvedClasspath(boolean) resolved class path}. The resolved
	 * classpath can change without the raw classpath changing (e.g. if a container resolves to a
	 * different set of classpath entries). And conversely, it is possible to construct a case where
	 * the raw classpath can change without the resolved classpath changing.
	 * 
	 * @since 3.0
	 */
	 ...
	 * is only valid if the element is a primary {@link ICompilationUnit}.
	 * 
	 * @since 3.0
	 */
	...
	/**
	 * Change flag indicating that a reconcile operation has affected the compilation unit AST
	 * created in a previous reconcile operation. Use {@link #getCompilationUnitAST()} to retrieve
	 * the AST (if any is available). This flag is only valid if the element is an
	 * {@link ICompilationUnit} in working copy mode.
	 * 
	 * @since 3.2
	 */
	...
	/**
	 * Change flag indicating that the categories of the element have changed. This flag is only
	 * valid if the element is an {@link IMember}.
	 * 
	 * @since 3.2
	 */
	...
	/**
	 * Change flag indicating that the {@link IJavaProject#getResolvedClasspath(boolean) resolved
	 * classpath} of a project has changed. This flag is only valid if the element is an
	 * {@link IJavaProject}. Also see {@link #F_CLASSPATH_CHANGED}, which indicates that there is a
	 * change to the {@link IJavaProject#getRawClasspath() raw class path}. The resolved classpath
	 * can change without the raw classpath changing (e.g. if a container resolves to a different
	 * set of classpath entries). And conversely, it is possible to construct a case where the raw
	...
	/**
	 * Change flag indicating that the annotations of the element have changed. Use
	 * {@link #getAnnotationDeltas()} to get the added/removed/changed annotations. This flag is
	 * only valid if the element is an {@link IAnnotatable}.
	 * 
	 * @since 3.4
	 */
	...
	/**
	 * Returns deltas for affected annotations (added, removed, or changed). Returns an empty array
	 * if no annotations was affected, or if this delta's element is not an {@link IAnnotatable}.
	 * 
	...
	/**
	 * Returns the compilation unit AST created by the last reconcile operation on this delta's
	...
	/**
	 * Returns flags that describe how an element has changed. Such flags should be tested using the
	...
	/**
	 * Returns the kind of this delta - one of {@link #ADDED}, {@link #REMOVED}, or {@link #CHANGED}
	 * .
	 * 
	...
	/**
	 * Returns an element describing this element in its new location, or <code>null</code> if the
