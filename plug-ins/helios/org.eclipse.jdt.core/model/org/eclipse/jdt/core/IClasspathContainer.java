...
/**
 * Interface of a classpath container. A classpath container provides a way to indirectly reference
 * a set of classpath entries through a classpath entry of kind <code>CPE_CONTAINER</code>.
 * Typically, a classpath container can be used to describe a complex library composed of multiple
 * JARs or projects, considering also that containers can map to different set of entries on each
 * project, in other words, several projects can reference the same generic container path, but have
 * each of them actually bound to a different container object.
 * <p>
 ...
 * </ul>
 * In particular, a classpath container can neither reference further classpath containers or
 * classpath variables.
 * <p>
 * Classpath container values are persisted locally to the workspace, but are not preserved from a
 * session to another. It is thus highly recommended to register a
 * <code>ClasspathContainerInitializer</code> for each referenced container (through the extension
...
public interface IClasspathContainer {
	...
	/**
	 * Kind for a container mapping to a default system library, implicitly contributed by the
	 * runtime
	 */
	 ...
	 * <li>project entries (<code>CPE_PROJECT</code>)</li>
	 * </ul>
	 * A classpath container can neither reference further classpath containers or classpath
	 * variables.
	 * </p>
	 * <p>
	 * This method is called by the Java model when it needs to resolve this classpath container
	 * entry into a list of library and project entries. The method is typically called exactly once
	 * for a given Java project, and the resulting list of entries cached internally by the Java
	 * model. This method must not be called by other clients.
	 * <p>
	 ...
	 * registered on the extension point "org.eclipse.jdt.core.classpathContainerInitializer", which
	 * can be invoked if needing to resolve the container before it is explicitly set.
	 * <p>
	 * 
