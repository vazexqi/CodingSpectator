...
public final class BindingKey {
	...
	/**
	 * Creates a new array type binding key from the given type binding key and the given array
	 * dimension.
	 * <p>
	 * For example:
	 * 
	...
	/**
	 * Creates a new parameterized type binding key from the given generic type binding key and the
	 * given argument type binding keys. If the argument type keys array is empty, then a raw type
	...
	/**
	 * Creates a new type binding key from the given type name. The type name must be either a fully
	 * qualified name, an array type name or a primitive type name. If the type name is fully
	 * qualified, then it is expected to be dot-based. Note that inner types, generic types and
	...
	/**
	 * Creates a new type variable binding key from the given type variable name and the given
	 * declaring key. The declaring key can either be a type binding key or a method binding key.
	 * <p>
	 * For example:
	 * 
	 * <pre>
	...
	/**
	 * Creates a new wildcard type binding key from the given type binding key and the given
	 * wildcard kind (one of {@link Signature#C_STAR}, {@link Signature#C_SUPER}, or
	 * {@link Signature#C_EXTENDS}. If the wildcard is {@link Signature#C_STAR}, the given type
	 * binding key is ignored.
	 * <p>
	 * For example:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param typeKey the binding key of the given type
	 * @param kind one of {@link Signature#C_STAR}, {@link Signature#C_SUPER}, or
	 *            {@link Signature#C_EXTENDS}
	...
	/**
	 * Creates a new wildcard type binding key from the given generic type binding key, the given
	 * wildcard kind (one of {@link Signature#C_STAR}, {@link Signature#C_SUPER}, or
	 * {@link Signature#C_EXTENDS} the given bound type binding key and the given rank. If the
	 * wildcard kind is {@link Signature#C_STAR}, the given bound type binding key is ignored.
	 * <p>
	 * For example:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param genericTypeKey the binding key of the generic type
	 * @param boundKind one of {@link Signature#C_STAR}, {@link Signature#C_SUPER}, or
	 *            {@link Signature#C_EXTENDS}
	 * @param boundTypeKey the binding key of the bounding type.
	 * @param rank the relative position of this wild card type in the parameterization of the
	 *            generic type.
	 * @return a new wildcard type binding key
	 * @since 3.5
	 */
	...
	/**
	 * Returns the thrown exception signatures of the element represented by this binding key. If
	 * this binding key does not represent a method or does not throw any exception, returns an
	 * empty array.
	 * 
	...
	/**
	 * Returns the type argument signatures of the element represented by this binding key. If this
	 * binding key doesn't represent a parameterized type or a parameterized method, returns an
	 * empty array.
	 * 
	...
	/**
	 * Returns whether this binding key represents a parameterized type, or if its declaring type is
	...
	/**
	 * Returns whether this binding key represents a parameterized method, or if its declaring
	 * method is a parameterized method.
	 * 
	...
	/**
	 * Transforms this binding key into a resolved signature. If this binding key represents a
	 * field, the returned signature is the field type's signature.
	 * 
