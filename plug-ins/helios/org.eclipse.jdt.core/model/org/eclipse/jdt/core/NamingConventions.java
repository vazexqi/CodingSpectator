 ...
 * 
 * <li> {@link JavaCore#CODEASSIST_STATIC_FIELD_PREFIXES} : Define the Prefixes for Static Field
 * Name.</li>
 * <li> {@link JavaCore#CODEASSIST_STATIC_FIELD_SUFFIXES} : Define the Suffixes for Static Field
 * Name.</li>
 * 
 * <li> {@link JavaCore#CODEASSIST_STATIC_FINAL_FIELD_PREFIXES} : Define the Prefixes for Static
 * Final Field Name.</li>
 * <li> {@link JavaCore#CODEASSIST_STATIC_FINAL_FIELD_SUFFIXES} : Define the Suffixes for Static
 * Final Field Name.</li>
 * 
...
public final class NamingConventions {
	...
	/**
	 * The base name associated to this base name kind is a simple name. When this base name is used
	 * the whole name is considered.
	 * 
	...
	/**
	 * The base name associated to this base name kind is a simple type name. When this base name is
	 * used all the words of the name are considered.
	 * 
	 * @see #suggestVariableNames(int, int, String, IJavaProject, int, String[], boolean)
	 * 
	 * @since 3.5
	 ...
	 * <p>
	 * If argument name prefix is <code>pre</code> and argument name suffix is <code>suf</code> then
	 * for an argument named <code>preArgsuf</code> the result of this method is <code>arg</code>.
	 * If there is no prefix or suffix defined in JavaCore options the result is the unchanged name
	 * <code>preArgsuf</code>.
	 * </p>
	 * <p>
	 * This method is affected by the following JavaCore options :
	 * {@link JavaCore#CODEASSIST_ARGUMENT_PREFIXES} and
	 ...
	 * @see JavaCore#getDefaultOptions()
	 * 
	 * @deprecated Use {@link #getBaseName(int, String, IJavaProject)} instead with
	 *             {@link #VK_PARAMETER} as variable kind.
	 */
	 ...
	 * <p>
	 * If field name prefix is <code>pre</code> and field name suffix is <code>suf</code> then for a
	 * field named <code>preFieldsuf</code> the result of this method is <code>field</code>. If
	 * there is no prefix or suffix defined in JavaCore options the result is the unchanged name
	 * <code>preFieldsuf</code>.
	 * </p>
	 * <p>
	 * This method is affected by the following JavaCore options :
	 * {@link JavaCore#CODEASSIST_FIELD_PREFIXES} , {@link JavaCore#CODEASSIST_FIELD_SUFFIXES} for
	 * instance field and {@link JavaCore#CODEASSIST_STATIC_FIELD_PREFIXES},
	 ...
	 * @param modifiers field's modifiers as defined by the class <code>Flags</code>.
	 * @return char[] the name without prefix and suffix.
	 * @see Flags
	 * @see JavaCore#setOptions(java.util.Hashtable)
	 * @see JavaCore#getDefaultOptions()
	 * 
	 * @deprecated Use {@link #getBaseName(int, String, IJavaProject)} instead with
	 *             {@link #VK_INSTANCE_FIELD} or {@link #VK_STATIC_FIELD} as variable kind.
	 */
	 ...
	 * <p>
	 * If local variable name prefix is <code>pre</code> and local variable name suffix is
	 * <code>suf</code> then for a local variable named <code>preLocalsuf</code> the result of this
	 * method is <code>local</code>. If there is no prefix or suffix defined in JavaCore options the
	 * result is the unchanged name <code>preLocalsuf</code>.
	 * </p>
	 * <p>
	 * This method is affected by the following JavaCore options :
	 * {@link JavaCore#CODEASSIST_LOCAL_PREFIXES} and {@link JavaCore#CODEASSIST_LOCAL_SUFFIXES}.
	 * </p>
	 * <p>
	 * For a complete description of these configurable options, see <code>getDefaultOptions</code>.
	 * For programmaticaly change these options, see <code>JavaCore#setOptions()</code>.
	 ...
	 * 
	 * @deprecated Use {@link #getBaseName(int, String, IJavaProject)} instead with
	 *             {@link #VK_LOCAL} as variable kind.
	 */
	 ...
	 * <p>
	 * e.g.<br>
	 * If the variable is a {@link #VK_LOCAL} and the variable name is <code>variableName</code>
	 * then the base name will be <code>variableName</code>.<br>
	 * If the variable is a {@link #VK_STATIC_FINAL_FIELD} and the variable name is
	 * <code>VARIABLE_NAME</code> then the base name will be <code>variableName</code>.<br>
	 * </p>
	 * <p>
	 * Prefixes and suffixes defined in JavaCore options will be also removed from the variable
	 * name.<br>
	 ...
	 * <li>{@link #VK_STATIC_FINAL_FIELD}: {@link JavaCore#CODEASSIST_STATIC_FINAL_FIELD_PREFIXES}
	 * and {@link JavaCore#CODEASSIST_STATIC_FINAL_FIELD_SUFFIXES}</li>
	 * </ul>
	 * </p>
	 * <p>
	 * e.g.<br>
	 * If the variable is a {@link #VK_LOCAL}, the variable name is <code>preVariableNamesuf</code>,
	 * a possible prefix is <code>pre</code> and a possible suffix is <code>suf</code> then the base
	 * name will be <code>variableName</code>.<br>
	 * </p>
	 * 
	 * @param variableKind specifies what type the variable is: {@link #VK_LOCAL},
	 *            {@link #VK_PARAMETER}, {@link #VK_STATIC_FIELD}, {@link #VK_INSTANCE_FIELD} or
	 *            {@link #VK_STATIC_FINAL_FIELD}.
	 * @param variableName a variable name
	 * @param javaProject project which contains the variable or <code>null</code> to take into
	 *            account only workspace settings.
	 * 
	 * @see #suggestVariableNames(int, int, String, IJavaProject, int, String[], boolean)
	 * @since 3.5
	 */
	...
	/**
	 * Suggest names for an argument. The name is computed from argument's type and possible
	 * prefixes or suffixes are added.
	 * <p>
	 * If the type of the argument is <code>TypeName</code>, the prefix for argument is
	 * <code>pre</code> and the suffix for argument is <code>suf</code> then the proposed names are
	 * <code>preTypeNamesuf</code> and <code>preNamesuf</code>. If there is no prefix or suffix the
	 * proposals are <code>typeName</code> and <code>name</code>.
	 * </p>
	 * <p>
	 * This method is affected by the following JavaCore options :
	 * {@link JavaCore#CODEASSIST_ARGUMENT_PREFIXES} and
	 ...
	 * @param excludedNames a list of names which cannot be suggested (already used names). Can be
	 *            <code>null</code> if there is no excluded names.
	 ...
	 * @deprecated Use
	 *             {@link #suggestVariableNames(int, int, String, IJavaProject, int, String[], boolean)}
	 *             instead with {@link #VK_PARAMETER} as variable kind.
	 */
	...
	public static String[] suggestArgumentNames(IJavaProject javaProject, String packageName, String qualifiedTypeName, int dim, String[] excludedNames) {
		return convertCharsToString(suggestArgumentNames(
				javaProject,
	...
	/**
	 * Suggest names for a field. The name is computed from field's type and possible prefixes or
	 * suffixes are added.
	 * <p>
	 * If the type of the field is <code>TypeName</code>, the prefix for field is <code>pre</code>
	 * and the suffix for field is <code>suf</code> then the proposed names are
	 * <code>preTypeNamesuf</code> and <code>preNamesuf</code>. If there is no prefix or suffix the
	 * proposals are <code>typeName</code> and <code>name</code>.
	 * </p>
	 * <p>
	 * This method is affected by the following JavaCore options :
	 * {@link JavaCore#CODEASSIST_FIELD_PREFIXES}, {@link JavaCore#CODEASSIST_FIELD_SUFFIXES} and
	 * for instance field and {@link JavaCore#CODEASSIST_STATIC_FIELD_PREFIXES},
	 * {@link JavaCore#CODEASSIST_STATIC_FIELD_SUFFIXES} for static field.
	 * </p>
	 * <p>
	 * For a complete description of these configurable options, see <code>getDefaultOptions</code>.
	 ...
	 * @deprecated Use
	 *             {@link #suggestVariableNames(int, int, String, IJavaProject, int, String[], boolean)}
	 *             instead with {@link #VK_INSTANCE_FIELD} or {@link #VK_STATIC_FIELD} as variable
	 *             kind.
	 */
	...
	public static String[] suggestFieldNames(IJavaProject javaProject, String packageName, String qualifiedTypeName, int dim, int modifiers, String[] excludedNames) {
		return convertCharsToString(suggestFieldNames(
				javaProject,
				packageName.toCharArray(),
				qualifiedTypeName.toCharArray(),
				dim,
	...
	/**
	 * Suggest name for a getter method. The name is computed from field's name and possible
	 * prefixes or suffixes are removed.
	 * <p>
	 * If the field name is <code>preFieldNamesuf</code> and the prefix for field is
	 * <code>pre</code> and the suffix for field is <code>suf</code> then the prosposed name is
	 * <code>isFieldName</code> for boolean field or <code>getFieldName</code> for others. If there
	 * is no prefix and suffix the proposal is <code>isPreFieldNamesuf</code> for boolean field or
	...
	/**
	 * Suggest names for a local variable. The name is computed from variable's type and possible
	 * prefixes or suffixes are added.
	 * <p>
	 * If the type of the local variable is <code>TypeName</code>, the prefix for local variable is
	 * <code>pre</code> and the suffix for local variable is <code>suf</code> then the proposed
	 * names are <code>preTypeNamesuf</code> and <code>preNamesuf</code>. If there is no prefix or
	 * suffix the proposals are <code>typeName</code> and <code>name</code>.
	 * </p>
	 * <p>
	 * This method is affected by the following JavaCore options :
	 * {@link JavaCore#CODEASSIST_LOCAL_PREFIXES} and {@link JavaCore#CODEASSIST_LOCAL_SUFFIXES}.
	 ...
	 * @deprecated Use
	 *             {@link #suggestVariableNames(int, int, String, IJavaProject, int, String[], boolean)}
	 *             instead with {@link #VK_LOCAL} as variable kind.
	 */
	...
	public static String[] suggestLocalVariableNames(IJavaProject javaProject, String packageName, String qualifiedTypeName, int dim, String[] excludedNames) {
		return convertCharsToString(suggestLocalVariableNames(
				javaProject,
				packageName.toCharArray(),
				qualifiedTypeName.toCharArray(),
				dim,
	...
	/**
	 * Suggest name for a setter method. The name is computed from field's name and possible
	 * prefixes or suffixes are removed.
	 * <p>
	 * If the field name is <code>preFieldNamesuf</code> and the prefix for field is
	 * <code>pre</code> and the suffix for field is <code>suf</code> then the proposed name is
	 * <code>setFieldName</code>. If there is no prefix and suffix the proposal is
	...
	/**
	 * Suggests names for a variable. The name is computed from a base name and possible prefixes or
	 * suffixes are added.
	 * 
	 * <p>
	 * The base name is used to compute the variable name. Some different kinds of base names are
	 * possible and each kind is associated to a different heuristic to compute variable names.<br>
	 * The heuristic depends also on the kind of the variable. Each kind of variable is identified
	 * by a constant starting with <code>VK_</code>.<br>
	 * When a prefix and a suffix can be added then all combinations of prefix and suffix are
	 * suggested. If the name is <code>name</code>, the prefix is <code>pre</code> and the suffix is
	 * <code>suf</code> then the suggested names will be <code>prenamesuf</code>,
	 * <code>prename</code>, <code>namesuf</code> and <code>name</code>.<br>
	 * <br>
	 * The different kinds of base names are:
	 * <ul>
	 * <li>{@link #BK_NAME}: the base name is a Java name and the whole base name is considered to
	 * compute the variable names. A prefix and a suffix can be added.<br>
	 * There is a heuristic by variable kind.
	 * <ul>
	 * <li>{@link #VK_PARAMETER}, {@link #VK_LOCAL}, {@link #VK_INSTANCE_FIELD} and
	 * {@link #VK_STATIC_FIELD}:<br>
	 * In this case the first word will be converted to lower case and the other characters won't be
	 * changed.<br>
	 * If the base name is <code>SimpleName</code> then the suggested name will be
	 * <code>simpleName</code>.<br>
	 * </li>
	 * <li>{@link #VK_STATIC_FINAL_FIELD} :<br>
	 * In this case all letters of the name will be converted to upper case and words will be
	 * separated by an underscore (<code>"_"</code>).<br>
	 * If the base name is <code>SimpleName</code> then the suggested name will be
	 * <code>SIMPLE_NAME</code>.</li>
	 * </ul>
	 * </li>
	 * <li>{@link #BK_TYPE_NAME}: the base name is a Java simple type name (e.g.
	 * <code>HashMap</code>) and all the words of the base name are considered to compute the
	 * variable names. A prefix and a suffix can be added to these names.<br>
	 * There is a heuristic by variable kind.
	 * <ul>
	 * <li>{@link #VK_PARAMETER}, {@link #VK_LOCAL}, {@link #VK_INSTANCE_FIELD} and
	 * {@link #VK_STATIC_FIELD}:<br>
	 * In this case a variable name will contain some words of the base name and the first word will
	 * be converted to lower case.<br>
	 * If the type is <code>TypeName</code> then the suggested names will be <code>typeName</code>
	 * and <code>name</code>.</li>
	 * <li>{@link #VK_STATIC_FINAL_FIELD} :<br>
	 * In this case a variable name will contain some words of the base name, all letters of the
	 * name will be converted to upper case and segments will be separated by a underscore (
	 * <code>"_"</code>).<br>
	 * If the base name is <code>TypeName</code> then the suggested name will be
	 * <code>TYPE_NAME</code> and <code>NAME</code>.</li>
	 * </ul>
	 * </li>
	 * </ul>
	 ...
	 * {@link JavaCore#getDefaultOptions()}. To programmatically change these options, see
	 * {@link JavaCore#setOptions(java.util.Hashtable)} and
	 ...
	 *            {@link #VK_PARAMETER}, {@link #VK_STATIC_FIELD}, {@link #VK_INSTANCE_FIELD} or
	 *            {@link #VK_STATIC_FINAL_FIELD}.
	 * @param baseNameKind specifies what type the base name is: {@link #BK_NAME} or
	 *            {@link #BK_TYPE_NAME}
	 * @param baseName name used to compute the suggested names.
	 * @param javaProject project which contains the variable or <code>null</code> to take into
	 *            account only workspace settings.
	 * @param dim variable dimension (0 if the field is not an array).
	 * @param excluded a list of names which cannot be suggested (already used names). Can be
	 *            <code>null</code> if there are no excluded names.
	 * @param evaluateDefault if <code>true</code>, the result is guaranteed to contain at least one
	 *            result. If <code>false</code>, the result can be an empty array.
