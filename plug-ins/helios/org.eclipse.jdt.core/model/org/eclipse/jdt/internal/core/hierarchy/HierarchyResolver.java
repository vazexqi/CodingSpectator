 ...
 */
		...
		try {
			if (suppliedType.isBinaryType()) {
				BinaryTypeBinding binaryTypeBinding= this.lookupEnvironment
						.cacheBinaryType((IBinaryType)suppliedType, false/*don't need field and method (bug 125067)*/, null /*no access restriction*/);
	...
	/**
	 * Resolve the supertypes for the types contained in the given openables (ICompilationUnits
	 * and/or IClassFiles). Inform the requestor of the resolved supertypes for each supplied source
	 * type using: connect(ISourceType suppliedType, IGenericType superclass, IGenericType[]
	 * superinterfaces)
	 * 
	 * Also inform the requestor of the supertypes of each additional requested super type which is
			...
			for (int i= 0; i < openablesLength; i++) {
				...
				if (openable instanceof org.eclipse.jdt.core.ICompilationUnit) {
					...
					if (binaryType != null) {
						try {
							BinaryTypeBinding binaryTypeBinding= this.lookupEnvironment
									.cacheBinaryType(binaryType, false/*don't need field and method (bug 125067)*/, null /*no access restriction*/);
