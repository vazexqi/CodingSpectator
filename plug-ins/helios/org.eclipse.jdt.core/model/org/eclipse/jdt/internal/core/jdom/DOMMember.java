...
abstract class DOMMember extends DOMNode implements IDOMMember {
	...
	/**
	 * The modifier flags for this member that can be analyzed with org.eclipse.jdt.core.Flags
	 */
	protected int fFlags= 0;

	/**
	 * The member's comments when it has been altered from the contents in the document, otherwise
	 * <code>null</code>.
	 */
	protected String fComment= null;

	/**
	 * The original inclusive source range of the member's preceding comments in the document, or
	 * -1's if the member did not originally have a comment.
	 */
	protected int[] fCommentRange;


	/**
	 * The member's modifiers textual representation when the modifiers (flags) have been altered
	 * from their original contents, otherwise <code>null</code>.
	 */
	protected char[] fModifiers= null;

	/**
	 * The original inclusive source range of the member's modifiers in the document, or -1's if the
	 * member did not originally have modifiers in the source code (that is, package default
	 * visibility).
	 */
	 ...
	 *            might immediately follow the name.
	...
	/**
	 * Appends the contents of this node to the given CharArrayBuffer, using the original document
	 * and indicies as a form for the current attribute values of this node.
	 * 
	 * <p>
	 * To facilitate the implementation of generating contents for members, the content of members
	 * is split into three sections - the header, declaration, and body sections. The header section
	 * includes any preceding comments and modifiers. The declaration section includes the portion
	 * of the member declaration that follows any modifiers and precedes the member body. The body
	 * section includes the member body and any trailing whitespace.
	 * 
	...
	/**
	 * Appends this member's body contents to the given CharArrayBuffer. Body contents include the
	 * member body and any trailing whitespace.
	 */
	protected abstract void appendMemberBodyContents(CharArrayBuffer buffer);

	/**
	 * Appends this member's declaration contents to the given CharArrayBuffer. The declaration
	 * contents includes the portion of this member that appears after any modifiers and precedes
	 * the body.
	 */
	protected abstract void appendMemberDeclarationContents(CharArrayBuffer buffer);

	/**
	 * Appends this member's header contents to the given CharArrayBuffer. Header contents include
	 * any preceding comments and modifiers.
	 */
	...
	/**
	 * Appends the contents of this node to the given CharArrayBuffer, using the original document
	 * and indicies as a form for the current attribute values of this node. This method is called
	 * when this node is know not to have detailed source indexes.
	 */
	protected abstract void appendSimpleContents(CharArrayBuffer buffer);

	/**
	 * Returns a copy of the given array with the new element appended to the end of the array.
	 */
	...
	/**
	 * Returns a <code>String</code> describing the modifiers for this member, ending with
	 * whitespace (if not empty). This value serves as a replacement value for the member's modifier
	 * range when the modifiers have been altered from their original contents.
	 */
	...
	/**
	 * Returns the String to be used for this member's flags when generating contents - either the
	 * original contents in the document or the replacement value.
	 */
