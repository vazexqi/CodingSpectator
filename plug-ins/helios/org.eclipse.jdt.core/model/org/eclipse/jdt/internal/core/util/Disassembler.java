...
/**
 * Disassembler of .class files. It generates an output in the Writer that looks close to the javap
 * output.
 */
public class Disassembler extends ClassFileBytesDisassembler {
	...
	private void disassembleEnumConstructor(IClassFileReader classFileReader, char[] className, IMethodInfo methodInfo, StringBuffer buffer, String lineSeparator, int tabNumber, int mode) {
		...
		final char[] signature= Signature.toCharArray(methodDescriptor, returnClassName(className, '.', COMPACT), getParameterNames(methodDescriptor, codeAttribute, accessFlags),
	...
	/**
	 * Answers back the disassembled string of the IClassFileReader according to the mode. This is
	 * an output quite similar to the javap tool.
	 * 
	...
	private void disassembleAsModifier(IRuntimeInvisibleParameterAnnotationsAttribute runtimeInvisibleParameterAnnotationsAttribute, StringBuffer buffer, int index, String lineSeparator,
	...
	private void disassembleAsModifier(IRuntimeVisibleParameterAnnotationsAttribute runtimeVisibleParameterAnnotationsAttribute, StringBuffer buffer, int index, String lineSeparator, int tabNumber,
			int mode) {
		IParameterAnnotation[] parameterAnnotations= runtimeVisibleParameterAnnotationsAttribute.getParameterAnnotations();
		if (parameterAnnotations.length > index) {
			disassembleAsModifier(parameterAnnotations[index], buffer, lineSeparator, tabNumber + 1, mode);
