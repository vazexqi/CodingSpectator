...
/**
 * <p>
 * This abstract class implements behavior common to <code>CreateElementInCUOperations</code>. To
 * create a compilation unit, or an element contained in a compilation unit, the source code for the
 * entire compilation unit is updated and saved.
 * 
 * <p>
 * The element being created can be positioned relative to an existing element in the compilation
 * unit via the methods <code>#createAfter</code> and <code>#createBefore</code>. By default, the
 * new element is positioned as the last child of its parent element.
 * 
 */
public abstract class CreateElementInCUOperation extends JavaModelOperation {
	 ...
	 * <code>fInsertPosition</code>, or <code>null</code> if the newly created element will be
	 * positioned last.
	 */
	 ...
	 * duplicate element would request in this flag being set to <code>false</code>. Ensures that no
	 * deltas are generated when creation does not occur.
	 */
	protected boolean creationOccurred= true;

	/**
	 * Constructs an operation that creates a Java Language Element with the specified parent,
	...
	/**
	 * Instructs this operation to position the new element after the given sibling, or to add the
	...
	/**
	 * Sets the default position in which to create the new type member. Operations that require a
	...
	/**
	 * Sets the name of the <code>DOMNode</code> that will be used to create this new element. Used
	 * by the <code>CopyElementsOperation</code> for renaming. Only used for
	 ...
	 * the new element as the last child of its parent if <code>null</code>. The
	 * <code>position</code> must be one of the position constants.
	 */
	 ...
	 * <code>null</code>.
	 * <li>INVALID_NAME - no name, a name was null or not a valid import declaration name.
