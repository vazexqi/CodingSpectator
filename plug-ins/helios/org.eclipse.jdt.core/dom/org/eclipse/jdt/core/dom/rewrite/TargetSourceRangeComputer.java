...
public class TargetSourceRangeComputer {
	...
	public static final class SourceRange {
		 ...
		 * 
		 * @param startPosition the 0-based character index, or <code>-1</code> if no source
		 *            position information is known
		 * @param length the (possibly 0) length, or <code>0</code> if no source position
		 ...
		 * 
		 * @return the 0-based character index, or <code>-1</code> if no source position information
		 *         is known
		 */
	...
	/**
	 * Returns the target source range of the given node. Unlike {@link ASTNode#getStartPosition()}
	 * and {@link ASTNode#getLength()}, the extended source range may include comments and
	 * whitespace immediately before or after the normal source range for the node.
	 * <p>
	 ...
	 * {@link CompilationUnit#getExtendedLength(ASTNode)} to compute the target source range.
	 * Clients may override or extend this method to expand or contract the source range of the
	 * given node. The resulting source range must cover at least the original source range of the
	 * node.
	 * </p>
	 * 
	 * @param node the node with a known source range in the compilation unit being rewritten
	 * @return the exact source range in the compilation unit being rewritten that should be
	 *         replaced (or deleted)
	 */
