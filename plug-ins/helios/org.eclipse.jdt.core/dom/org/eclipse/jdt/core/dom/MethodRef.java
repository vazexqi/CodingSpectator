...
/**
 * AST node for a method or constructor reference within a doc comment ({@link Javadoc}). The
 * principal uses of these are in "@see" and "@link" tag elements, for references to method and
 * constructor members.
 * 
 * <pre>
 * MethodRef:
...
public class MethodRef extends ASTNode implements IDocElement {
	...
	/**
	 * The parameter declarations (element type: <code>MethodRefParameter</code>). Defaults to an
	 * empty list.
	 */
	...
	/**
	 * Creates a new AST node for a method reference owned by the given AST. By default, the method
	 * reference is for a method with an unspecified, but legal, name; no qualifier; and an empty
	 * parameter list.
	 * <p>
	 ...
	 * 
	 * @return the live list of method parameter references (element type:
