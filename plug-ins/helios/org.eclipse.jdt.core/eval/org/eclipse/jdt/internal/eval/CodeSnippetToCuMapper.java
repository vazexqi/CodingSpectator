...
/**
 * Maps back and forth a code snippet to a compilation unit. The structure of the compilation unit
 * is as follows:
 * 
 * <pre>
...
class CodeSnippetToCuMapper implements EvaluationConstants {
	 ...
	 */
	public CodeSnippetToCuMapper(char[] codeSnippet, char[] packageName, char[][] imports, char[] className, char[] varClassName, char[][] localVarNames, char[][] localVarTypeNames,
			int[] localVarModifiers, char[] declaringTypeName, String lineSeparator) {
	...
	/**
	 * Returns a completion requestor that wraps the given requestor and shift the results according
	 * to the start offset and line number offset of the code snippet in the generated compilation
	 * unit.
	 */
				...
				switch (proposal.getKind()) {
					case CompletionProposal.TYPE_REF:
						...
						if ((flags & Flags.AccEnum) == 0 &&
							...
							if (CharOperation.equals(packageName, CodeSnippetToCuMapper.this.snippetPackageName)
									...
									|| CharOperation.equals(className, CodeSnippetToCuMapper.this.snippetVarClassName)))
								return;

							if (CharOperation.equals(packageName, PACKAGE_NAME)
									&& CharOperation.equals(className, ROOT_CLASS_NAME))
								return;
						}
						break;
					...
					case CompletionProposal.METHOD_REF_WITH_CASTED_RECEIVER:
						...
						if (CharOperation.equals(declaringTypePackageName, PACKAGE_NAME)
								&& CharOperation.equals(declaringTypeName, ROOT_CLASS_NAME))
							return;
						break;
				}
	...
	/**
	 * Returns the type of evaluation that corresponds to the given line number in the generated
	 * compilation unit.
	 */
	...
	/**
	 * Returns a selection requestor that wraps the given requestor and shift the problems according
	 * to the start offset and line number offset of the code snippet in the generated compilation
	 * unit.
	 */
	public ISelectionRequestor getSelectionRequestor(final ISelectionRequestor originalRequestor) {
		return new ISelectionRequestor() {
			...
			public void acceptMethod(char[] declaringTypePackageName, char[] declaringTypeName, String enclosingDeclaringTypeSignature, char[] selector, char[][] parameterPackageNames,
					char[][] parameterTypeNames, String[] parameterSignatures, char[][] typeParameterNames, char[][][] typeParameterBoundNames, boolean isConstructor, boolean isDeclaration,
					char[] uniqueKey, int start, int end) {
				originalRequestor.acceptMethod(declaringTypePackageName, declaringTypeName, enclosingDeclaringTypeSignature, selector, parameterPackageNames, parameterTypeNames, parameterSignatures,
						typeParameterNames, typeParameterBoundNames, isConstructor, isDeclaration, uniqueKey, start, end);
			}
			...
			public void acceptMethodTypeParameter(char[] declaringTypePackageName, char[] declaringTypeName, char[] selector, int selectorStart, int selectorEnd, char[] typeParameterName,
