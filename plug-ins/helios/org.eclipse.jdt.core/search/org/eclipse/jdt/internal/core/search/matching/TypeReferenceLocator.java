 ...
 */
	...
	/**
	 * Reports the match of the given reference. Also provide a local and other elements to
			...
			if (!knownTypes.includes(type)) {
				if (isBinary) {
					...
					if (scope != null) {
						...
						this.match= new TypeDeclarationMatch(((JavaElement)type).resolved(typeBinding), SearchMatch.A_ACCURATE, offset, typeDecl.sourceEnd - offset + 1, locator.getParticipant(),
								resource);
						locator.report(this.match);
					}
				}
 ...
 */
	...
	/**
	 * Returns whether the given type binding or one of its enclosing types matches the given simple
	 * name pattern and qualification pattern. Returns ACCURATE_MATCH if it does. Returns
	 * INACCURATE_MATCH if resolve failed. Returns IMPOSSIBLE_MATCH if it doesn't.
	 */
