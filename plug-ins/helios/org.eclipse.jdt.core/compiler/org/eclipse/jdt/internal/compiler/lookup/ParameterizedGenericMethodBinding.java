...
/**
 * Binding denoting a generic method after type parameter substitutions got performed. On
 * parameterized type bindings, all methods got substituted, regardless whether their signature did
 * involve generics or not, so as to get the proper declaringClass for these methods.
 */
public class ParameterizedGenericMethodBinding extends ParameterizedMethodBinding implements Substitution {
	 ...
	 */
	private static ParameterizedGenericMethodBinding inferFromArgumentTypes(Scope scope, MethodBinding originalMethod, TypeBinding[] arguments, TypeBinding[] parameters,
...
//							}
		...
		if (considerEXTENDSConstraints && inferenceContext.hasUnresolvedTypeArgument()) {
			...
			nextTypeParameter: for (int i= 0; i < varLength; i++) {
				...
				if (glb != null)
					mostSpecificSubstitute= glb[0]; // TODO (philippe) need to improve
	 ...
	 * arguments, and erasure of method variables) Only invoked for non-static generic methods of
	 * raw type
	 */
	...
	/**
	 * Returns true if some parameters got substituted. NOTE: generic method invocation delegates to
	 * its declaring method (could be a parameterized one)
	 */
	...
	/**
	 * Returns true if the return type got substituted. NOTE: generic method invocation delegates to
	 * its declaring method (could be a parameterized one)
	 */
	...
	/**
	 * Given some type expectation, and type variable bounds, perform some inference. Returns true
	 * if still had unresolved type variable at the end of the operation
	 */
