 ...
 */
	...
	/**
	 * This method is responsible for adding a <clinit> method declaration to the type method
	 * collections. Note that this implementation is inserting it in first place (as VAJ or javac),
	 * and that this impacts the behavior of the method ConstantPool.resetForClinit(int. int), in so
	 * far as the latter will have to reset the constant pool state accordingly (if it was added
	 * first, it does not need to preserve some of the method specific cached entries since this
	 * will be the first method). inserts the clinit method declaration in the first position.
	 * 
...
/*		if (binding.methods == null) {
	...
	/**
	 * Check for constructor vs. method with no return type. Answers true if at least one
...
// anonymous type constructor creation: rank is important since bindings already got sorted
	...
	/**
	 * Finds the matching type amoung this type's member types. Returns null if no type with this
	 * name is found. The type name is a compound name relative to this type eg. if this type is X
	 * and we're looking for Y.X.A.B then a type name would be {X, A, B}
	 */
...
//				}
	...
	/**
	 * Access emulation for a local member type force to emulation of access to direct enclosing
	 * instance. By using the initializer scope, we actually only request an argument emulation, the
	 * field is not added until actually used. However we will force allocations to be qualified
	 * with an enclosing instance.
	 * 
	...
	/**
	 * A <clinit> will be requested as soon as static fields or assertions are present. It will be
	 * eliminated during classfile creation if no bytecode was actually produced based on some
	...
	/**
	 * MaxFieldCount's computation is necessary so as to reserve space for the flow info field
	 * portions. It corresponds to the maximum amount of fields this class or one of its innertypes
	 * have.
	 * 
	 * During name resolution, types are traversed, and the max field count is recorded on the
	 * outermost type. It is then propagated down during the flow analysis.
	 * 
