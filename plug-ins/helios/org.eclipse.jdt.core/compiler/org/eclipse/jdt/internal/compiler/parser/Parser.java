...
//		} else
	...
	protected void consumePrimaryNoNewArrayNameSuper() {
		...
		pushOnExpressionStack(new QualifiedSuperReference(
				typeReference,
				this.intStack[this.intPtr--],
				this.endPosition));
	}
	...
	protected void consumePrimaryNoNewArrayNameThis() {
		...
		pushOnExpressionStack(new QualifiedThisReference(
				typeReference,
				this.intStack[this.intPtr--],
				this.endPosition));
	}
...
// This method is part of an automatic generation : do NOT edit-modify  
	...
	protected void consumeStatementBreakWithLabel() {
		...
		pushOnAstStack(new BreakStatement(
				this.identifierStack[this.identifierPtr--],
				this.intStack[this.intPtr--],
	...
	protected void consumeStatementContinue() {
		...
		pushOnAstStack(new ContinueStatement(
				null,
				this.intStack[this.intPtr--],
				this.endStatementPosition));
	}
...
//	}
		...
		switch (type) {
			...
			case TokenNameLongLiteral:
				pushOnExpressionStack(new LongLiteral(
						this.scanner.getCurrentTokenSource(),
						this.scanner.startPosition,
						this.scanner.currentPosition - 1));
				break;
			case TokenNameFloatingPointLiteral:
				pushOnExpressionStack(new FloatLiteral(
						this.scanner.getCurrentTokenSource(),
						this.scanner.startPosition,
						this.scanner.currentPosition - 1));
				break;
			...
			case TokenNameCharacterLiteral:
				pushOnExpressionStack(new CharLiteral(
						this.scanner.getCurrentTokenSource(),
						this.scanner.startPosition,
						this.scanner.currentPosition - 1));
				break;
	...
	/**
	 * Given the current comment stack, answer whether some comment is available in a certain
	 * exclusive range
	 * 
 ...
 */
		...
		switch (nextCharacter) {
			case '\\':
				...
				if (!(((c1= ScannerHelper.getNumericValue(comment[index[0]++])) > 15
						...
						|| ((c3= ScannerHelper.getNumericValue(comment[index[0]++])) > 15 || c3 < 0) || ((c4= ScannerHelper.getNumericValue(comment[index[0]++])) > 15 || c4 < 0))) {
	...
	protected TypeReference getTypeReferenceForGenericType(int dim, int identifierLength, int numberOfIdentifiers) {
		if (identifierLength == 1 && numberOfIdentifiers == 1) {
			...
			ParameterizedSingleTypeReference parameterizedSingleTypeReference= new ParameterizedSingleTypeReference(this.identifierStack[this.identifierPtr], typeArguments, dim,
					this.identifierPositionStack[this.identifierPtr--]);
			if (dim != 0) {
	...
	/**
	 * Look for a specific tag comment leading a given source range (comment located after any
	 * statement in astStack)
	 * 
	 * @param rangeEnd int
	 * @return boolean
	 */
 ...
 */
	...
	/**
	 * Returns this parser's problem reporter initialized with its reference context. Also it is
	 * assumed that a problem is going to be reported, so initializes the compilation result's line
	 * positions.
	 * 
