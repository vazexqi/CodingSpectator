...
public abstract class Expression extends Statement {
	...
	/**
	 * More sophisticated for of the flow analysis used for analyzing expressions, and be able to
	 * optimize out portions of expressions where no actual value is required.
	 * 
	 * @param currentScope
	 * @param flowContext
	 * @param flowInfo
	...
	/**
	 * Check the local variable of this expression, if any, against potential NPEs given a flow
	 * context and an upstream flow info. If so, report the risk to the context. Marks the local as
	 * checked, which affects the flow info.
	 * 
	...
	/**
	 * Base types need that the widening is explicitly done by the compiler using some bytecode like
	 * i2f. Also check unsafe type operations.
	 */
...
//				}
	...
	/**
	 * Expression statements are plain expressions, however they generate like normal expressions
	 * with no value required.
	 * 
...
//(this request some work d be done by the VM on signed numbers)
	...
	/**
	 * Returns the local variable referenced by this node. Can be a direct reference
	 * (SingleNameReference) or thru a cast expression etc...
	 */
...
// this is no more called for now, waiting for inter procedural null reference analysis
	...
	/**
	 * Constant usable for bytecode pattern optimizations, but cannot be inlined since it is not
	 * strictly equivalent to the definition of constant expressions. In particular, some
	 * side-effects may be required to occur (only the end value is known).
	 * 
	...
	/**
	 * Returns the type of the expression after required implicit conversions. When expression type
	 * gets promoted or inserted a generic cast, the converted type will differ from the resolved
	 * type (surface side-effects from #computeConversion(...)).
	 * 
	...
	/**
	 * Returns an object which can be used to identify identical JSR sequence targets (see
	 * TryStatement subroutine codegen) or <code>null</null> if not reusable
	 */
	...
	/**
	 * Record the type expectation before this expression is typechecked. e.g. String s = foo();,
	 * foo() will be tagged as being expected of type String Used to trigger proper inference of
	 * generic method invocations.
	 * 
	 * @param expectedType The type denoting an expectation in the context of an assignment
	 *            conversion
	 */
