 ...
 */
	 ...
	 * wave into this flow info the consequences upon null information of a possible path into the
	 * operations that resulted into otherInits. The fact that this path may be left unexecuted
	 * under peculiar conditions results into less specific results than
	 * {@link #addInitializationsFrom(FlowInfo) addInitializationsFrom}; moreover, only the null
	...
	final public boolean cannotBeDefinitelyNullOrNonNull(LocalVariableBinding local) {
		...
		if ((position= local.id + this.maxFieldCount) < BitCacheSize) {
			// use bits
			return ((~this.nullBit1
					& (this.nullBit2 & this.nullBit3 | this.nullBit4) | ~this.nullBit2 & ~this.nullBit3 & this.nullBit4) & (1L << position)) != 0;
		}
		// use extra vector
		if (this.extra == null) {
		...
		return ((~this.extra[2][vectorIndex]
					& ((a2= this.extra[3][vectorIndex]) & (a3= this.extra[4][vectorIndex]) | (a4= this.extra[5][vectorIndex])) | ~a2 & ~a3 & a4) & (1L << (position % BitCacheSize))) != 0;
	}
	...
	final public boolean cannotBeNull(LocalVariableBinding local) {
		...
		if ((position= local.id + this.maxFieldCount) < BitCacheSize) {
			...
			return (this.nullBit1 & this.nullBit3
					& ((this.nullBit2 & this.nullBit4) | ~this.nullBit2) & (1L << position)) != 0;
		}
		// use extra vector
		if (this.extra == null) {
			return false; // if vector not yet allocated, then not initialized
		...
		return (this.extra[2][vectorIndex] & this.extra[4][vectorIndex]
				& ((this.extra[3][vectorIndex] & this.extra[5][vectorIndex]) |
					~this.extra[3][vectorIndex]) & (1L << (position % BitCacheSize))) != 0;
	}
	...
	/**
	 * Discard definite inits and potential inits from this, then return this. The returned flow
	 * info only holds null related information.
	 * 
	...
	/**
	 * Check status of definite assignment at a given position. It deals with the dual
	 * representation of the InitializationInfo2: bits for the first 64 entries, then an array of
	 * booleans.
	 */
	...
	final public boolean isDefinitelyUnknown(LocalVariableBinding local) {
		...
		return ((this.extra[2][vectorIndex] & this.extra[5][vectorIndex]
				& ~this.extra[3][vectorIndex] & ~this.extra[4][vectorIndex]) & (1L << (position % BitCacheSize))) != 0;
	}
	...
	final public boolean isPotentiallyUnknown(LocalVariableBinding local) {
		...
		if (position < BitCacheSize) { // use bits
			return (this.nullBit4
					& (~this.nullBit1 | ~this.nullBit2 & ~this.nullBit3) & (1L << position)) != 0;
		}
		// use extra vector
		if (this.extra == null) {
			return false; // if vector not yet allocated, then not initialized
		...
		if (this != DEAD_END) {
			...
			if ((position= local.id + this.maxFieldCount) < BitCacheSize) {
				...
				if (((mask= 1L << (position % BitCacheSize))
						...
						& ~(a3= this.extra[3 + 1][vectorIndex]) & (a4= this.extra[4 + 1][vectorIndex])) != 0) {
