...
public class KnownfilesRecorder {
	...
	boolean isFileKnown(IFile file) {
		return knownfiles.containsKey(RecorderHelper.getPortableFilePath(file));
	}

	void addKnownfile(IFile file) {
		knownfiles.setProperty(RecorderHelper.getPortableFilePath(file), String.valueOf(System.currentTimeMillis()));
	}

	Object removeKnownfile(IFile file) {
		return knownfiles.remove(RecorderHelper.getPortableFilePath(file));
	}
