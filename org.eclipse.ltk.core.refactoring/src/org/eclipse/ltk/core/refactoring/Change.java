...
public abstract class Change implements IAdaptable {
	 ...
	 * <p>
	 * Subclasses of changes created by {@link Refactoring#createChange(IProgressMonitor)} should
	 * override this method to return a {@link RefactoringChangeDescriptor}. A change tree created
	 * by a particular refactoring is supposed to contain at most one change which returns a
	 * refactoring descriptor. Refactorings usually return an instance of {@link CompositeChange} in
	 * their {@link Refactoring#createChange(IProgressMonitor)} method which implements this method.
	 * The refactoring framework searches the change tree top-down until a refactoring descriptor is
	 * found.
	 * </p>
	 * 
	...
	/**
	 * Returns the human readable name of this change. The name <em>MUST</em> not be
	 * <code>null</code>.
	 * 
	...
	/**
	 * Sets the parent of this change. Requires that this change isn't already connected to a
	 * parent. The parent can be <code>null</code> to disconnect this change from a parent.
	 * 
	 ...
	 * <code>isValid</code> method. This method gets called after a change or a whole change tree
	 * has been created.
	 * <p>
	 ...
	 * invalid if it receives a certain delta. Is this the case the implementor must take care of
	 * unhooking the listener in <code>dispose</code>.</li>
	 * <li>the change remembers some information allowing to decide if a change object is still
	 * valid when <code>isValid</code> is called.</li>
	 * </ul>
	 * <p>
	 * For example, a change object that manipulates the content of an <code>IFile</code> could
	 ...
	 * <code>perform</code>. If a refactoring status with a severity of
	 * {@link RefactoringStatus#FATAL} is returned then the change has to be treated as invalid and
	 * can no longer be executed. Performing such a change produces an unspecified result and will
	 * very likely throw an exception.
	 * <p>
	 * This method is also called by the {@link IUndoManager UndoManager} to decide if an undo or
	 ...
	 * 
	 * @throws CoreException if an error occurred during validation check. The change is to be
	 *             treated as invalid if an exception occurs
	 * 
	 ...
	 * result is unspecified. Changes should in general not respond to
	 * {@link IProgressMonitor#isCanceled()} since canceling a change tree in the middle of its
	 * execution leaves the workspace in a half changed state.
	 * 
	 * @param pm a progress monitor
	 * 
	 ...
	 * <p>
	 * This default implementation returns <code>null</code> to indicate that the affected elements
	 * are unknown. Subclasses should reimplement this method if they can compute the set of
	 * affected elements.
	 * </p>
	 * 
