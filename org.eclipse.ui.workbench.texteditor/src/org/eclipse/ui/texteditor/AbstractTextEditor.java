 ...
 * 
 */
public abstract class AbstractTextEditor extends EditorPart implements ITextEditor, IReusableEditor, ITextEditorExtension, ITextEditorExtension2, ITextEditorExtension3, ITextEditorExtension4,
	...
	class ElementStateListener implements IElementStateListener, IElementStateListenerExtension {
		...
		/**
		 * Internal <code>VerifyListener</code> for performing the state validation of the editor
		 * input in case of the first attempted manipulation via typing on the keyboard.
		 * 
		 * @since 2.0
		 */
		 ...
		 * <p>
		 * See https://bugs.eclipse.org/bugs/show_bug.cgi?id=76765 for details about why the
		 * parameter <code>postAsync</code> has been introduced in the course of 3.1.
		 * 
		 * @param runnable runnable to be executed
		 * @param postAsync <code>true</code> if the runnable must be posted asynchronous,
	...
	/**
	 * Internal text listener for updating all content dependent actions. The updating is done
	 * asynchronously.
	 */
	...
	/**
	 * Internal interface for a cursor listener. I.e. aggregation of mouse and key listener.
	 * 
	 * @since 2.0
	 */
	...
	class ScrollLinesAction extends Action {
		...
		/**
		 * Creates a new scroll action that scroll the given number of lines. If the increment is
		 * &lt; 0, it's scrolling up, if &gt; 0 it's scrolling down.
		 * 
	...
	/**
	 * Action to toggle the insert mode. The action is checked if smart mode is turned on.
	 * 
	 * @since 2.1
	 */
	...
	/**
	 * This action implements smart end. Instead of going to the end of a line it does the
	 * following: - if smart home/end is enabled and the caret is before the line's last
	 * non-whitespace and then the caret is moved directly after it - if the caret is after last
	 * non-whitespace the caret is moved at the end of the line - if the caret is at the end of the
	 * line the caret is moved directly after the line's last non-whitespace character
	 * 
	...
	protected class LineEndAction extends TextNavigationAction {
		 ...
		 * @param doSelect a boolean flag which tells if the text up to the line end should be
		 *            selected
		 */
	 ...
	 * non-whitespace then the caret is moved directly before it - if the caret is before the line's
	 * first non-whitespace the caret is moved to the beginning of the line - if the caret is at the
	 * beginning of the line the caret is moved directly before the line's first non-whitespace
	 * character
	 * 
	 * @since 2.1
	 */
	protected class LineStartAction extends TextNavigationAction {
		 ...
		 * 
		 * @param textWidget the styled text widget
		 * @param doSelect a boolean flag which tells if the text up to the beginning of the line
		 *            should be selected
		 */
	...
	protected static class ColumnSupport implements IColumnSupport {
		...
		/**
		 * Returns the currently visible column matching <code>id</code>, <code>null</code> if none.
		 * 
		...
		/**
		 * Computes the insertion index for a column contribution into the currently visible
		 * columns.
		 * 
	...
	/**
	 * This action behaves in two different ways: If there is no current text hover, the javadoc is
	 * displayed using information presenter. If there is a current text hover, it is converted into
	 * a information presenter in order to make it sticky.
	 * 
	 * @since 3.3
	 */
	...
	/**
	 * Key used to look up font preference. Value: <code>"org.eclipse.jface.textfont"</code>
	 * 
	 ...
	 * <code>SourceViewerConfiguration</code> to configure its source viewer. This viewer does not
	 * have a range indicator installed, nor any menu id set. By default, the created editor runs in
	 * 1.0 context menu registration compatibility mode.
	 */
	...
	/**
	 * Returns the editor's range indicator. May return <code>null</code> if no range indicator is
	 * installed.
	 * 
	...
	/**
	 * Returns the editor's source viewer configuration. May return <code>null</code> before the
	...
	/**
	 * Returns the editor's source viewer. May return <code>null</code> before the editor's part has
	 * been created and after disposal.
	 * 
	...
	/**
	 * Returns the editor's vertical ruler. May return <code>null</code> before the editor's part
	 * has been created and after disposal.
	 * 
	...
	/**
	 * Returns the editor's context menu id. May return <code>null</code> before the editor's part
	 * has been created.
	 * 
	...
	/**
	 * Returns the ruler's context menu id. May return <code>null</code> before the editor's part
	 * has been created.
	 * 
	...
	/**
	 * Returns the editor's help context id or <code>null</code> if none has been set.
	 * 
	...
	/**
	 * Returns this editor's preference store or <code>null</code> if none has been set.
	 * 
	...
	/**
	 * Sets this editor's document provider. This method must be called before the editor's control
	 * is created.
	 * 
	 ...
	 * This method must be called before the editor's control is created. If not, this editor uses a
	...
	/**
	 * Sets the annotation which this editor uses to represent the highlight range if the editor is
	 * configured to show the entire document. If the range indicator is not set, this editor will
	 * not show a range indication.
	 * 
	...
	/**
	 * Sets the context menu registration 1.0 compatibility mode. (See class description for more
	 * details.)
	 * 
	...
	/**
	 * Sets this editor's preference store. This method must be called before the editor's control
	 * is created.
	 * 
	 * @param store the preference store or <code>null</code> to remove the preference store
	 */
	 ...
	 * </p>
	 * 
	 * @return the selection provider or <code>null</code> if the editor has been disposed
	 */
	...
	/**
	 * Remembers the current selection of this editor. This method is called when, e.g., the content
	 * of the editor is about to be reverted to the saved state. This method remembers the selection
	 * in a semantic format, i.e., in a format which allows to restore the selection even if the
	 * originally selected text is no longer part of the editor's content.
	 * <p>
	 * Subclasses should implement this method including all necessary state. This default
	 * implementation remembers the textual range only and is thus purely syntactic.
	 * </p>
	 * 
	...
	/**
	 * Restores a selection previously remembered by <code>rememberSelection</code>. Subclasses may
	 * reimplement this method and thereby semantically adapt the remembered selection. This default
	 * implementation just selects the remembered textual range.
	 * 
	...
	/**
	 * Returns this editor's selection changed listener to be installed on the editor's source
	 * viewer.
	 * 
	 * @return the listener
	 */
	...
	/**
	 * Returns this editor's "cursor" listener to be installed on the editor's source viewer. This
	 * listener is listening to key and mouse button events. It triggers the updating of the status
	 * line by calling <code>handleCursorPositionChanged()</code>.
	 * 
	 * @return the listener
	 * @since 2.0
	 */
	...
	/**
	 * Implements the <code>init</code> method of <code>IEditorPart</code>. Subclasses replacing
	 * <code>init</code> may choose to call this method in their implementation.
	 * 
...
//			getSite().getWorkbenchWindow().getWorkbench().getProgressService().run(false, true, runnable);
	 ...
	 * Subclasses may re-implement this method using the {@link ColumnSupport}, e.g. by returning
	 * <code>new ColumnSupport(this, RulerColumnRegistry.getDefault());</code>.
	 * <p>
	 * <strong>Note:</strong> If you override this method to provide column support you will also
	 * need to override {@link #createVerticalRuler()} to return a {@link CompositeRuler}.
	 * </p>
	 * <p>
	 * Out of the box this class does not install this support and hence this implementation always
	...
	/**
	 * Initializes the drag and drop support for the given viewer based on provided editor adapter
	 * for drop target listeners.
	 * 
	 ...
	 * creates the vertical ruler and source viewer.
	 * <p>
	 * Subclasses may extend this method. Besides extending this method, the behavior of
	 * <code>createPartControl</code> may be customized by calling, extending or replacing the
	 * following methods: <br>
	 * Subclasses may supply customized implementations for some members using the following methods
	 * before <code>createPartControl</code> is invoked:
	 * <ul>
	 * <li>
	 * {@linkplain #setSourceViewerConfiguration(SourceViewerConfiguration)
	 * setSourceViewerConfiguration} to supply a custom source viewer configuration,</li>
	 * <li>{@linkplain #setRangeIndicator(Annotation) setRangeIndicator} to provide a range
	 * indicator,</li>
	 * <li>{@linkplain #setHelpContextId(String) setHelpContextId} to provide a help context id,</li>
	 * <li>{@linkplain #setEditorContextMenuId(String) setEditorContextMenuId} to set a custom
	 * context menu id,</li>
	 * <li>{@linkplain #setRulerContextMenuId(String) setRulerContextMenuId} to set a custom ruler
	 * context menu id.</li>
	 * </ul>
	 * <br>
	 ...
	 * <ul>
	 * <li>{@linkplain #createVerticalRuler() createVerticalRuler} to supply a custom vertical
	 * ruler,</li>
	 * <li>{@linkplain #createSourceViewer(Composite, IVerticalRuler, int) createSourceViewer} to
	 * supply a custom source viewer,</li>
	 * <li>{@linkplain #getSelectionProvider() getSelectionProvider} to supply a custom selection
	 * provider.</li>
	 * </ul>
	 * <br>
	 * Subclasses may extend the following methods called from within <code>createPartControl</code>:
	 * <ul>
	 * <li>
	 * {@linkplain #initializeViewerColors(ISourceViewer) initializeViewerColors} to customize the
	 * viewer color scheme (may also be replaced),</li>
	 * <li>
	 * {@linkplain #initializeDragAndDrop(ISourceViewer) initializeDragAndDrop} to customize drag
	 * and drop (may also be replaced),</li>
	 * <li>{@linkplain #createNavigationActions() createNavigationActions} to add navigation
	 * actions,</li>
	 * <li>{@linkplain #createActions() createActions} to add text editor actions.</li>
	 * </ul>
	 * </p>
	 * 
...
//						}
	...
	/**
	 * Tells whether the editor input should be included when adding object contributions to this
	...
	/**
	 * Initializes the fore- and background colors of the given viewer for both normal and selected
	 * text.
	 * 
	...
	/**
	 * Initializes the background color used for highlighting the document ranges defining search
	 * scopes.
	 * 
	 ...
	 * <p>
	 * <strong>Note:</strong> We use the editor's image instead of the image from the editor input
	 * to distinguish situations where the same editor input is opened in different kinds of
	 * editors.
	 * </p>
	 * 
	...
	/**
	 * Hook method for setting the document provider for the given input. This default
	 * implementation does nothing. Clients may reimplement.
	 * 
	...
	/**
	 * If there is no explicit document provider set, the implicit one is re-initialized based on
	...
	/**
	 * Called directly from <code>setInput</code> and from within a workspace runnable from
	 * <code>init</code>, this method does the actual setting of the editor input. Closes the editor
	 * if <code>input</code> is <code>null</code>. Disconnects from any previous editor input and
	 * its document provider and connects to the new one.
	 * <p>
	 ...
	 * <p>
	 * Note that many methods may return <code>null</code> after the editor is disposed.
	 * </p>
	 */
	public void dispose() {
	...
	/**
	 * Determines whether the given preference change affects the editor's presentation. This
	 * implementation always returns <code>false</code>. May be reimplemented by subclasses.
	 * 
	 * @param event the event which should be investigated
	 * @return <code>true</code> if the event describes a preference change affecting the editor's
	 *         presentation
	 * @since 2.0
	 */
	...
	/**
	 * Handles a property change event describing a change of the editor's preference store and
	...
	/**
	 * Returns the progress monitor related to this editor. It should not be necessary to extend
	 * this method.
	 * 
	...
	/**
	 * The <code>AbstractTextEditor</code> implementation of this <code>IEditorPart</code> method
	 * calls <code>performSaveAs</code>. Subclasses may reimplement.
	 */
	...
	/**
	 * Performs a save as and reports the result state back to the given progress monitor. This
	 * default implementation does nothing. Subclasses may reimplement.
	 * 
	 ...
	 * 
	 * @param enable <code>true</code> if sanity checking should be enabled, <code>false</code>
	 *            otherwise
	 * @since 2.0
	 */
	 ...
	 * 
	 * @param enable <code>true</code> if state validation should be enabled, <code>false</code>
	 *            otherwise
	 * @since 2.1
	 */
	...
	//CODINGSPECTATOR: Added calls to isInReplayMode() to prevent unnecessary dialog boxes. 
	/**
	...
	/**
	 * The <code>AbstractTextEditor</code> implementation of this <code>ITextEditor</code> method
	 * may be extended by subclasses.
	 */
	...
	/**
	 * Performs any additional action necessary to perform after the input document's content has
	 * been replaced.
	 * <p>
	 ...
	 * <p>
	 * XXX: This is called by the Java editor for its breadcrumb feature. We don't want to make this
	 * risky method API because the Java editor breadcrumb might become a Platform UI feature during
	 * 3.5 and hence we can then delete this workaround.
	 * </p>
	 * 
	...
	/**
	 * Return an {@link IOperationApprover} appropriate for approving the undo and redo of
	 * operations that have the specified undo context.
	 * <p>
	 * Subclasses may override.
	 * </p>
	 * 
	 * @param undoContext the IUndoContext of operations that should be examined by the operation
	 *            approver
	 * @return the <code>IOperationApprover</code> appropriate for approving undo and redo
	 *         operations inside this editor, or <code>null</code> if no approval is needed
	 * @since 3.1
	 */
	...
	/**
	 * Convenience method to add the action installed under the given action id to the specified
	 * group of the menu.
	 * 
	 * @param menu the menu to add the action to
	...
	/**
	 * Returns the set of legal insert modes. If insert modes are configured all defined insert
	 * modes are legal.
	 * 
	...
	/**
	 * Configures the given insert mode as legal or illegal. This call is ignored if the set of
	 * legal input modes would be empty after the call.
	 * 
	 ...
	 * 
	 * @param enable <code>true</code> to enable new overwrite mode, <code>false</code> to disable
	 * @since 3.0
	 */
	...
	/**
	 * Returns whether this editor can handle the move of the original element so that it ends up
	 * being the moved element. By default this method returns <code>true</code>. Subclasses may
	 * reimplement.
	 * 
	 * @param originalElement the original element
	 * @param movedElement the moved element
	 * @return whether this editor can handle the move of the original element so that it ends up
	 *         being the moved element
	 * @since 2.0
	 */
	...
	/**
	 * Returns the offset of the given source viewer's document that corresponds to the given widget
	 * offset or <code>-1</code> if there is no such offset.
	 * 
	...
	/**
	 * Returns the offset of the given source viewer's text widget that corresponds to the given
	 * model offset or <code>-1</code> if there is no such offset.
	 * 
	 * @param viewer the source viewer
	...
	/**
	 * Returns the minimal region of the given source viewer's document that completely comprises
	 * everything that is visible in the viewer's widget.
	 * 
	 * @param viewer the viewer go return the coverage for
	 * @return the minimal region of the source viewer's document comprising the contents of the
	 *         viewer's widget
	 * @since 2.1
	 */
	 ...
	 * 
	 * @param forward <code>true</code> if search direction is forward, <code>false</code> if
	 *            backward
	...
	/**
	 * Updates the source viewer's indent prefixes with the values provided by the source viewer
	 * configuration.
	 * 
	 * @since 3.3
	 */
