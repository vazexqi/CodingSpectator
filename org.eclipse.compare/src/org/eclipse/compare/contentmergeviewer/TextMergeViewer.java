...
/**
 * A text merge viewer uses the <code>RangeDifferencer</code> to perform a textual, line-by-line
 * comparison of two (or three) input documents. It is based on the <code>ContentMergeViewer</code>
 * and uses <code>TextViewer</code>s to implement the ancestor, left, and right content areas.
 * <p>
 * In the three-way compare case ranges of differing lines are highlighted and framed with different
 * colors to show whether the difference is an incoming, outgoing, or conflicting change. The
 * <code>TextMergeViewer</code> supports the notion of a current "differing range" and provides
 * toolbar buttons to navigate from one range to the next (or previous).
 * <p>
 * If there is a current "differing range" and the underlying document is editable the
 * <code>TextMergeViewer</code> enables actions in context menu and toolbar to copy a range from one
 * side to the other side, thereby performing a merge operation.
 * <p>
 * In addition to a line-by-line comparison the <code>TextMergeViewer</code> uses a token based
 * compare on differing lines. The token compare is activated when navigating into a range of
 * differing lines. At first the lines are selected as a block. When navigating into this block the
 * token compare shows for every line the differing token by selecting them.
 * <p>
 * The <code>TextMergeViewer</code>'s default token compare works on characters separated by
 * whitespace. If a different strategy is needed (for example, Java tokens in a Java-aware merge
 * viewer), clients can create their own token comparators by implementing the
 * <code>ITokenComparator</code> interface and overriding the
 ...
 * <code>IMergeViewerContentProvider</code>. Its <code>get<it>X</it></code>Content</code> methods
 * must return either an <code>IDocument</code>, an <code>IDocumentRange</code>, or an
 * <code>IStreamContentAccessor</code>. In the <code>IDocumentRange</code> case the
 * <code>TextMergeViewer</code> works on a subrange of a document. In the
 * <code>IStreamContentAccessor</code> case a document is created internally and initialized from
 * the stream.
 * <p>
 * A <code>TextMergeViewer</code> can be used as is. However clients may subclass to customize the
 * behavior. For example a <code>MergeTextViewer</code> for Java would override the
 * <code>configureTextViewer</code> method to configure the <code>TextViewer</code> for Java source
 * code, the <code>createTokenComparator</code> method to create a Java specific tokenizer.
 * <p>
 * In 3.5 a new API has been introduced to let clients provide their own source viewers
 * implementation with an option to configure them basing on a corresponding editor input.
 * 
...
public class TextMergeViewer extends ContentMergeViewer implements IAdaptable {
	...
	class ContributorInfo implements IElementStateListener, VerifyListener, IDocumentListener, IEncodingSupport {
		...
		/**
		 * Connect to a shared document if possible. Return <code>null</code> if the connection was
		 * not possible.
		 * 
		 * @return the shared document or <code>null</code> if connection to a shared document was
		 *         not possible
		 */
		...
		/**
		 * Return the document key used to obtain a shared document. A <code>null</code> is returned
		...
		boolean hasSharedDocument(Object object) {
			return (fElement == object &&
					fDocumentProvider != null && fDocumentProvider.getDocument(getDocumentKey()) != null);
		}
	...
	/**
	 * Sets the viewer's background color to the given RGB value. If the value is <code>null</code>
	 * the system's default background color is used.
	 * 
	 * @param background the background color or <code>null</code> to use the system's default
	 *            background color
	 * @since 2.0
	 */
	...
	/**
	 * Sets the viewer's foreground color to the given RGB value. If the value is <code>null</code>
	 * the system's default foreground color is used.
	 * 
	 * @param foreground the foreground color or <code>null</code> to use the system's default
	 *            foreground color
	 * @since 2.0
	 */
	...
	/**
	 * Configures the passed text viewer. This method is called after the three text viewers have
	 * been created for the content areas. The <code>TextMergeViewer</code> implementation of this
	 * method will configure the viewer with a {@link SourceViewerConfiguration}. Subclasses may
	...
	/**
	 * Creates an <code>ITokenComparator</code> which is used to show the intra line differences.
	 * The <code>TextMergeViewer</code> implementation of this method returns a tokenizer that
	 * breaks a line into words separated by whitespace. Subclasses may reimplement to provide a
	 * specific tokenizer.
	 * 
	...
	/**
	 * Setup the given document for use with this viewer. By default, the partitioner returned from
	 * {@link #getDocumentPartitioner()} is registered as the default partitioner for the document.
	 * Subclasses that return a partitioner must also override {@link #getDocumentPartitioning()} if
	 * they wish to be able to use shared documents (i.e. file buffers).
	 * 
	...
	/**
	 * Returns a document partitioner which is suitable for the underlying content type. This method
	 * is only called if the input provided by the content provider is a
	 * <code>IStreamContentAccessor</code> and an internal document must be obtained. This document
	 * is initialized with the partitioner returned from this method.
	 * <p>
	 * The <code>TextMergeViewer</code> implementation of this method returns <code>null</code>.
	 * Subclasses may reimplement to create a partitioner for a specific content type. Subclasses
	 * that do return a partitioner should also return a partitioning from
	 * {@link #getDocumentPartitioning()} in order to make use of shared documents (e.g. file
	 * buffers).
	 * 
	 ...
	 * {@link #getDocumentPartitioner()} is to be associated. Return <code>null</code> only if
	 * partitioning is not needed or if the subclass overrode {@link #setupDocument(IDocument)}
	 * directly. By default, <code>null</code> is returned which means that shared documents that
	 * return a partitioner from {@link #getDocumentPartitioner()} will not be able to use shared
	 * documents.
	 * 
	...
	/**
	 * Creates a new source viewer. This method is called when creating and initializing the content
	 * areas of the merge viewer (see {@link #createControls(Composite)}). It is called three times
	 * for each text part of the comparison: ancestor, left, right. Clients may implement to provide
	 * their own type of source viewers. The viewer is not expected to be configured with a source
	 * viewer configuration.
	 * 
	 * @param parent the parent of the viewer's control
	 * @param textOrientation style constant bit for text orientation
	 * @return Default implementation returns an instance of {@link SourceViewer}.
	 * @since 3.5
	 */
	...
	/**
	 * Returns the contents of the underlying document as an array of bytes using the current
	 * workbench encoding.
	 * 
	 * @param left if <code>true</code> the contents of the left side is returned; otherwise the
	 *            right side
	...
	/**
	 * Perform a two level 2- or 3-way diff. The first level is based on line comparison, the second
	 * level on token comparison.
	 */
	private void doDiff() {
...
//			return;
	...
	/**
	 * This method returns {@link ITextEditor} used in the {@link ChangeEncodingAction}. It provides
	 * implementation of methods that are used by the action by delegating them to
	 * {@link ContributorInfo} that corresponds to the side that has focus.
	 * 
