--This file is licensed under the University of Illinois/NCSA Open Source
--License. See LICENSE.TXT for details.

\c false

* VARCHAR_SIZE = 100

* EXPERT_MIN_ALL_REFACTORINGS = 50

* EXPERT_MIN_DISTINCT_REFACTORINGS = 5

DROP TABLE "PUBLIC"."NUMERIC_RESULTS" IF EXISTS;

CREATE TABLE "PUBLIC"."NUMERIC_RESULTS" (

  "NAME" VARCHAR(*{VARCHAR_SIZE}),

  "VALUE" INT

);

DROP TABLE "PUBLIC"."UDC_EVENTS" IF EXISTS;

CREATE TABLE "PUBLIC"."UDC_EVENTS" (

  "USERID" INT,

  "DESCRIPTION" VARCHAR(*{VARCHAR_SIZE}),

  "TIME" BIGINT

);

* *DSV_COL_SPLITTER = ,

* *DSV_TARGET_TABLE = "PUBLIC"."UDC_EVENTS"

\mq udc-refactoring-data-projected.csv

\p Imported the refactoring and undo events from the UDC data set.

INSERT INTO "PUBLIC"."NUMERIC_RESULTS" ("NAME", "VALUE")

VALUES (

'The number of users that have performed a refactoring or undo operation', 

(SELECT COUNT(DISTINCT("T"."USERID")) FROM

"PUBLIC"."UDC_EVENTS" "T")

);

INSERT INTO "PUBLIC"."NUMERIC_RESULTS" ("NAME", "VALUE")

VALUES (

'The total number of refactoring and undo operations', 

(SELECT COUNT(*) FROM "PUBLIC"."UDC_EVENTS")

);

* *DSV_COL_DELIM=,

* *DSV_ROW_DELIM=\n

* *DSV_TARGET_FILE=refactoring-frequencies.csv

DROP TABLE "PUBLIC"."UDC_REFACTORING_FREQUENCIES" IF EXISTS;

CREATE TABLE "PUBLIC"."UDC_REFACTORING_FREQUENCIES" (

  "REFACTORING" VARCHAR(*{VARCHAR_SIZE}),

  "FREQUENCY" INT,

  "USERS" INT

);

INSERT INTO "PUBLIC"."UDC_REFACTORING_FREQUENCIES" (

  "REFACTORING",

  "FREQUENCY",

  "USERS"
)

SELECT

"T"."DESCRIPTION" AS "REFACTORING",

COUNT(*) AS "FREQUENCY",

COUNT(DISTINCT("T"."USERID")) AS "USERS"

FROM "PUBLIC"."UDC_EVENTS" "T"

GROUP BY "T"."DESCRIPTION"

ORDER BY "FREQUENCY" DESC, "USERS" DESC;

\x SELECT * FROM "PUBLIC"."UDC_REFACTORING_FREQUENCIES";

\p Reported the frequency and number of users of each refactoring in *{*DSV_TARGET_FILE}.

INSERT INTO "PUBLIC"."NUMERIC_RESULTS" ("NAME", "VALUE")

VALUES (

'The total number of refactoring events', 

(SELECT SUM("T"."FREQUENCY") FROM

"PUBLIC"."UDC_REFACTORING_FREQUENCIES" "T"

WHERE "T"."REFACTORING" <> 'undo')

);

DROP TABLE "PUBLIC"."UDC_REFACTORING_FREQUENCIES_BY_USER" IF EXISTS;

CREATE TABLE "PUBLIC"."UDC_REFACTORING_FREQUENCIES_BY_USER" (

  "USERID" INT,

  "INTRODUCE_FACTORY" INT,

  "CHANGE_TYPE" INT,

  "CONVERT_ANONYMOUS_TO_NESTED" INT,

  "EXTRACT_CLASS" INT,

  "EXTRACT_CONSTANT" INT,

  "EXTRACT_INTERFACE" INT,

  "EXTRACT_LOCAL_VARIABLE" INT,

  "EXTRACT_METHOD" INT,

  "EXTRACT_SUPERCLASS" INT,

  "INFER_TYPE_ARGUMENTS" INT,

  "INLINE" INT,

  "INTRODUCE_INDIRECTION" INT,

  "INTRODUCE_PARAMETER" INT,

  "INTRODUCE_PARAMETER_OBJECT" INT,

  "MODIFY_METHOD_PARAMETERS" INT,

  "MOVE_ELEMENT" INT,

  "MOVE_INNER_TO_TOP_LEVEL" INT,

  "PROMOTE_LOCAL_VARIABLE" INT,

  "PULL_UP" INT,

  "PUSH_DOWN" INT,

  "RENAME_ELEMENT" INT,

  "SELF_ENCAPSULATE_FIELD" INT,

  "USE_SUPERTYPE" INT,

  "ALL_REFACTORINGS" INT,

  "DISTINCT_REFACTORINGS" INT

);

INSERT INTO "PUBLIC"."UDC_REFACTORING_FREQUENCIES_BY_USER" (

  "USERID",

  "INTRODUCE_FACTORY",

  "CHANGE_TYPE",

  "CONVERT_ANONYMOUS_TO_NESTED",

  "EXTRACT_CLASS",

  "EXTRACT_CONSTANT",

  "EXTRACT_INTERFACE",

  "EXTRACT_LOCAL_VARIABLE",

  "EXTRACT_METHOD",

  "EXTRACT_SUPERCLASS",

  "INFER_TYPE_ARGUMENTS",

  "INLINE",

  "INTRODUCE_INDIRECTION",

  "INTRODUCE_PARAMETER",

  "INTRODUCE_PARAMETER_OBJECT",

  "MODIFY_METHOD_PARAMETERS",

  "MOVE_ELEMENT",

  "MOVE_INNER_TO_TOP_LEVEL",

  "PROMOTE_LOCAL_VARIABLE",

  "PULL_UP",

  "PUSH_DOWN",

  "RENAME_ELEMENT",

  "SELF_ENCAPSULATE_FIELD",

  "USE_SUPERTYPE",

  "ALL_REFACTORINGS",

  "DISTINCT_REFACTORINGS"

)

SELECT

"T"."USERID" AS "USERID",

COUNT(NULLIF("T"."DESCRIPTION" = 'introduce.factory', FALSE)) AS "INTRODUCE_FACTORY",

COUNT(NULLIF("T"."DESCRIPTION" = 'change.type', FALSE)) AS "CHANGE_TYPE",

COUNT(NULLIF("T"."DESCRIPTION" = 'convert.anonymous.to.nested', FALSE)) AS "CONVERT_ANONYMOUS_TO_NESTED",

COUNT(NULLIF("T"."DESCRIPTION" = 'extract.class', FALSE)) AS "EXTRACT_CLASS",

COUNT(NULLIF("T"."DESCRIPTION" = 'extract.constant', FALSE)) AS "EXTRACT_CONSTANT",

COUNT(NULLIF("T"."DESCRIPTION" = 'extract.interface', FALSE)) AS "EXTRACT_INTERFACE",

COUNT(NULLIF("T"."DESCRIPTION" = 'extract.local.variable', FALSE)) AS "EXTRACT_LOCAL_VARIABLE",

COUNT(NULLIF("T"."DESCRIPTION" = 'extract.method', FALSE)) AS "EXTRACT_METHOD",

COUNT(NULLIF("T"."DESCRIPTION" = 'extract.superclass', FALSE)) AS "EXTRACT_SUPERCLASS",

COUNT(NULLIF("T"."DESCRIPTION" = 'infer.type.arguments', FALSE)) AS "INFER_TYPE_ARGUMENTS",

COUNT(NULLIF("T"."DESCRIPTION" = 'inline', FALSE)) AS "INLINE",

COUNT(NULLIF("T"."DESCRIPTION" = 'introduce.indirection', FALSE)) AS "INTRODUCE_INDIRECTION",

COUNT(NULLIF("T"."DESCRIPTION" = 'introduce.parameter', FALSE)) AS "INTRODUCE_PARAMETER",

COUNT(NULLIF("T"."DESCRIPTION" = 'introduce.parameter.object', FALSE)) AS "INTRODUCE_PARAMETER_OBJECT",

COUNT(NULLIF("T"."DESCRIPTION" = 'modify.method.parameters', FALSE)) AS "MODIFY_METHOD_PARAMETERS",

COUNT(NULLIF("T"."DESCRIPTION" = 'move.element', FALSE)) AS "MOVE_ELEMENT",

COUNT(NULLIF("T"."DESCRIPTION" = 'move.inner.to.top.level', FALSE)) AS "MOVE_INNER_TO_TOP_LEVEL",

COUNT(NULLIF("T"."DESCRIPTION" = 'promote.local.variable', FALSE)) AS "PROMOTE_LOCAL_VARIABLE",

COUNT(NULLIF("T"."DESCRIPTION" = 'pull.up', FALSE)) AS "PULL_UP",

COUNT(NULLIF("T"."DESCRIPTION" = 'push.down', FALSE)) AS "PUSH_DOWN",

COUNT(NULLIF("T"."DESCRIPTION" = 'rename.element', FALSE)) AS "RENAME_ELEMENT",

COUNT(NULLIF("T"."DESCRIPTION" = 'self.encapsulate.field', FALSE)) AS "SELF_ENCAPSULATE_FIELD",

COUNT(NULLIF("T"."DESCRIPTION" = 'use.supertype', FALSE)) AS "USE_SUPERTYPE",

COUNT("T"."DESCRIPTION") AS "ALL_REFACTORINGS",

COUNT(DISTINCT "T"."DESCRIPTION") AS "DISTINCT_REFACTORINGS"

FROM "PUBLIC"."UDC_EVENTS" "T"

WHERE "T"."DESCRIPTION" <> 'undo'

GROUP BY "T"."USERID"

ORDER BY "T"."USERID";

* *DSV_COL_DELIM=,

* *DSV_ROW_DELIM=\n

* *DSV_TARGET_FILE=user-refactoring-frequencies.csv

\x SELECT * FROM "PUBLIC"."UDC_REFACTORING_FREQUENCIES_BY_USER";

\p Reported the frequencies of the refactorings of each user in *{*DSV_TARGET_FILE}.

DROP TABLE "PUBLIC"."USERS_DISTINCT_REFACTORINGS" IF EXISTS;

CREATE TABLE "PUBLIC"."USERS_DISTINCT_REFACTORINGS" (

  "DISTINCT_REFACTORINGS" INT,

  "USERS" INT

);

INSERT INTO "PUBLIC"."USERS_DISTINCT_REFACTORINGS" (

  "DISTINCT_REFACTORINGS",

  "USERS"

)

SELECT

"T"."DISTINCT_REFACTORINGS" AS "DISTINCT_REFACTORINGS",

COUNT(*) AS "USERS"

FROM "PUBLIC"."UDC_REFACTORING_FREQUENCIES_BY_USER" "T"

GROUP BY "T"."DISTINCT_REFACTORINGS"

ORDER BY "T"."DISTINCT_REFACTORINGS" ASC;

INSERT INTO "PUBLIC"."USERS_DISTINCT_REFACTORINGS" ("DISTINCT_REFACTORINGS",
"USERS") VALUES (17, 0), (18, 0), (20, 0), (22, 0), (23, 0);

* *DSV_COL_DELIM=,

* *DSV_ROW_DELIM=\n

* *DSV_TARGET_FILE=users-distinct-refactorings.csv

\x SELECT * FROM "PUBLIC"."USERS_DISTINCT_REFACTORINGS" "T" ORDER BY "T"."DISTINCT_REFACTORINGS" ASC;

\p Reported the number of users with the same number of distinct refactorings in *{*DSV_TARGET_FILE}.

DROP TABLE "PUBLIC"."USERS_ALL_REFACTORINGS" IF EXISTS;

CREATE TABLE "PUBLIC"."USERS_ALL_REFACTORINGS" (

  "REFACTORINGS" INT,

  "USERS" INT

);

INSERT INTO "PUBLIC"."USERS_ALL_REFACTORINGS" (

  "REFACTORINGS",

  "USERS"

)

SELECT

"T"."ALL_REFACTORINGS" AS "REFACTORINGS",

COUNT("T"."USERID") AS "USERS"

FROM "PUBLIC"."UDC_REFACTORING_FREQUENCIES_BY_USER" "T"

GROUP BY "T"."ALL_REFACTORINGS"

ORDER BY "T"."ALL_REFACTORINGS" ASC;

* *DSV_COL_DELIM=,

* *DSV_ROW_DELIM=\n

* *DSV_TARGET_FILE=users-all-refactorings.csv

\x SELECT * FROM "PUBLIC"."USERS_ALL_REFACTORINGS";

\p Reported the number of users invoking the same number of refactorings in *{*DSV_TARGET_FILE}.

DROP TABLE "PUBLIC"."EXPERT_USERS" IF EXISTS;

CREATE TABLE "PUBLIC"."EXPERT_USERS" (

  "USERID" INT

);

INSERT INTO "PUBLIC"."EXPERT_USERS" (

  "USERID"

)

SELECT

"T"."USERID" AS "USERID"

FROM "PUBLIC"."UDC_REFACTORING_FREQUENCIES_BY_USER" "T"

WHERE "T"."ALL_REFACTORINGS" >= *{EXPERT_MIN_ALL_REFACTORINGS} AND
"T"."DISTINCT_REFACTORINGS" >= *{EXPERT_MIN_DISTINCT_REFACTORINGS};

\p Creating an index on ALL_DATA

DROP INDEX "EXPERT_USERS_INDEX" IF EXISTS;

CREATE INDEX "EXPERT_USERS_INDEX" ON "PUBLIC"."EXPERT_USERS" (

  "USERID"

);

INSERT INTO "PUBLIC"."NUMERIC_RESULTS" ("NAME", "VALUE")

VALUES (

'Number of expert users', 

(SELECT COUNT(*) FROM "PUBLIC"."EXPERT_USERS" "T")

);

\p Identified the expert users of refactorings.

DROP TABLE "PUBLIC"."UDC_EVENTS_OF_EXPERTS" IF EXISTS;

CREATE TABLE "PUBLIC"."UDC_EVENTS_OF_EXPERTS" (

  "USERID" INT,

  "DESCRIPTION" VARCHAR(*{VARCHAR_SIZE}),

  "TIME" BIGINT

);

INSERT INTO "PUBLIC"."UDC_EVENTS_OF_EXPERTS" (

  "USERID",

  "DESCRIPTION",

  "TIME"

)

SELECT

"T"."USERID" AS "USERID",

"T"."DESCRIPTION" AS "DESCRIPTION",

"T"."TIME" AS "TIME"

FROM "PUBLIC"."UDC_EVENTS" "T"

WHERE "T"."USERID" IN (SELECT "T2"."USERID" FROM "PUBLIC"."EXPERT_USERS" "T2");

* *DSV_COL_DELIM=,

* *DSV_ROW_DELIM=\n

* *DSV_TARGET_FILE=udc-events-of-experts.csv

\x SELECT * FROM "PUBLIC"."UDC_EVENTS_OF_EXPERTS";

\p Reported the UDC events of expert users in *{*DSV_TARGET_FILE}.

INSERT INTO "PUBLIC"."NUMERIC_RESULTS" ("NAME", "VALUE")

VALUES (

'Number of events of expert users', 

(SELECT COUNT(*) FROM "PUBLIC"."UDC_EVENTS_OF_EXPERTS" "T")

);

* *DSV_COL_DELIM=,

* *DSV_ROW_DELIM=\n

* *DSV_TARGET_FILE=numeric-results.csv

INSERT INTO "PUBLIC"."NUMERIC_RESULTS" ("NAME", "VALUE")

VALUES (

'The number of users that have performed a refactoring', 

(SELECT SUM("T"."USERS") FROM

"PUBLIC"."USERS_DISTINCT_REFACTORINGS" "T"

WHERE "T"."DISTINCT_REFACTORINGS" > 0)

);

\x SELECT * FROM "PUBLIC"."NUMERIC_RESULTS";

