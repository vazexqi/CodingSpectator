 ...
 * <li>A project can be open or closed; a closed project is passive and has a minimal in-memory
 * footprint.</li>
 ...
 * </p>
 * <p>
 * Projects implement the <code>IAdaptable</code> interface; extensions are managed by the
...
public interface IProject extends IContainer, IAdaptable {
	/**
	 * Option constant (value 1) indicating that a snapshot to be loaded or saved contains a
	 * resource tree (refresh information).
	 * 
	...
	/**
	 * Invokes the <code>build</code> method of the specified builder for this project. Does nothing
	 * if this project is closed. If this project has multiple builders on its build spec matching
	 * the given name, only the first matching builder will be run.
	 * <p>
	 * The builder name is declared in the extension that plugs in to the standard
	 * <code>org.eclipse.core.resources.builders</code> extension point. The arguments are builder
	 * specific.
	 * </p>
	 * <p>
	 ...
	 *            <li> <code>IncrementalProjectBuilder.INCREMENTAL_BUILD</code> - indicates an
	 *            incremental build.</li>
	 *            <li><code>CLEAN_BUILD</code>- indicates a clean request. Clean does not actually
	 *            build anything, but rather discards all problems and build states.
	 *            </ul>
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
	 * @exception CoreException if the build fails. The status contained in the exception may be a
	 *                generic <code>BUILD_FAILED</code> code, but it could also be any other status
	 *                code; it might also be a multi-status.
	 * @exception OperationCanceledException if the operation is canceled. Cancelation can occur
	 *                even if no progress monitor is provided.
	 * 
	 * @see IProjectDescription
	 ...
	 * <p>
	 * Closing a project involves ensuring that all important project-related state is safely stored
	 * on disk, and then discarding the in-memory representation of its resources and other volatile
	 * state, including session properties. After this method, the project continues to exist in the
	 * workspace but its member resources (and their members, etc.) do not. A closed project can
	 * later be re-opened.
	 * </p>
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event that includes an indication that this project has been closed and its members have been
	 * removed.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 ...
	 * description. If there is an existing project description file, it is not overwritten. In
	 * either case, this method does not cause natures to be configured.
	 * </p>
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that the project has been added to the workspace.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	 ...
	 *                <li>The project description file could not be created in the project content
	 *                area.</li>
	 *                <li>Resource changes are disallowed during certain types of resource change
	 *                event notification. See <code>IResourceChangeEvent</code> for more details.</li>
	 *                </ul>
	...
	/**
	 * Creates a new project resource in the workspace with files in the default location in the
	 * local file system. Upon successful completion, the project will exist but be closed.
	 * <p>
	 * Newly created projects have no session or persistent properties.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that this project has been added to the workspace.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	...
	/**
	 * Returns the specified project nature for this project or <code>null</code> if the project
	 ...
	 * </p>
	 * 
	 * @param natureId the fully qualified nature extension identifier, formed by combining the
	 *            nature extension id with the id of the declaring plug-in. (e.g.
	 *            <code>"com.example.acmeplugin.coolnature"</code>)
	...
	/**
	 * Returns the location in the local file system of the project-specific working data area for
	 * use by the given plug-in or <code>null</code> if the project does not exist.
	 * <p>
	 * The content, structure, and management of this area is the responsibility of the plug-in.
	...
	/**
	 * Returns the location in the local file system of the project-specific working data area for
	 * use by the bundle/plug-in with the given identifier, or <code>null</code> if the project does
	 * not exist.
	 * <p>
	 * The content, structure, and management of this area is the responsibility of the
	...
	/**
	 * Returns the list of all open projects which reference this project. This project may or may
	 * not exist. Returns an empty array if there are no referencing projects.
	 * 
	...
	/**
	 * Returns whether the project nature specified by the given nature extension id has been added
	 * to this project.
	 * 
	 ...
	 * <li>The nature has a prerequisite that is not enabled for this project.</li>
	 * <li>The nature specifies "one-of-nature" membership in a set, and there is another nature on
	 * this project belonging to that set.</li>
	 ...
	 * </p>
	 * 
	 * @return <code>true</code> if this project is open, <code>false</code> if this project is
	 ...
	 * <p>
	 * Opening a project constructs an in-memory representation of its resources from information
	 * stored on disk.
	 * </p>
	 * <p>
	 * When a project is opened for the first time, initial information about the project's existing
	 * resources can be obtained in the following ways:
	 * <ul>
	 * <li>If a {@link #loadSnapshot(int, URI, IProgressMonitor)} call has been made before the
	 * open, resources are restored from that file (a file written by
	 * {@link #saveSnapshot(int, URI, IProgressMonitor)}). When the snapshot includes resource tree
	 * information and can be loaded without error, no refresh is initiated, so the project's
	 * resource tree will match what the snapshot provides.
	 * <li>Otherwise, when the {@link IResource#BACKGROUND_REFRESH} flag is specified, resources on
	 * disk will be added to the project in the background after this method returns. Child
	 * resources of the project may not be available until this background refresh completes.
	 * <li>Otherwise, resource information is obtained with a refresh operation in the foreground,
	 * before this method returns.
	 * </ul>
	 * </p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event that includes an indication that the project has been opened and its resources have
	 * been added to the tree. If the <code>BACKGROUND_REFRESH</code> update flag is specified,
	 * multiple resource change events may occur as resources on disk are discovered and added to
	 * the tree. </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	...
	/**
	 * Changes this project resource to match the given project description. This project should
	 * exist and be open.
	 * <p>
	 * This is a convenience method, fully equivalent to:
	 * 
	 ...
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that the project's content has changed.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	 ...
	 *                <li>The workspace is out of sync with the project description file in the
	 ...
	 * exist and be open.
	 * <p>
	 * The given project description is used to change the project's natures, build spec, comment,
	 ...
	 * location, use {@link IResource#move(IProjectDescription, int, IProgressMonitor)}. The
	 * project's session and persistent properties are not affected.
	 * </p>
	 * <p>
	 ...
	 * of the project description file should be captured in the workspace's local history. The
	 * local history mechanism serves as a safety net to help the user recover from mistakes that
	 * might otherwise result in data loss. Specifying <code>KEEP_HISTORY</code> is recommended.
	 * Note that local history is maintained with each individual project, and gets discarded when a
	 * project is deleted from the workspace.
	 * </p>
	 * <p>
	 * The <code>AVOID_NATURE_CONFIG</code> update flag controls whether or not added and removed
	 ...
	 * {@link IResourceRuleFactory#modifyRule} is required; If the flag is not specified, the
	 * {@link IWorkspaceRoot} scheduling rule is required.
	 * </p>
	 * <p>
	 * Update flags other than <code>FORCE</code>, <code>KEEP_HISTORY</code>, and
	 * <code>AVOID_NATURE_CONFIG</code> are ignored.
	 * </p>
	 * <p>
	 * Prior to modifying the project description file, the file modification validator (if provided
	 * by the Team plug-in), will be given a chance to perform any last minute preparations.
	 * Validation is performed by calling <code>IFileModificationValidator.validateSave</code> on
	 * the project description file. If the validation fails, then this operation will fail.
	 * </p>
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that the project's content has changed.
	 ...
	 *            <code>KEEP_HISTORY</code> and <code>AVOID_NATURE_CONFIG</code>)
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li>This project does not exist in the workspace.</li>
	 ...
	 *                <li>The location in the local file system corresponding to the project
	 *                description file is occupied by a directory.</li>
	 *                <li>The workspace is not in sync with the project description file in the
