 ...
 * <p>
 * Markers themselves are handles in the same way as <code>IResources</code> are handles. Instances
 * of <code>IMarker</code> do not hold the attributes themselves but rather uniquely refer to the
 * attribute container. As such, their state may change underneath the handle with no warning to the
 * holder of the handle.
 * </p>
 * The Resources plug-in provides a general framework for defining and manipulating markers and
 * provides several standard marker types. </p>
 * <p>
 * Each marker has:
 * <ul>
 * <li>a type string, specifying its type (e.g. <code>"org.eclipse.core.resources.taskmarker"</code>
 * ),</li>
 ...
 * </ul>
 * The plug-in also provides an extension point ( <code>org.eclipse.core.resources.markers</code>)
 * into which other plug-ins can install marker type declaration extensions.
 * </p>
 * <p>
 * Marker types are declared within a multiple inheritance type system. New markers are defined in
 * the <code>plugin.xml</code> file of the declaring plug-in. A valid declaration contains elements
 * as defined by the extension point DTD:
 * <ul>
 ...
 * <li><i>attributes</i> - the list of standard attributes which may be present on this type of
 * marker</li>
 * <li><i>persistent</i> - whether markers of this type should be persisted by the platform</li>
 * </li>
 * </p>
 * <p>
 * All markers declared as <code>persistent</code> are saved when the workspace is saved, except
 * those explicitly set as transient (the <code>TRANSIENT</code> attribute is set as
 * <code>true</code>). A plug-in which defines a persistent marker is not directly involved in
 * saving and restoring the marker. Markers are not under version and configuration management, and
 * cannot be shared via VCM repositories.
 * </p>
 * <p>
 * Markers implement the <code>IAdaptable</code> interface; extensions are managed by the platform's
 * adapter manager.
 * </p>
 * 
...
public interface IMarker extends IAdaptable {
	 ...
	 * users. The content and form of this attribute is not specified or interpreted by the
	 * platform.
	 * 
	 * @see #getAttribute(String, String)
	 */
	...
	/**
	 * Priority marker attribute. A number from the set of high, normal and low priorities defined
	 * by the platform.
	 * 
	 * @see #PRIORITY_HIGH
	 * @see #PRIORITY_NORMAL
	...
	/**
	 * Character start marker attribute. An integer value indicating where a text marker starts.
	 ...
	 * manually change the marker (e.g. a task). The default is <code>true</code>. Note that the
	 * value of this attribute is to be used by the UI as a suggestion and its value will NOT be
	 * interpreted by Core in any manner and will not be enforced by Core when performing any
	 ...
	 * existence is not enforced. This attribute is intended to improve serviceability by providing
	 * a value that product support personnel or automated tools can use to determine appropriate
	 * help and resolutions for markers.
	 * 
	 * @see #getAttribute(String, String)
	 * @since 3.3
	 */
	...
	/**
	 * Deletes this marker from its associated resource. This method has no effect if this marker
	 * does not exist.
	 * 
	...
	/**
	 * Returns whether this marker exists in the workspace. A marker exists if its resource exists
	 * and has a marker with the marker's id.
	 * 
	 * @return <code>true</code> if this marker exists, otherwise <code>false</code>
	 */
	public boolean exists();

	/**
	 * Returns the attribute with the given name. The result is an instance of one of the following
	 * classes: <code>String</code>, <code>Integer</code>, or <code>Boolean</code>. Returns
	 * <code>null</code> if the attribute is undefined.
	 * 
	 ...
	 * the attribute is undefined or the marker does not exist or is not a string value.
	 * 
	 * @param attributeName the name of the attribute
	 * @param defaultValue the value to use if no value is found
	 * @return the value or the default value if no value was found.
	 ...
	 * 
	 * @return a map of attribute keys and values (key type : <code>String</code> value type :
	 *         <code>String</code>, <code>Integer</code>, or <code>Boolean</code>) or
	 *         <code>null</code>.
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li>This marker does not exist.</li>
	...
	/**
	 * Returns the attributes with the given names. The result is an an array whose elements
	 * correspond to the elements of the given attribute name array. Each element is
	 ...
	 *         created and midnight, January 1, 1970 UTC, or <code>0L</code> if the creation time is
	 *         not known (this can occur in workspaces created using v2.0 or earlier).
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li>This marker does not exist.</li>
	 *                </ul>
	...
	/**
	 * Returns the id of the marker. The id of a marker is unique relative to the resource with
	 * which the marker is associated. Marker ids are not globally unique.
	 * 
	...
	/**
	 * Returns the type of this marker. The returned marker type will not be <code>null</code>.
	 * 
	 ...
	 * type.
	 * 
	 * @return boolean <code>true</code>if the marker's type is the same as (or a sub-type of) the
	 *         given type.
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li>This marker does not exist.</li>
	...
	/**
	 * Sets the attribute with the given name. The value must be <code>null</code> or an instance of
	 * one of the following classes: <code>String</code>, <code>Integer</code>, or
	 * <code>Boolean</code>. If the value is <code>null</code>, the attribute is considered to be
	 * undefined.
	 * 
	 * <p>
	 * The attribute value cannot be <code>String</code> whose UTF encoding exceeds 65535 bytes. On
	 ...
	 * must be an instance of one of the following classes: <code>String</code>,
	 * <code>Integer</code>, or <code>Boolean</code>. Attributes previously set on the marker but
	 * not included in the given map are considered to be removals. Setting the given map to be
	 ...
	 *            <code>String</code> value type : <code>String</code>, <code>Integer</code>, or
