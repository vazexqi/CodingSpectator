 ...
 * <p>
 * If more than one rule is needed, they can be aggregated using the <code>MultiRule.combine</code>
 * method. Simplifying a group of rules does not change the set of resources that are covered, but
 * can improve job scheduling performance.
 * <p>
 * Note that <code>null</code> is a valid scheduling rule (indicating that no resources need to be
 * locked), and thus all methods in this class may return <code>null</code>.
 * 
 * @see WorkspaceJob
...
public interface IResourceRuleFactory {
	...
	/**
	 * Returns the scheduling rule that is required for changing the charset setting for a file or
	 * the default charset setting for a container.
	 * 
	...
	/**
	 * Returns the scheduling rule that is required for creating, modifying, or deleting markers on
	 * a resource.
	 * 
	...
	/**
	 * Returns the scheduling rule that is required for modifying a resource. For files,
	 * modification includes setting and appending contents. For projects, modification includes
	 * opening or closing the project, or setting the project description using the
	 * {@link IResource#AVOID_NATURE_CONFIG} flag. For all resources <code>touch</code> is
