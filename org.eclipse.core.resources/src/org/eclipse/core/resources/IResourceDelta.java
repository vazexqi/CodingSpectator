 ...
 * in time.
 * <p>
 * Resource deltas implement the <code>IAdaptable</code> interface; extensions are managed by the
...
public interface IResourceDelta extends IAdaptable {
	 ...
	 * That is, one that appears in the "after" state, not in the "before" one.
	 * 
	 * @see IResourceDelta#getKind()
	 */
	...
	/**
	 * Delta kind constant (bit mask) indicating that the resource has been changed. That is, one
	 * that appears in both the "before" and "after" states.
	 * 
	 * @see IResourceDelta#getKind()
	 */
	 ...
	 * location in the "before" state can be retrieved using <code>getMovedFromPath()</code>.
	 * 
	 * @see IResourceDelta#getFlags()
	 */
	 ...
	 * location in the new state can be retrieved using <code>getMovedToPath()</code>.
	 * 
	 * @see IResourceDelta#getFlags()
	 */
	 ...
	 * location in the "before" state can be retrieved using <code>getMovedFromPath()</code>. This
	 * flag is only used when describing potential changes using an
	...
	/**
	 * Change constant (bit mask) indicating that the resource was opened or closed. This flag is
	 * also set when the project did not exist in the "before" state. For example, if the current
	 * state of the resource is open then it was previously closed or did not exist.
	 * 
	 * @see IResourceDelta#getFlags()
	 */
	...
	/**
	 * Change constant (bit mask) indicating that the resource's sync status has changed. This type
	 * of change is not included in build deltas, only in those for resource notification.
	 * 
	 * @see IResourceDelta#getFlags()
	 */
	...
	/**
	 * Change constant (bit mask) indicating that the underlying file or folder of the linked
	 ...
	 * <code>REMOVED</code>, and <code>CHANGED</code>. The visitor's <code>visit</code> method is
	 * called with this resource delta if applicable. If the visitor returns <code>true</code>, the
	 * resource delta's children are also visited.
	 * <p>
	 * This is a convenience method, fully equivalent to
	 * <code>accept(visitor, IResource.NONE)</code>. Although the visitor will be invoked for this
	 * resource delta, it will not be invoked for any team-private member resources.
	 * </p>
	 * 
	 ...
	 * resource delta. If the visitor returns <code>true</code>, the resource delta's children are
	 * also visited.
	 * <p>
	 * This is a convenience method, fully equivalent to:
	 * 
	 ...
	 * <p>
	 * If the <code>INCLUDE_PHANTOMS</code> member flag is not specified (recommended), only child
	 * resource deltas involving existing resources will be visited (kinds <code>ADDED</code>,
	 * <code>REMOVED</code>, and <code>CHANGED</code>). If the <code>INCLUDE_PHANTOMS</code> member
	 * flag is specified, the result will also include additions and removes of phantom resources
	 * (kinds <code>ADDED_PHANTOM</code> and <code>REMOVED_PHANTOM</code>).
	 * </p>
	 * <p>
	 * If the <code>INCLUDE_TEAM_PRIVATE_MEMBERS</code> member flag is not specified (recommended),
	 * resource deltas involving team private member resources will be excluded from the visit. If
	 * the <code>INCLUDE_TEAM_PRIVATE_MEMBERS</code> member flag is specified, the visit will also
	 * include additions and removes of team private member resources.
	 * </p>
	 * 
	 * @param visitor the visitor
	 * @param memberFlags bit-wise or of member flag constants (
	 *            <code>IContainer.INCLUDE_PHANTOMS</code>, <code>INCLUDE_HIDDEN</code> and
	 *            <code>INCLUDE_TEAM_PRIVATE_MEMBERS</code>) indicating which members are of
	 *            interest
	 * @exception CoreException if the visitor failed with this exception.
	 * @see IResource#isPhantom()
	 ...
	 * <code>null</code> if no such descendent exists. The supplied path may be absolute or
	 * relative; in either case, it is interpreted as relative to this delta. Trailing separators
	 * are ignored. If the path is empty this delta is returned.
	 * <p>
	 * This is a convenience method to avoid manual traversal of the delta tree in cases where the
	 ...
	 * </pre>
	 * 
	 * Team-private member resources are <b>not</b> included in the result; neither are phantom
	 * resources.
	 * </p>
	 * 
	 ...
	 * mask. Kind masks are formed by the bitwise or of <code>IResourceDelta</code> kind constants.
	 ...
	 * </p>
	 * 
	 * @param kindMask a mask formed by the bitwise or of <code>IResourceDelta </code> delta kind
	 *            constants
	 * @return the resource deltas for all affected children
	 * @see IResourceDelta#ADDED
	 * @see IResourceDelta#REMOVED
	...
	/**
	 * Returns resource deltas for all children of this resource whose kind is included in the given
	 * mask. Masks are formed by the bitwise or of <code>IResourceDelta</code> kind constants.
	 * Returns an empty array if there are no affected children.
	 * <p>
	 * If the <code>INCLUDE_TEAM_PRIVATE_MEMBERS</code> member flag is not specified, (recommended),
	 * resource deltas involving team private member resources will be excluded. If the
	 * <code>INCLUDE_TEAM_PRIVATE_MEMBERS</code> member flag is specified, the result will also
	 * include resource deltas of the specified kinds to team private member resources.
	 * </p>
	 * <p>
	 * If the {@link IContainer#INCLUDE_HIDDEN} member flag is not specified, (recommended),
	 * resource deltas involving hidden resources will be excluded. If the
	 * {@link IContainer#INCLUDE_HIDDEN} member flag is specified, the result will also include
	 * resource deltas of the specified kinds to hidden resources.
	 * </p>
	 * <p>
	 * Specifying the <code>IContainer.INCLUDE_PHANTOMS</code> member flag is equivalent to
	 * including <code>IContainer.ADDED_PHANTOM</code> and <code>IContainer.REMOVED_PHANTOM</code>
	 * in the kind mask.
	 * </p>
	 * 
	 * @param kindMask a mask formed by the bitwise or of <code>IResourceDelta</code> delta kind
	 *            constants
	 * @param memberFlags bit-wise or of member flag constants (
	 *            <code>IContainer.INCLUDE_PHANTOMS</code>,
	 *            <code>IContainer.INCLUDE_TEAM_PRIVATE_MEMBERS</code> and
	 *            <code>IContainer.INCLUDE_HIDDEN</code>) indicating which members are of interest
	 * @return the resource deltas for all affected children
	 * @see IResourceDelta#ADDED
	 * @see IResourceDelta#REMOVED
	 * @see IResourceDelta#CHANGED
	 ...
	 * <p>
	 * The following codes (bit masks) are used when kind is <code>CHANGED</code>, and also when the
	 * resource is involved in a move:
	 * <ul>
	 * <li><code>CONTENT</code> - The bytes contained by the resource have been altered, or
	 * <code>IResource.touch</code> has been called on the resource.</li>
	 * <li><code>DERIVED_CHANGED</code> - The derived flag of the resource has been altered.</li>
	 * <li><code>ENCODING</code> - The encoding of the resource may have been altered. This flag is
	 * not set when the encoding changes due to the file being modified, or being moved.</li>
	 * <li><code>DESCRIPTION</code> - The description of the project has been altered, or
	 * <code>IResource.touch</code> has been called on the project. This flag is only valid for
	 * project resources.</li>
	 * <li><code>OPEN</code> - The project's open/closed state has changed. If it is not open, it
	 * was closed, and vice versa. This flag is only valid for project resources.</li>
	 ...
	 * delete or move), and was subsequently re-created (either by a create, move, or copy).</li>
	 * <li><code>LOCAL_CHANGED</code> - The resource is a linked resource, and the underlying file
	 * system object has been added or removed.</li>
	 * </ul>
	 ...
	 * the path of where it was moved to.</li>
	 * </ul>
	 ...
	 * return the path of where it was moved from.</li>
	 * </ul>
	 ...
	 * <code>getMovedFromPath()</code>.</li>
	 * </ul>
	 * 
	 * A simple move operation would result in the following delta information. If a resource is
	 * moved from A to B (with no other changes to A or B), then A will have kind
	 * <code>REMOVED</code>, with flag <code>MOVED_TO</code>, and <code>getMovedToPath</code> on A
	 * will return the path for B. B will have kind <code>ADDED</code>, with flag
	 * <code>MOVED_FROM</code>, and <code>getMovedFromPath</code> on B will return the path for A.
	 * B's other flags will describe any other changes to the resource, as compared to its previous
	 * location at A.
	 * </p>
	 * <p>
	 * Note that the move flags only describe the changes to a single resource; they don't
	 * necessarily imply anything about the parent or children of the resource. If the children were
	 * moved as a consequence of a subtree move operation, they will have corresponding move flags
	 * as well.
	 * </p>
	 * <p>
	 * Note that it is possible for a file resource to be replaced in the workspace by a folder
	 * resource (or the other way around). The resource delta, which is actually expressed in terms
	 * of paths instead or resources, shows this as a change to either the content or children.
	 * </p>
	 * 
	 * @return the flags
	...
	/**
	 * Returns the kind of this resource delta. Normally, one of <code>ADDED</code>,
	 * <code>REMOVED</code>, <code>CHANGED</code>. When phantom resources have been explicitly
	 * requested, there are two additional kinds: <code>ADDED_PHANTOM</code> and
	 * <code>REMOVED_PHANTOM</code>.
	 * 
	 ...
	 * was moved. This value is only valid if the <code>MOVED_FROM</code> change flag is set;
	...
	/**
	 * Returns the full path (in the "after" state) to which this resource (in the "before" state)
	 * was moved. This value is only valid if the <code>MOVED_TO</code> change flag is set;
	 * otherwise, <code>null</code> is returned.
	 * <p>
	 * Note: the returned path never has a trailing separator.
	 * 
	 ...
	 * <p>
	 * For additions (<code>ADDED</code>), this handle describes the newly-added resource; i.e., the
	 * one in the "after" state.
	 * <p>
	 ...
	 * state. Even though this resource would not normally exist in the current workspace, the type
	 * of resource that was removed can be determined from the handle.
	 * <p>
	 * For phantom additions and removals (<code>ADDED_PHANTOM</code> and
	 * <code>REMOVED_PHANTOM</code>), this is the handle of the phantom resource.
	 * 
