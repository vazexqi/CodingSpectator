 ...
 * <p>
 * Files, like folders, may exist in the workspace but not be local; non-local file resources serve
 * as place-holders for files whose content and properties have not yet been fetched from a
 * repository.
 * </p>
 * <p>
 * Files implement the <code>IAdaptable</code> interface; extensions are managed by the platform's
 * adapter manager.
 * </p>
 * 
...
public interface IFile extends IResource, IEncodedStorage, IAdaptable {
	...
	/**
	 * Character encoding constant (value 1) which identifies files that are encoded with the
	...
	/**
	 * Character encoding constant (value 4) which identifies files that are encoded with the
	 * UTF-16BE character encoding scheme.
	 * 
	 * @see IFile#getEncoding()
	 * @deprecated see getEncoding for details
	 */
	...
	/**
	 * Character encoding constant (value 5) which identifies files that are encoded with the
	 * UTF-16LE character encoding scheme.
	 * 
	 * @see IFile#getEncoding()
	 * @deprecated see getEncoding for details
	 */
	 ...
	 * event, including an indication that this file's content have been changed.
	 * </p>
	 * <p>
	 ...
	 *            local file system
	 * @param keepHistory a flag indicating whether or not to store the current contents in the
	 *            local history
	 ...
	 *                directory.</li>
	 *                <li>The workspace is not in sync with the corresponding location in the local
	 ...
	 * </p>
	 * <p>
	 * If this file is non-local then this method will always fail. The only exception is when
	 * <code>FORCE</code> is specified and the file exists in the local file system. In this case
	 * the file is made local and the given contents are appended.
	 * </p>
	 * <p>
	 * The <code>KEEP_HISTORY</code> update flag controls whether or not a copy of current contents
	 ...
	 * recover from mistakes that might otherwise result in data loss. Specifying
	 * <code>KEEP_HISTORY</code> is recommended except in circumstances where past states of the
	 * files are of no conceivable interest to the user. Note that local history is maintained with
	 * each individual project, and gets discarded when a project is deleted from the workspace.
	 * This flag is ignored if the file was not previously local.
	 * </p>
	 * <p>
	 * Update flags other than <code>FORCE</code> and <code>KEEP_HISTORY</code> are ignored.
	 * </p>
	 * <p>
	 * Prior to modifying the contents of this file, the file modification validator (if provided by
	 * the VCM plug-in), will be given a chance to perform any last minute preparations. Validation
	 ...
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that the file has been added to its parent.
	 * </p>
	 * <p>
	 ...
	 *            <code>null</code> if the file should be marked as not local
	 * @param force a flag controlling how to deal with resources that are not in sync with the
	 *            local file system
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
	 * @exception CoreException if this method fails. Reasons include:
	 ...
	 * contents are supplied by the data in the given stream. This method closes the stream whether
	 * it succeeds or fails. If the stream is <code>null</code> then a file is not created in the
	 * local file system and the created file resource is marked as being non-local.
	 * <p>
	 * The {@link IResource#FORCE} update flag controls how this method deals with cases where the
	 * workspace is not completely in sync with the local file system. If {@link IResource#FORCE} is
	 * not specified, the method will only attempt to write a file in the local file system if it
	 * does not already exist. This option ensures there is no unintended data loss; it is the
	 * recommended setting. However, if {@link IResource#FORCE} is specified, this method will
	 * attempt to write a corresponding file in the local file system, overwriting any existing one
	 * if need be.
	 * </p>
	 * <p>
	 ...
	 *            {@link IResource#DERIVED}, and {@link IResource#TEAM_PRIVATE})
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
	 * @exception CoreException if this method fails. Reasons include:
	 *                <ul>
	 *                <li>This resource already exists in the workspace.</li>
	 ...
	 * <p>
	 * The {@link IResource#ALLOW_MISSING_LOCAL} update flag controls how this method deals with
	 ...
	 * the operation will fail in the case where the local file system file does not exist or the
	 ...
	 *            {@link IResource#ALLOW_MISSING_LOCAL}, {@link IResource#REPLACE} and
	...
	/**
	 * Creates a new file resource as a member of this handle's parent resource. The file's contents
	 * will be located in the file specified by the given URI. The given URI must be either
	 * absolute, or a relative URI whose first path segment is the name of a workspace path
	 * variable.
	 * <p>
	 ...
	 * variable. If <code>ALLOW_MISSING_LOCAL</code> is not specified, the operation will fail in
	 * the case where the file system file does not exist or the path is relative to an undefined
	 * variable.
	 * </p>
	 * <p>
	 * The {@link IResource#REPLACE} update flag controls how this method deals with cases where a
	 ...
	 *                <li>The name of this resource is not valid (according to
	 *                <code>IWorkspace.validateName</code>).</li>
	 *                <li>The corresponding location in the file system does not exist, or is
	 *                relative to an undefined variable, and <code>ALLOW_MISSING_LOCAL</code> is not
	 *                specified.</li>
	 *                <li>The corresponding location in the file system is occupied by a directory
	 *                (as opposed to a file).</li>
	 *                <li>Resource changes are disallowed during certain types of resource change
	 *                event notification. See <code>IResourceChangeEvent</code> for more details.</li>
	 *                <li>The team provider for the project which contains this folder does not
	 *                permit linked resources.</li>
	 ...
	 * characters.
	 * <p>
	 * This refinement of the corresponding {@link IEncodedStorage} method is a convenience method,
	 * fully equivalent to:
	 * 
	 * <pre>
	 * getCharset(true);
	 ...
	 * </p>
	 * <p>
	 * <b>Note 1</b>: this method does not check whether the result is a supported charset name.
	 * Callers should be prepared to handle <code>UnsupportedEncodingException</code> where this
	 * charset is used.
	 * </p>
	 * <p>
	 * <b>Note 2</b>: this method returns a cached value for the encoding that may be out of date if
	 * the file is not synchronized with the local file system and the encoding has since changed in
	 * the file system.
	 * </p>
	 * 
	 ...
	 * characters.
	 * <p>
	 * If checkImplicit is <code>false</code>, this method will return the charset defined by
	 * calling <code>setCharset</code>, provided this file exists, or <code>null</code> otherwise.
	 * </p>
	 * <p>
	 * If checkImplicit is <code>true</code>, this method uses the following algorithm to determine
	...
	/**
	 * Returns the name of a charset to be used to encode the given contents when saving to this
	 * file. This file does not have to exist. The character stream is <em>not</em> automatically
	 * closed.
	 * <p>
	 ...
	 * one can be determined, or</li>
	 * <li>the default encoding for the parent resource (as defined by
	 * <code>IContainer#getDefaultCharset</code>).</li>
	 * </ol>
	 * </p>
	 * <p>
	 * <b>Note</b>: this method does not check whether the result is a supported charset name.
	 * Callers should be prepared to handle <code>UnsupportedEncodingException</code> where this
	 * charset is used.
	 * </p>
	 * 
	 ...
	 * <code>getDescriptionFor(getContents(), getName(), IContentDescription.ALL)</code> on
	 * <code>IContentTypeManager</code>, but provides better opportunities for improved performance.
	...
	/**
	 * Returns an open input stream on the contents of this file. This refinement of the
	 * corresponding <code>IStorage</code> method returns an open input stream on the contents of
	 * this file. The client is responsible for closing the stream when finished.
	 * 
	...
	/**
	 * This refinement of the corresponding <code>IStorage</code> method returns an open input
	 * stream on the contents of this file. The client is responsible for closing the stream when
	 * finished. If force is <code>true</code> the file is opened and an input stream returned
	 * regardless of the sync state of the file. The file is not synchronized with the workspace. If
	 * force is <code>false</code> the method fails if not in sync.
	 * 
	 * @param force a flag controlling how to deal with resources that are not in sync with the
	 *            local file system
	 * @return an input stream containing the contents of the file
	 ...
	 *                <li>This resource is not local.</li>
	 *                <li>The workspace is not in sync with the corresponding location in the local
	 *                file system and force is <code>false</code>.</li>
	 *                </ul>
	 */
	 ...
	 * could not be determined. The returned constant will be one of the ENCODING_* constants
	 * defined on IFile.
	 * 
	 * This method attempts to guess the file's character encoding by analyzing the first few bytes
	 * of the file. If no identifying pattern is found at the beginning of the file, ENC_UNKNOWN
	 * will be returned. This method will not attempt any complex analysis of the file to make a
	 * guess at the encoding that is used.
	 * 
	...
	/**
	 * Returns the full path of this file. This refinement of the corresponding
	 * <code>IStorage</code> and <code>IResource</code> methods links the semantics of resource and
	 * storage object paths such that <code>IFile</code>s always have a path and that path is
	 * relative to the containing workspace.
	 * 
	 ...
	 * that <code>IFile</code>s always have a name and that name equivalent to the last segment of
	 * its full path.
	 * 
	...
	/**
	 * Returns whether this file is read-only. This refinement of the corresponding
	 * <code>IStorage</code> and <code>IResource</code> methods links the semantics of read-only
	 * resources and read-only storage objects.
	 * 
	 ...
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that this file has been removed from its parent and a new file
	 ...
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that this file's encoding has changed.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	 ...
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event, including an indication that this file's contents have been changed.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
