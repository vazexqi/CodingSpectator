...
public abstract class ResourceMapping extends PlatformObject {
	...
	/**
	 * Accepts the given visitor for all existing resources in this mapping. The visitor's
	 * {@link IResourceVisitor#visit} method is called for each accessible resource in this mapping.
	 * 
	 ...
	 * <p>
	 * This method always returns <code>false</code> when the given resource mapping's model
	 * provider id does not match that the of the receiver.
	 * </p>
	 * 
	 * @param mapping the given resource mapping
	 * @return <code>true</code> if this mapping contains all the resources of the given mapping,
	 ...
	 * @return <code>true</code> if the receiver is equal to the given object, and
	...
	/**
	 * Return the model provider for the model object of this resource mapping. The model provider
	 * is obtained using the id returned from <code>getModelProviderId()</code>.
	 * 
	 ...
	 * which they are to be traversed. This method returns an array of traversals in order to
	 * provide flexibility in describing the traversals that constitute a model element.
	 * <p>
	 * Subclasses should, when possible, include all resources that are or may be members of the
	 * model element. For instance, a model element should return the same list of resources
	 * regardless of the existence of the files on the file system. For example, if a logical
	 * resource called "form" maps to "/p1/form.xml" and "/p1/form.java" then whether form.xml or
	 * form.java existed, they should be returned by this method.
	 * </p>
	 * <p>
	 ...
	 * (e.g. a repository). This method is provided with a context which, when provided, gives
	 * access to the members of corresponding remote containers and the contents of corresponding
	 * remote files. This gives the model element the opportunity to deduce what additional
	 * resources should be included in the traversal.
	 * </p>
	 * 
	 * @param context gives access to the state of remote resources that correspond to local
	 *            resources for the purpose of determining traversals that adequately cover the
	 *            model element resources given the state of the model element in another location.
	 *            This parameter may be <code>null</code>, in which case the implementor can assume
	 *            that only the local resources are of interest to the client.
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
