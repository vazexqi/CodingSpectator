 ...
 * <p>
 * The flags of the traversal indicate which special resources should be included or excluded from
 * the traversal. The flags used are the same as those passed to the
 * {@link IResource#accept(IResourceVisitor, int, int)} method.
 * 
 * <p>
...
public class ResourceTraversal {
	...
	/**
	 * Returns all markers of the specified type on existing resources in this traversal. If
	 ...
	 * contained within the same project and need not exist in the local file system. The traversal
	 * of the returned resources should be done considering the flag returned by getDepth. If a
	 * resource returned by a traversal is a file, it should always be visited. If a resource of a
	 * traversal is a folder then files contained in the folder can only be visited if the folder is
