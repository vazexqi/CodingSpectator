...
public abstract class RemoteResourceMappingContext extends ResourceMappingContext {
	...
	/**
	 * Refresh flag constant (bit mask value 1) indicating that the mapping will be making use of
	 * the contents of the files covered by the traversals being refreshed.
	 */
	 ...
	 * access the contents of the base resource that corresponds to the given local resource. The
	 * base of a resource is the contents of the resource before any local modifications were made.
	 * If the base file does not exist, or if this is a two-way comparison, <code>null</code> is
	 * returned. The provided local file handle need not exist locally. A exception is thrown if the
	 * corresponding base resource is not a file.
	 * <p>
	 * This method may be long running as a server may need to be contacted to obtain the contents
	 * of the file.
	 * </p>
	 * 
	 ...
	 *                <li>The corresponding remote resource is not a container (status code will be
	 *                {@link IResourceStatus#RESOURCE_WRONG_TYPE}).</li>
	 *                </ul>
	 */
	 ...
	 * exception is thrown if the base resource is not capable of having members. This method
	 * returns <code>null</code> if the base members cannot be computed, in which case clients
	 * should call {@link #fetchMembers(IContainer, IProgressMonitor)} which returns the combined
	 * members for the base and remote. </p>
	 * <p>
	 * This method may be long running as a server may need to be contacted to obtain the members of
	 * the base resource.
	 * </p>
	 * <p>
	 * This default implementation always returns <code>null</code>, but subclasses may override.
	 * </p>
	 * 
	 ...
	 *                <li>The base resource is not a container (status code will be
	 *                {@link IResourceStatus#RESOURCE_WRONG_TYPE}).</li>
	 *                </ul>
	 * @since 3.3
	 */
	 ...
	 * container. The container need not exist locally and the result may include entries that do
	 * not exist locally and may not include all local children. An empty list is returned if the
	 * remote resource which corresponds to the container is empty or if the remote does not exist.
	 * An exception is thrown if the corresponding remote is not capable of having members.
	 * <p>
	 * This method may be long running as a server may need to be contacted to obtain the members of
	 * the container's corresponding remote resource.
	 * </p>
	 * 
	 * @param container the local container
	 * @param monitor a progress monitor, or <code>null</code> if progress reporting is not desired
	 ...
	 * <code>null</code> is returned. The provided local file handle need not exist locally. A
	...
	/**
	 * Returns the members of the remote resource corresponding to the given container. The
	 * container and the returned members need not exist locally and may not include all children
	 * that exist locally. An empty list is returned if the remote resource is empty or does not
	 * exist. An exception is thrown if the remote resource is not capable of having members. This
	 * method returns <code>null</code> if the remote members cannot be computed, in which case
	 * clients should call {@link #fetchMembers(IContainer, IProgressMonitor)} which returns the
	 * combined members for the base and remote. </p>
	 * <p>
	 * This method may be long running as a server may need to be contacted to obtain the members of
	 * the remote resource.
	 * </p>
	 ...
	 *                <ul>
	 *                <li>The server could not be contacted for some reason.</li>
	 *                <li>The remote resource is not a container (status code will be
	 *                {@link IResourceStatus#RESOURCE_WRONG_TYPE}).</li>
	 *                </ul>
	 * @since 3.3
	 */
	...
	/**
	 * For three-way comparisons, this method indicates whether local modifications have been made
	 * to the given resource. For two-way comparisons, calling this method has the same effect as
	 * calling {@link #hasRemoteChange(IResource, IProgressMonitor)}.
	 * 
	 ...
	 * contents.
	 * <p>
	 * For three-way comparisons, return whether the contents of the corresponding remote differ
	 ...
	 * remote contents.
	 * <p>
	 * For two-way comparisons, return <code>true</code> if the remote contents differ from the
	 * local contents. In this case, this method is equivalent to
	 * {@link #hasLocalChange(IResource, IProgressMonitor)}
	 * <p>
	 * This can be used by clients to determine if they need to fetch the remote contents in order
	 * to determine if the resources that constitute the model element are different in the remote
	 * location. If the local file exists and the remote file does not, or the remote file exists
	 * and the local does not then the contents will be said to differ (i.e. <code>true</code> is
	 * returned). Also, implementors will most likely use a timestamp based comparison to determine
	 * if the contents differ. This may lead to a situation where <code>true</code> is returned but
	 * the actual contents do not differ. Clients must be prepared handle this situation.
	 * </p>
	 * 
	 ...
	 * <p>
	 * Note that this is really only a hint to the context provider. It is up to implementors to
	 * decide, based on the provided traversals, how to efficiently perform the refresh. In the
	 * ideal case, calls to {@link #hasRemoteChange(IResource, IProgressMonitor)} and
	 * {@link #fetchMembers} would not need to contact the server after a call to a refresh with
	 * appropriate traversals. Also, ideally, if {@link #FILE_CONTENTS_REQUIRED} is on of the flags,
	 * then the contents for these files will be cached as efficiently as possible so that calls to
	 * {@link #fetchRemoteContents} will also not need to contact the server. This may not be
	 * possible for all context providers, so clients cannot assume that the above mentioned methods
	 * will not be long running. It is still advisable for clients to call {@link #refresh} with as
	 * much details as possible since, in the case where a provider is optimized, performance will
	 * be much better.
	 * </p>
	 * 
	 ...
	 *            one of the flags, this indicates that the client will be accessing the contents of
	 *            the files covered by the traversals. {@link #NONE} should be used when no
