...
public interface ISaveParticipant extends EventListener {
	...
	/**
	 * Tells this participant that the workspace is about to be saved. In preparation, the
	 * participant is expected to suspend its normal operation until further notice.
	 * <code>saving</code> will be next, followed by either <code>doneSaving</code> or
	 * <code>rollback</code> depending on whether the workspace save was successful.
	 * <p>
	 * Note: This method is called by the platform; it is not intended to be called directly by
	 * clients.
	 * </p>
	 ...
	 * the participant must absolutely guarantee that any important user data it has gathered will
	 * not be irrecoverably lost in the event of a crash. The only difference is in the space-time
	 ...
	 * that will aid it in retaining user state and configuration information and quickly getting
	 * back in sync with the state of the platform at a later point.</li>
	 * <li>Snapshots: the participant is discouraged from saving non-essential information that
	 * could be recomputed in the unlikely event of a crash. This lifecycle event will happen often
	 * and participant actions should take an absolute minimum of time.</li>
	 ...
	 * the other hand, the Java IDE would not save the image builder state on a snapshot because
	 * that information is non-essential; in the unlikely event of a crash, the image should be
	 * rebuilt either from scratch or from the last saved state.
	 * </p>
	 * <p>
	 ...
	 * all resource changes that have happened since then, if this information is still available.
	 * The following snippet shows what a participant plug-in would need to do if and when it is
	 * reactivated:
	 * 
	 * <pre>
	 *     IWorkspace ws = ...; // known
