 ...
 * interested in.
 * <p>
 * This class is intended to be subclassed by the team component in conjunction with the
 * <code>org.eclipse.core.resources.teamHook</code> standard extension point. Individual team
 * providers may also subclass this class. It is not intended to be subclassed by other clients. The
 * methods defined on this class are called from within the implementations of workspace API methods
 * and must not be invoked directly by clients.
 * </p>
 * 
 * @since 2.1
 */
public abstract class TeamHook extends InternalTeamHook {
	...
	/**
	 * Returns the resource scheduling rule factory that should be used when workspace operations
	 * are invoked on resources in that project. The workspace will ask the team hook this question
	 * only once per project, per session. The workspace will assume the returned result is valid
	 * for the rest of that session, unless the rule is changed by calling
	 * <code>setRuleFactory</code>.
	 * <p>
	 * This method must not return <code>null</code>. If no special rules are required by the team
	 ...
	 * project. This method only needs to be called if the factory has changed since the initial
	 * call to <code>getRuleFactory</code> for the given project
	 * <p>
	 * The supplied factory must not be <code>null</code>. If no special rules are required by the
	 * team hook for the given project, the value of the <code>defaultFactory</code> field should be
	 * used.
	 * <p>
	 * Note that the new rule factory will only take effect for resource changing operations that
	 * begin after this method completes. Care should be taken to avoid calling this method during
	 * the invocation of any resource changing operation (in any thread). The best time to change
	 * rule factories is during resource change notification when the workspace is locked for
	 * modification.
	 * 
	 ...
	 * an opportunity to hook into the beginning of the implementation of
	 * <code>IFile.createLink</code>.
	 * <p>
	 ...
	 *         status object with severity <code>IStatus.ERROR</code> indicating why the creation is
	 *         not allowed.
	 ...
	 * Validates whether a particular attempt at link creation is allowed. This gives team providers
	 * an opportunity to hook into the beginning of the implementation of
	 * {@link IFile#createLink(URI, int, IProgressMonitor) }
	 * <p>
	 * The implementation of this method runs "below" the resources API and is therefore very
	 * restricted in what resource API method it can call. The list of useable methods includes most
	 * resource operations that read but do not update the resource tree; resource operations that
	 ...
	 * Validates whether a particular attempt at link creation is allowed. This gives team providers
	 * an opportunity to hook into the beginning of the implementation of
	 * {@link IFolder#createLink(URI, int, IProgressMonitor)}
	 * <p>
	 * The implementation of this method runs "below" the resources API and is therefore very
	 * restricted in what resource API method it can call. The list of useable methods includes most
	 * resource operations that read but do not update the resource tree; resource operations that
