...
/**
 * This class is used to maintain a list of listeners. It is a fairly lightweight object, occupying
 * minimal space when no listeners are registered.
 * <p>
 * Note that the <code>add</code> method checks for and eliminates duplicates based on identity (not
 * equality). Likewise, the <code>remove</code> method compares based on identity.
 * </p>
 * <p>
 * This implementation is thread safe. The listener list is copied every time it is modified, so
 * readers do not need to copy or synchronize. This optimizes for frequent reads and infrequent
 * writes, and assumes that readers can be trusted not to modify the returned array.
 */
public class ResourceChangeListenerList {
	 ...
	 * array singleton instance (no garbage is created). Use this method when notifying listeners,
	 * so that any modifications to the listener list during the notification will have no effect on
