 ...
 * @since 2.0
 * 
 *        Implementation note: Since the move/delete hook involves running third party code, the
 *        workspace lock is not held. This means the workspace lock must be re-acquired whenever we
 *        need to manipulate the workspace in any way. All entry points from third party code back
 *        into the tree must be done in an acquire/release pair.
 */
class ResourceTree implements IResourceTree {
	...
	/**
	 * Copies the local history of source to destination. Note that if source is an IFolder, it is
	 * assumed that the same structure exists under destination and the local history of any IFile
	 * under source will be copied to the associated IFile under destination.
	 */
	...
	/**
	 * Makes sure that the destination directory for a project move is unoccupied. Returns true if
	 * successful, and false if the move should be aborted
	 */
	...
	/**
	 * Helper method for #standardDeleteFile. Returns a boolean indicating whether or not the delete
	 * was successful.
	 */
	...
	/**
	 * Helper method for #standardDeleteFolder. Returns a boolean indicating whether or not the
	 * deletion of this folder was successful. Does a best effort delete of this resource and its
	 * children.
	 */
	...
	/**
	 * Refreshes the resource hierarchy with its children. In case of failure adds an appropriate
	 * status to the resource tree's status.
	 */
	...
	/**
	 * The specific operation for which this tree was created has completed and this tree should not
	 * be used anymore. Ensure that this is the case by making it invalid. This is checked by all
	 * API methods.
	 */
	void makeInvalid() {
		this.isValid= false;
	...
	/**
	 * Helper method for moving the project content. Determines the content location based on the
	 * project description. (default location or user defined?)
	 */
	...
	/**
	 * Helper method to update all the timestamps in the tree to match those in the file system.
	 * Used after a #move.
	 */
