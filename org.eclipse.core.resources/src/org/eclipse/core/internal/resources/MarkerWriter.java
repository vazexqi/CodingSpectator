...
public class MarkerWriter {
	...
	/**
	 * Returns an Object array of length 2. The first element is an Integer which is the number of
	 * persistent markers found. The second element is an array of boolean values, with a value of
	 * true meaning that the marker at that index is to be persisted.
	 */
	...
	/**
	 * SAVE_FILE -> VERSION_ID RESOURCE+ VERSION_ID -> int RESOURCE -> RESOURCE_PATH MARKERS_SIZE
	 * MARKER+ RESOURCE_PATH -> String MARKERS_SIZE -> int MARKER -> MARKER_ID TYPE ATTRIBUTES_SIZE
	 * ATTRIBUTE* CREATION_TIME MARKER_ID -> long TYPE -> INDEX | QNAME INDEX -> byte int QNAME ->
	 * byte String ATTRIBUTES_SIZE -> short ATTRIBUTE -> ATTRIBUTE_KEY ATTRIBUTE_VALUE ATTRIBUTE_KEY
	 * -> String ATTRIBUTE_VALUE -> INTEGER_VALUE | BOOLEAN_VALUE | STRING_VALUE | NULL_VALUE
	 * INTEGER_VALUE -> byte int BOOLEAN_VALUE -> byte boolean STRING_VALUE -> byte String
	 * NULL_VALUE -> byte CREATION_TIME -> long
	 * 
	 */
	 ...
	 * RESOURCE -> RESOURCE_PATH MARKER_SIZE MARKER+ RESOURCE_PATH -> String MARKER_SIZE -> int
	 * MARKER -> MARKER_ID TYPE ATTRIBUTES_SIZE ATTRIBUTE* CREATION_TIME MARKER_ID -> long TYPE ->
	 * INDEX | QNAME INDEX -> byte int QNAME -> byte String ATTRIBUTES_SIZE -> short ATTRIBUTE ->
	 * ATTRIBUTE_KEY ATTRIBUTE_VALUE ATTRIBUTE_KEY -> String ATTRIBUTE_VALUE -> BOOLEAN_VALUE |
	 * INTEGER_VALUE | STRING_VALUE | NULL_VALUE BOOLEAN_VALUE -> byte boolean INTEGER_VALUE -> byte
	 * int STRING_VALUE -> byte String NULL_VALUE -> byte CREATION_TIME -> long
	 */
