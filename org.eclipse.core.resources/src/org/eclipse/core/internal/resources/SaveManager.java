...
public class SaveManager implements IElementInfoFlattener, IManager, IStringPoolParticipant {
	...
	/**
	 * The number of empty operations that are equivalent to a single non- trivial operation.
	 */
	...
	/**
	 * A flag indicating that a save operation is occurring. This is a signal that snapshot should
	 * not be scheduled if a nested operation occurs during save.
	 */
	...
	/**
	 * In-memory representation of plugins saved state. Maps String (plugin id)-> SavedState. This
	 * map is accessed from API that is not synchronized, so it requires independent
	 * synchronization. This is accomplished using a synchronized wrapper map.
	 */
	protected Map savedStates;

	/**
	 * Ids of plugins that participate on a workspace save. Maps String (plugin id)->
	 * ISaveParticipant. This map is accessed from API that is not synchronized, so it requires
	 * independent synchronization. This is accomplished using a synchronized wrapper map.
	 */
	...
	/**
	 * Remove the delta expiration timestamp from the master table, either because the saved state
	 * has been processed, or the delta has expired.
	 */
	...
	/**
	 * Marks the current participants to not receive deltas next time they are registered as save
	 * participants. This is done in order to maintain consistency if we crash after a snapshot. It
	 * would force plug-ins to rebuild their state.
	 */
	...
	/**
	 * Collects the set of ElementTrees we are still interested in, and removes references to any
	 * other trees.
	 */
	...
	/**
	 * Given a collection of save participants, compute the collection of <code>SaveContexts</code>
	 * to use during the save lifecycle. The keys are plugins and values are SaveContext objects.
	 */
	...
	/**
	 * Returns a table mapping having the plug-in id as the key and the old tree as the value. This
	 * table is based on the union of the current savedStates</code> and the given table of
	 * contexts. The specified tree is used as the tree for any newly created saved states. This
	 * method is used to compute the set of saved states to be written out.
	 */
	...
	/**
	 * Hooks the end of a save operation, for debugging and performance monitoring purposes.
	 */
	...
	/**
	 * Hooks the start of a save operation, for debugging and performance monitoring purposes.
	 */
	...
	/**
	 * Remove marks from current save participants. This marks prevent them to receive their deltas
	 * when they register themselves as save participants.
	 */
	...
	/**
	 * Reset the snapshot mechanism for the non-workspace files. This includes the markers and sync
	 * info.
	 */
	...
	/**
	 * Restores the state of this workspace by opening the projects which were open when it was last
	 * saved.
	 */
	...
	/**
	 * Restores the contents of this project. Throw an exception if the project could not be
	 * restored.
	 * 
	 * @return <code><code>true</code> if the project data was restored successfully, and
	 *         <code>false</code> if non-critical problems occurred while restoring.
	...
	/**
	 * Restores the contents of this project from a refresh snapshot, if possible. Throws an
	 * exception if the snapshot is found but an error occurs when reading the file.
	 * 
	 * @return <code><code>true</code> if the project data was restored successfully, and
	 *         <code>false</code> if the refresh snapshot was not found or could not be opened.
	...
	/**
	 * Reads the markers which were originally saved for the tree rooted by the given resource.
	 */
	...
	/**
	 * Restores the contents of this project. Throw an exception if the project description could
	 * not be restored.
	 */
	...
	/**
	 * Restores the workspace tree from snapshot files in the event of a crash. The workspace tree
	 * must be open when this method is called, and will be open at the end of this method. In the
	 * event of a crash recovery, the snapshot file is not deleted until the next successful save.
	 */
	...
	/**
	 * Reads the sync info which was originally saved for the tree rooted by the given resource.
	 */
	...
	/**
	 * Reads the contents of the tree rooted by the given resource from the file system. This method
	 * is used when restoring a complete workspace after workspace save/shutdown.
	 * 
	...
	/**
	 * Restores the trees for the builders of this project from the local disk. Does nothing if the
	 * tree file does not exist (this means the project has never been saved). This method is used
	 * when restoring a saved/closed project. restoreTree(Workspace) is used when restoring a
	 * complete workspace after workspace save/shutdown.
	 * 
	...
	/**
	 * Writes the metainfo (e.g. descriptions) of the given workspace and all projects to the local
	 * disk.
	 */
	...
	/**
	 * Ensures that the project meta-info is saved. The project meta-info is usually saved as soon
	 * as it changes, so this is just a sanity check to make sure there is something on disk before
	 * we shutdown.
	 * 
	...
	/**
	 * Writes a snapshot of project refresh information to the specified location.
	 * 
	 ...
	 * save. saveTree(Project) is used to save the state of an individual project.
	 * 
	...
	/**
	 * Performs a snapshot if one is deemed necessary. Encapsulates rules for determining when a
	 * snapshot is needed. This should be called at the end of every top level operation.
	 */
	 ...
	 * parent will be tree i-1. The input tree array may contain duplicate trees.
	 */
	...
	/**
	 * Update the expiration time for the given plug-in's tree. If the tree was never loaded, use
	 * the current value in the master table. If the tree has been loaded, use the provided new
	 * timestamp.
	 * 
	 * The timestamp is used in the policy for cleaning up tree's of plug-ins that are not often
	 * activated.
	 */
	...
	/**
	 * Visit the given resource (to depth infinite) and write out extra information like markers and
	 * sync info. To be called during a full save and project save.
	 * 
	...
	/**
	 * Visit the given resource (to depth infinite) and write out extra information like markers and
	 * sync info. To be called during a snapshot
	 * 
	 * FIXME: This method is ugly. Fix it up and look at merging with #visitAndSnap
	 */
	...
	/**
	 * Writes out persistent information about all builders for which a last built tree is
	 * available. File format is: int - number of builders for each builder: UTF - project name UTF
	 * - fully qualified builder extension name int - number of interesting projects for builder For
	 * each interesting project: UTF - interesting project name
	 */
	 ...
	 * builder with a previously built state). Throws an IOException if anything went wrong during
	 * save. Attempts to close the provided stream at all costs.
	 */
	protected void writeTree(Project project, DataOutputStream output, IProgressMonitor monitor) throws IOException, CoreException {
		...
		try {
			...
			try {
				/**
				 * Obtain a list of BuilderPersistentInfo. This list includes builders that have
				 * never been instantiated but already had a last built state.
				 */
				ArrayList builderInfos= workspace.getBuildManager().createBuildersPersistentInfo(project);
				if (builderInfos == null)
