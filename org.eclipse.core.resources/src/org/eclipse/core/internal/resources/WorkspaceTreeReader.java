...
/**
 * Default tree reader that does not read anything. This is used in cases where the tree format is
 * unknown (for example when opening a workspace from a future version).
 */
public abstract class WorkspaceTreeReader {

	/**
	 * Configuration setting to have an existing workspace project name take precedence over data
	 * being read, when set to <code>true</code>.
	 */
	 ...
	 * 
	 * @param renameProjectNode if <code>true</code>, set up the reader to have the existing root
	 *            node in the workspace (that is, the project being read into) take precedence over
	 *            the root node being read from the file. Otherwise, the tree file is read
	 *            unmodified.
	 */
