...
/**
 * This class provides the same interface as <code>WorkspaceDescription</code> but instead of
 * changing/obtaining values from its internal state, it changes/obtains properties in/from the
 * workspace plug-in's preferences.
 * 
 * Note: for performance reasons, some frequently called accessor methods are reading a cached value
 * from the super class instead of reading the corresponding property preference store. To keep the
 * cache synchronized with the preference store, a property change listener is used.
 */
...
public class WorkspacePreferences extends WorkspaceDescription {
	 ...
	 * string2",..."stringN"} to a string in the form "string1,string2,... stringN".
	 */
