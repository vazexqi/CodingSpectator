...
public class ModelObjectWriter implements IModelObjectConstants {
	...
	/**
	 * Returns the string representing the serialized set of build triggers for the given command
	 */
	...
	private boolean shouldWriteTriggers(BuildCommand command) {
		...
		if (!command.isConfigurable())
			return false;
		return !command.isBuilding(IncrementalProjectBuilder.AUTO_BUILD) || !command.isBuilding(IncrementalProjectBuilder.CLEAN_BUILD) || !command.isBuilding(IncrementalProjectBuilder.FULL_BUILD)
	...
	/**
	 * Writes a location to the XML writer. For backwards compatibility, local file system locations
	 * are written and read using a different tag from non-local file systems.
	 * 
	 * @param location
	 * @param writer
	 */
	...
	/**
	 * The parameter tempLocation is a location to place our temp file (copy of the target one) to
	 * be used in case we could not successfully write the new file.
	 */
