...
public abstract class Resource extends PlatformObject implements IResource, ICoreConstants, Cloneable, IPathRequestor {
	...
	public static IResourceListener resourceListener= new IResourceListener() { //default stub that does nothing
		...
		public void aboutToSaveCompareEditor(Object compareEditor) {//stub
		}

		public void savedCompareEditor(Object compareEditor) {//stub
		}
	};
	...
	/**
	 * Throws an exception if the link preconditions are not met. Returns the file info for the file
	 * being linked to, or <code>null</code> if not available.
	 * 
	...
	/**
	 * This method reports errors in two different ways. It can throw a CoreException or return a
	 * status. CoreExceptions are used according to the specification of the copy method.
	 * Programming errors, that would usually be prevented by using an "Assert" code, are reported
	 * as an IStatus. We're doing this way because we have two different methods to copy resources:
	 * IResource#copy and IWorkspace#copy. The first one gets the error and throws its message in an
	 * AssertionFailureException. The second one just throws a CoreException using the status
	...
	/**
	 * Checks that this resource does not exist. If the file system is not case sensitive, this
	 * method also checks for a case variant.
	 */
	...
	/**
	 * This method reports errors in two different ways. It can throw a CoreException or log a
	 * status. CoreExceptions are used according to the specification of the move method.
	 * Programming errors, that would usually be prevented by using an "Assert" code, are reported
	 * as an IStatus. We're doing this way because we have two different methods to move resources:
	 * IResource#move and IWorkspace#move. The first one gets the error and throws its message in an
	 * AssertionFailureException. The second one just throws a CoreException using the status
	 * returned by this method.
	 * 
	...
	/**
	 * Count the number of resources in the tree from this container to the specified depth. Include
	 * this resource. Include phantoms if the phantom boolean is true.
	 */
	...
	/**
	 * This method should be called to delete a resource from the tree because it will also delete
	 * its properties and markers. If a status object is provided, minor exceptions are added,
	 * otherwise they are thrown. If major exceptions occur, they are always thrown.
	 */
	...
	/**
	 * Helper method for case insensitive file systems. Returns an existing resource whose path
	 * differs only in case from the given path, or null if no such resource exists.
	 */
	...
	/**
	 * Searches for a variant of the given target in the list, that differs only in case. Returns
	 * the variant from the list if one is found, otherwise returns null.
	 */
	...
	/**
	 * Note the depth parameter is intentionally ignored because this method is over-ridden by
	 * Container.isLocal().
	 * 
	 * @deprecated
	 */
	...
	/**
	 * Returns true if this resource is a linked resource, or a child of a linked resource, and
	 * false otherwise.
	 */
	...
	/**
	 * Calls the move/delete hook to perform the deletion. Since this method calls client code, it
	 * is run "unprotected", so the workspace lock is not held.
	 */
	...
	/**
	 * Calls the move/delete hook to perform the move. Since this method calls client code, it is
	 * run "unprotected", so the workspace lock is not held. Returns true if resources were actually
