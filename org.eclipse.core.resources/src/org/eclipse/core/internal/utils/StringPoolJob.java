...
public class StringPoolJob extends Job {
	...
	/**
	 * Stores all registered string pool participants, along with the scheduling rule required when
	 * running it.
	 */
	...
	/**
	 * Adds a string pool participant. The job periodically builds a string pool and asks all
	 * registered participants to share their strings in the pool. Once all participants have added
	 * their strings to the pool, the pool is discarded to avoid additional memory overhead.
	 * 
	 * Adding a participant that is equal to a participant already registered will replace the
	 * scheduling rule associated with the participant, but will otherwise be ignored.
	 * 
	 * @param participant The participant to add
	 * @param rule The scheduling rule that must be owned at the time the participant is called.
	 *            This allows a participant to protect their data structures against access at
	 *            unsafe times.
	 * 
	...
	/**
	 * Removes the indicated log listener from the set of registered string pool participants. If no
	 * such participant is registered, no action is taken.
	 * 
