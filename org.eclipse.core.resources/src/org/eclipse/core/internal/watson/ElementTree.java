...
/**
 * An ElementTree can be viewed as a generic rooted tree that stores a hierarchy of elements. An
 * element in the tree consists of a (name, data, children) 3-tuple. The name can be any String, and
 * the data can be any Object. The children are a collection of zero or more elements that logically
 * fall below their parent in the tree. The implementation makes no guarantees about the ordering of
 * children.
 * 
 * Elements in the tree are referenced by a key that consists of the names of all elements on the
 * path from the root to that element in the tree. For example, if root node "a" has child "b",
 * which has child "c", element "c" can be referenced in the tree using the key (/a/b/c). Keys are
 * represented using IPath objects, where the Paths are relative to the root element of the tree.
 * 
 * @see IPath
 * 
 *      Each ElementTree has a single root element that is created implicitly and is always present
 *      in any tree. This root corresponds to the key (/), or the singleton <code>Path.ROOT</code>.
 *      The root element cannot be created or deleted, and its data and name cannot be set. The root
 *      element's children however can be modified (added, deleted, etc). The root path can be
 *      obtained using the <code>getRoot()</code> method.
 * 
 *      ElementTrees are modified in generations. The method <code>newEmptyDelta()</code> returns a
 *      new tree generation that can be modified arbitrarily by the user. For the purpose of
 *      explanation, we call such a tree "active". When the method <code>immutable()</code> is
 *      called, that tree generation is frozen, and can never again be modified. A tree must be
 *      immutable before a new tree generation can start. Since all ancestor trees are immutable,
 *      different active trees can have ancestors in common without fear of thread corruption
 *      problems.
 * 
 *      Internally, any single tree generation is simply stored as the set of changes between itself
 *      and its most recent ancestor (its parent). This compact delta representation allows chains
 *      of element trees to be created at relatively low cost. Clients of the ElementTree can
 *      instantaneously "undo" sets of changes by navigating up to the parent tree using the
 *      <code>getParent()</code> method.
 * 
 *      Although the delta representation is compact, extremely long delta chains make for a large
 *      structure that is potentially slow to query. For this reason, the client is encouraged to
 *      minimize delta chain lengths using the <code>collapsing(int)</code> and
 *      <code>makeComplete()</code> methods. The <code>getDeltaDepth()</code> method can be used to
 *      discover the length of the delta chain. The entire delta chain can also be re-oriented in
 *      terms of the current element tree using the <code>reroot()</code> operation.
 * 
...
public class ElementTree {
	 ...
	 * 
	 * <p>
	 * This operation should be used to collapse chains of element trees created by
	 * newEmptyDelta()/immutable().
	 * 
	 * <p>
	 * This element tree must be immutable at the start of this operation, and will be immutable
	 * afterwards.
	 * 
	 * @return this tree.
	 */
	...
	/**
	 * Creates the indicated element and sets its element info. The parent element must be present,
	 * otherwise an IllegalArgumentException is thrown. If the indicated element is already present
	 * in the tree, its element info is replaced and any existing children are deleted.
	 * 
	...
	/**
	 * Creates or replaces the subtree below the given path with the given tree. The subtree can
	 * only have one child below the root, which will become the node specified by the given key in
	 * this tree.
	 * 
	...
	/**
	 * Given an array of element trees, returns the index of the oldest tree. The oldest tree is the
	 * tree such that no other tree in the array is a descendent of that tree. Note that this
	 * counter-intuitive concept of oldest is based on the ElementTree orientation such that the
	 * complete tree is always the newest tree.
	 */
	...
	/**
	 * Returns the number of children of the element specified by the given path. The given element
	 * must be present in this tree.
	 */
	...
	/**
	 * Returns the IDs of the children of the specified element. If the specified element is null,
	 * returns the root element path.
	 */
	...
	/**
	 * Returns the paths of the children of the element specified by the given path. The given
	 * element must be present in this tree.
	 */
	...
	/**
	 * Returns the element data for the given element identifier. The given element must be present
	 * in this tree.
	 */
	...
	/**
	 * Returns the names of the children of the specified element. The specified element must exist
	 * in the tree. If the specified element is null, returns the root element path.
	 */
	...
	/**
	 * Returns the subtree rooted at the given key. In the resulting tree, the implicit root node
	 * (designated by Path.ROOT), has a single child, which is the node specified by the given key
	 * in this tree.
	 * 
	...
	/**
	 * Returns true if there have been changes in the tree between the two given layers. The two
	 * must be related and new must be newer than old. That is, new must be an ancestor of old.
	 */
	...
	/**
	 * Returns true if this element tree includes an element with the given key, ignoring the case
	 * of the key, and false otherwise.
	 */
	...
	/**
	 * Merges a chain of deltas for a certain subtree to this tree. If this tree has any data in the
	 * specified subtree, it will be overwritten. The receiver tree must be open, and it will be
	 * made immutable during the merge operation. The trees in the provided array will be replaced
	 * by new trees that have been merged into the receiver's delta chain.
	 * 
	 * @param path The path of the subtree chain to merge
	 * @param trees The chain of trees to merge. The trees can be in any order, but they must all
	 *            form a simple ancestral chain.
	...
	/**
	 * Creates a new element tree which is represented as a delta on this one. Initially they have
	 * the same content. Subsequent changes to the new tree will not affect this one.
	 */
	...
	/**
	 * Returns a mutable copy of the element data for the given path. This copy will be held onto in
	 * the most recent delta. ElementTree data MUST implement the IElementTreeData interface for
	 * this method to work. If the data does not define that interface this method will fail.
	 */
	public synchronized Object openElementData(IPath key) {
		...
		if (lookup.isPresent) {
			if (lookup.foundInFirstDelta)
				return lookup.data;
			/**
			 * The node has no data in the most recent delta. Pull it up to the present delta by
			 * setting its data with a clone.
			 */
	...
	/**
	 * Sets the element for the given element identifier. The given element must be present in this
	 * tree.
	 * 
