...
/**
 * A class for performing operations on each element in an element tree. For example, this can be
 * used to print the contents of a tree.
 * <p>
 * When creating an ElementTree iterator, an element tree and root path must be supplied. When the
 * <code>iterate()</code> method is called, a visitor object must be provided. The visitor is called
 * once for each node of the tree. For each node, the visitor is passed the entire tree, the object
...
public class ElementTreeIterator implements IPathRequestor {
	...
	/**
	 * Iterates through the given element tree and visit each element (node) passing in the
	 * element's ID and element object.
	 */
	...
	/**
	 * Iterates through this iterator's tree and visits each element in the subtree rooted at the
	 * given path. The visitor is passed each element's data and a request callback for obtaining
	 * the path.
	 */
