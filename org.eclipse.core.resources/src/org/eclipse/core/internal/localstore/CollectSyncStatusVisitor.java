...
//
/**
 * Visits a unified tree, and collects local sync information in a multi-status. At the end of the
 * visit, the resource tree will NOT be synchronized with the file system, but all discrepancies
 * between the two will be recorded in the returned status.
 */
public class CollectSyncStatusVisitor extends RefreshLocalVisitor {
	protected List affectedResources;

	/**
	 * Determines how to treat cases where the resource is missing from the local file system. When
	 * performing a deletion with force=false, we don't care about files that are out of sync
	 * because they do not exist in the file system.
	 */
	...
	/**
	 * Returns the list of resources that were not synchronized with the local file system, or
	 * <code>null</code> if all resources are synchronized.
	 */
