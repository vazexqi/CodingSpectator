 ...
 * <code>DeltaDataTree</code>. It is a node that existed in the parent tree, but no longer exists in
 * the current delta tree. It has no children or data.
 */
public class DeletedNode extends AbstractDataTreeNode {
	...
	AbstractDataTreeNode compareWithParent(IPath key, DeltaDataTree parent, IComparator comparator) {
		/**
		 * Just because there is a deleted node, it doesn't mean there must be a corresponding node
		 * in the parent. Deleted nodes can live in isolation.
		 */
	...
	/**
	 * Return a unicode representation of the node. This method is used for debugging purposes only
	 * (no NLS please)
	 */
