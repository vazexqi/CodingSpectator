...
/**
 * This class and its subclasses are used to represent nodes of AbstractDataTrees. Refer to the
 * DataTree API comments for more details.
 * 
...
public abstract class AbstractDataTreeNode {
	...
	/**
	 * Returns a node which if applied to the receiver will produce the corresponding node in the
	 * given parent tree.
	 * 
	...
	/**
	 * If this node is a node in a comparison tree, this method reverses the comparison for this
	...
	/**
	 * Returns the result of assembling nodes with the given forward delta nodes. Both arrays must
	 * be sorted by name. The result is sorted by name. If keepDeleted is true, explicit
	 * representations of deletions are kept, otherwise nodes to be deleted are removed in the
	 * result.
	 */
	...
	/**
	 * Returns the child with the given local name. Returns null if the child does not exist.
	 * 
	...
	/**
	 * Returns the child with the given local name, ignoring case. If multiple case variants exist,
	 * the search will favour real nodes over deleted nodes. If multiple real nodes are found, the
	 * first one encountered in case order is returned. Returns null if no matching children are
	 * found.
	 * 
	 * @param localName name of child to retrieve
	 */
	...
	/**
	 * Replaces the receiver's children between "from" and "to", with the children in otherNode
	 * starting at "start". This method replaces the Smalltalk #replaceFrom:to:with:startingAt:
	 * method for copying children in data nodes
	 */
	...
	/**
	 * Returns true if the receiver has a child with the given local name, false otherwise
	 */
	...
	/**
	 * Returns true if the receiver represents delta information, false if it represents the
	...
	/**
	 * Returns a unicode representation of the node. This method is used for debugging purposes only
	 * (no NLS support needed)
	 */
